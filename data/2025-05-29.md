...
Motivation: 随着多模态大语言模型生成更长的推理链，其性能提升的同时也伴随着幻觉问题的加剧，即模型逐渐偏离图像内容而依赖语言先验。

Method: 通过注意力分析发现视觉输入关注度下降是幻觉的原因，提出RH-AUC指标量化感知准确性与推理长度的关系，并发布RH-Bench基准评估推理能力与幻觉的权衡。

Result: 研究发现（i）更大的模型通常能更好地平衡推理与感知，（ii）这种平衡更多受训练数据类型和领域影响而非数据总量。

Conclusion: 强调了同时评估推理质量和感知保真度的重要性，为未来模型设计提供了指导。
...
<details>
  <summary>Details</summary>
Motivation: 现有ASR数据集（如LibriSpeech）在规模和多样性上存在不足，且其他新数据集因许可证、转录质量或数据问题无法满足工业界需求。

Method: 提出Loquacious Set，包含多样化的语音类型（朗读、自发、演讲、干净、嘈杂）和大量不同口音的说话者。

Result: Loquacious Set为学术界和工业界提供了一个大规模、多样化的ASR基准数据集。

Conclusion: Loquacious Set填补了现有ASR数据集的空白，支持更真实的ASR系统开发。
...
<details>
  <summary>Details</summary>
Motivation: 在有限的计算资源下，不同领域数据的采样比例显著影响模型性能，因此需要研究如何确定最优的领域权重。

Method: 将现有方法细分为离线（启发式、算法式、函数拟合式）和在线（在线极小极大优化、在线混合法则等）方法，并总结了每种方法的算法和关系。

Result: 对各类方法的优缺点进行了分析，并明确了它们之间的联系与区别。

Conclusion: 论文总结了数据混合领域的关键挑战，并指出了未来研究方向。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型（LLMs）推理开销大，而蒸馏后的小语言模型（SLMs）效率高但性能不足，无法完全跟随LLMs的推理路径。

Method: 引入R2R方法，仅对关键令牌使用LLMs，其余令牌由SLM生成，并开发了自动数据生成管道训练轻量级路由器。

Result: 在数学、编程和QA任务上，R2R以平均5.6B参数规模，性能超越R1-7B，接近R1-32B，且速度提升2.8倍。

Conclusion: R2R在推理效率和性能之间取得了显著平衡，推动了测试时扩展效率的帕累托前沿。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏对LLMs在错误信息中具体表现和影响程度的细粒度分析，MisBench旨在填补这一空白。

Method: 提出MisBench基准，包含知识冲突和风格变化的错误信息，并基于实验结果提出Reconstruct to Discriminate（RtD）方法。

Result: LLMs在检测错误信息方面表现尚可，但仍易受知识冲突和风格变化的影响。RtD方法能有效提升LLMs的检测能力。

Conclusion: MisBench为评估LLMs与错误信息的交互提供了重要见解，可作为未来LLM检测器的基准，提升其在实际应用中的可靠性。
...
Method: 采用迭代框架，结合文献选择、Word2Vec模型训练和嵌入空间监测，预测高性能材料。

Result: 成功预测了氧还原、氢析出和氧析出反应中的高性能材料，并通过实验验证。

Conclusion: 迭代文献优化框架为材料发现提供了高效、可扩展的工具，尤其适用于数据稀缺的场景。
...
TLDR: SMILE是一种新方法，用于解释大型语言模型（如GPT、LLAMA和Claude）如何响应提示的不同部分，通过可视化热图展示关键部分，提高模型透明度。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型缺乏透明度，尤其在需要信任和问责的领域，这可能导致问题。

Method: SMILE通过微调输入并测量输出变化，识别提示中对输出影响最大的部分，生成可视化热图。

Result: 测试表明，SMILE在准确性、一致性、稳定性和保真度方面提供了清晰可靠的解释。

Conclusion: SMILE提升了AI模型的透明度和可信度，是迈向更透明AI的重要一步。
...
<details>
  <summary>Details</summary>
Motivation: 异常值对LLM的性能（如量化和压缩）有显著影响，但现有研究对其根源探索不足。

Method: 全面调查异常值的形成机制，并提出消除策略。

Result: 提出高效方法，消除大部分异常值且对精度影响最小。

Conclusion: 通过深入分析异常值，优化量化过程，提升LLM在边缘设备上的部署效率。
...
Method: 使用ILDC数据集（7,593份标注请愿书），结合DistilBERT、LegalBERT等嵌入技术和数值特征，训练多种机器学习模型（如随机森林、XGBoost）。

Result: 随机森林和决策树模型表现最佳，准确率超99%，Spearman秩相关0.99。数值特征已接近最优效果。

Conclusion: 自动化请愿排序可优化司法流程，减少积压，提升公平性，LLM嵌入仅带来边际增益。
...
<details>
  <summary>Details</summary>
Motivation: 由于LLMs的英语预训练可能导致跨语言文化偏见，而现有评估方法受限，因此需要一种全面的多语言评估框架。

Method: 利用Wikidata的多语言结构作为跨语言锚点，自动识别模型输出中的文化实体，并链接到结构化知识，提出四个文化意识指标。

Result: 评估了7个LLMs，发现模型在英语中表现出更强的文化意识，英语提示更有效激活文化知识。

Conclusion: MAKIEval为多语言文化意识评估提供了可扩展的解决方案，揭示了LLMs在跨语言文化表达上的差异。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型（LLM）的幻觉问题严重影响了其可靠性，因此需要精确识别其知识缺口。现有探测方法存在不一致性问题，亟需评估和改进。

Method: 提出了一种新方法，通过输入变化和定量指标评估知识缺口探测方法，重点分析了内部不一致性和跨方法不一致性。

Result: 研究发现：1）同一方法中，微小的非语义提示变化会导致知识缺口检测结果显著差异；2）不同方法之间对模型是否知道答案的判断高度不一致。

Conclusion: 现有知识缺口探测方法存在严重不一致性，亟需开发抗干扰的探测框架。
...
TLDR: ChatGPT在检测不当内容方面表现良好，但在针对性语言检测上存在变异性。研究强调了模型持续改进的重要性。


<details>
  <summary>Details</summary>
Motivation: 评估ChatGPT在在线评论中识别不当和针对性语言的有效性，以应对社交媒体内容审核的挑战。

Method: 比较ChatGPT与众包标注和专家评估的性能，分析其准确性、检测范围和一致性。

Result: ChatGPT在检测不当内容上表现优异，但在针对性语言检测上存在高假阳性率。

Conclusion: ChatGPT有潜力提升自动化内容审核系统，但需进一步改进模型和上下文理解能力。
...
<details>
  <summary>Details</summary>
Motivation: LLM的行为不可预测，尤其是在高风险场景中，解释其行为的能力至关重要。

Method: 扩展反事实可模拟性方法至生成任务，以新闻摘要和医疗建议为例进行评估。

Result: LLM解释在新闻摘要中帮助用户预测输出，但在医疗建议中效果有限；评估方法更适合技能型任务。

Conclusion: 反事实可模拟性评估方法在生成任务中具有潜力，但需针对任务类型优化。
...
<details>
  <summary>Details</summary>
Motivation: LLMs在临床辅助任务中表现被动，缺乏主动识别关键信息或风险的能力，需要行为适应。

Method: 提出BehaviorBench数据集评估行为表现，并设计BehaviorSFT训练策略，通过行为标记动态调整模型行为。

Result: BehaviorSFT在BehaviorBench上达到97.3%的Macro F1，主动任务表现提升（如Qwen2.5-7B-Ins从95.0%到96.5%）。临床盲评确认其行为更真实。

Conclusion: BehaviorSFT有效提升LLMs在临床任务中的主动性和平衡性，优于标准微调或显式指令方法。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）的置信度校准不足影响其可靠性，需要更准确的置信度估计方法。

Method: CCPS通过对抗性扰动最终隐藏状态，提取特征并利用轻量级分类器预测答案正确性。

Result: 在多个LLM和基准测试中，CCPS显著降低校准误差和Brier分数，同时提高准确性和其他指标。

Conclusion: CCPS提供了一种高效、广泛适用且更准确的LLM置信度估计方法，增强了模型的信任度。
...
<details>
  <summary>Details</summary>
Motivation: 研究低资源语言对的语音翻译性能，探索不同微调方法对E2E ST的影响。

Method: 微调SeamlessM4T-v2进行ASR、MT和E2E ST，并尝试直接E2E微调、多任务训练及参数初始化。

Result: 直接E2E微调效果强；使用微调ASR编码器提升未训练语言的ST性能；多任务训练略有帮助。

Conclusion: 直接E2E微调是有效方法，ASR编码器初始化对未训练语言有益，多任务训练效果有限。
...
TLDR: VeriTrail是一种新的方法，用于检测多步生成过程中的幻觉内容，并提供可追溯性。


<details>
  <summary>Details</summary>
Motivation: 语言模型在多步生成过程中更容易产生未经证实的幻觉内容，需要一种方法来检测并追溯其来源。

Method: 提出VeriTrail方法，设计数据集包含中间输出和人类标注的忠实度。

Result: VeriTrail在数据集上表现优于基线方法。

Conclusion: VeriTrail为多步生成过程中的幻觉检测提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 阿拉伯语方言多样性显著，但现有NLP任务中广泛采用的假设（如方言可按地区分组）未经定量验证，可能影响任务效果。

Method: 通过扩展和分析多标签数据集，手动评估11种国家级别方言中每个句子的有效性。

Result: 分析表明四种假设过于简化现实，部分假设并不总是准确。

Conclusion: 这些假设的准确性不足可能阻碍阿拉伯语NLP任务的进一步发展。
...
<details>
  <summary>Details</summary>
Motivation: 解决生成模型中多样性和偏见的不足，确保输出能按比例代表训练数据中的兴趣群体。

Method: 引入“代表性均匀和非均匀生成”概念，提出“群体闭包维度”作为关键组合量，分析无限假设类和群体集合下的信息论和计算问题。

Result: 证明了在特定条件下对无限假设类的可行性，但仅通过成员查询无法计算，与标准生成模型的结果形成对比。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在处理长上下文时存在“中间迷失”现象，导致对多文档摘要的源覆盖率不均。本文旨在通过分步内容选择提升覆盖率。

Method: 将摘要任务分为三步：1) 提取原子关键点；2) 使用DPP选择多样性内容；3) 重写为最终摘要。结合提示和DPP技术。

Result: 在DiverseSumm基准测试中，该方法显著提升了不同大语言模型的源覆盖率，并能生成个性化摘要。

Conclusion: 分步内容选择和DPP技术的结合有效提升了大语言模型在多文档摘要中的表现，同时支持个性化需求。
...
<details>
  <summary>Details</summary>
Motivation: 评估LLMs在RAG设置中的鲁棒性，解决RAG是否总是优于非RAG、更多检索文档是否总是提升性能以及文档顺序是否影响结果的问题。

Method: 建立包含1500个开放领域问题的基准，每个问题附带维基百科检索文档，并引入三个鲁棒性指标对应研究问题。

Result: 实验涉及11种LLMs和3种提示策略，结果显示LLMs表现出较高的检索鲁棒性，但仍存在不同程度的不足。

Conclusion: 尽管LLMs在RAG中表现出高鲁棒性，但仍有改进空间以充分利用RAG的优势。
...
<details>
  <summary>Details</summary>
Motivation: 在填充任务中，KV缓存重用常因提示格式的结构问题而受限，导致计算效率低下。

Method: 提出EFIM，一种改进的提示格式，并引入片段标记化训练方法，以解决部分词生成问题。

Result: 实验表明，EFIM能降低52%的延迟，提高98%的吞吐量，同时保持原有填充能力。

Conclusion: EFIM通过优化提示格式和训练方法，显著提升了LLMs在填充任务中的效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有多智能体系统在资源利用上效率低下，高令牌消耗和执行时间长。

Method: 利用历史成功轨迹中的“捷径”指令，绕过冗余推理，优化协作问题解决。

Result: 令牌使用减少50.85%，代码质量提升10.06%。

Conclusion: Co-Saving在资源效率和任务质量上优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有知识图谱基础模型主要关注结构信息，限制了其在更具挑战性的外部任务中的应用。

Method: 提出多视角条件消息传递（CMP）架构、动态残差融合模块和灵活边评分机制，整合文本与结构信息。

Result: 在28个数据集上评估，MERRY在多数场景中优于基线模型，展现了强大的推理和泛化能力。

Conclusion: MERRY通过融合文本与结构信息，显著提升了知识图谱内外任务的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有评估缺乏对混合Web-OS攻击场景的支持，RedTeamCUA旨在填补这一空白，提供更现实的测试环境。

Method: 提出RedTeamCUA框架，结合VM和Docker的混合沙盒，支持灵活的对抗场景配置和直接注入点测试。

Result: 测试显示CUAs存在显著漏洞，如Claude 3.7 Sonnet的ASR达42.9%，Claude 4 Opus更高达48%。
...
Method: 采用自适应图神经网络（GNN）方法IdiomCE，学习习语表达之间的复杂映射关系，并泛化到训练中未见过的节点。

Result: 在多个习语翻译数据集上评估，使用无参考指标，显著提升了英语到多种印度语言的习语翻译质量。

Conclusion: IdiomCE方法在资源受限环境下仍能提升翻译质量，适用于小型模型。
...
Result: 实验表明，RISE在多跳问答基准测试中显著提升了推理准确性和任务表现。

Conclusion: RISE通过迭代自我探索有效增强模型推理能力，为复杂推理任务提供了新解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 探索推理时重复采样方法在多语言生成任务中的有效性，填补该领域的研究空白。

Method: 使用基于困惑度和奖励的验证器，在Aya Evaluation Suite和m-ArenaHard两个多语言基准上评估重复采样方法。

Result: 结果显示质量持续提升，某些情况下增益超过35%；基于奖励的验证器在需要推理的任务中表现更优。

Conclusion: 重复采样对多语言文本生成具有广泛实用性，选择合适的验证器对任务至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 领域特定指令调优中，数据选择方法常忽略知识冲突问题，导致模型性能下降和幻觉问题。

Method: KDS框架通过两个知识感知指标（上下文-记忆知识对齐和记忆内知识一致性）量化知识冲突，筛选高质量数据。

Result: 在医学领域的实验中，KDS显著优于基线方法，提升模型性能并缓解幻觉问题。

Conclusion: KDS是一种简单有效的框架，能够优化数据选择，提升领域特定任务中LLMs的表现和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有后训练方法多为手动设计或仅优化单一组件，缺乏自动化构建完整管道的探索。

Method: LaMDAgent利用LLM代理系统探索模型生成技术、数据集和超参数配置，通过任务反馈优化管道。

Result: 实验显示LaMDAgent提升工具使用准确性9.0分，同时发现传统方法忽略的有效策略。
...
<details>
  <summary>Details</summary>
Motivation: LVLMs在多模态任务中表现出色，但视觉输入的引入扩大了攻击面，暴露了新的安全漏洞。

Method: 通过系统表征分析，揭示传统对抗性攻击如何绕过LVLMs的安全机制，并提出两阶段评估框架。

Result: 框架能区分指令不遵从、直接拒绝和成功攻击，并量化模型输出对有害意图的满足程度。

Conclusion: 提出了一种规范性模式，定义了面对有害提示时的理想模型行为，为多模态系统的安全对齐提供了目标。
...
<details>
  <summary>Details</summary>
Motivation: 主流视觉语言模型（LVLMs）存在文化偏见，需要多样化的多模态数据集来弥补这一不足。

Method: 通过高级代理工作流程和45名阿拉伯世界注释者的人工标注，构建了包含K个多模态示例的Pearl数据集，涵盖十个文化重要领域。

Result: 评估表明，基于推理的指令对齐显著提升了模型的文化理解能力，优于传统扩展方法。

Conclusion: Pearl为文化感知的多模态建模研究提供了基础资源，所有数据集和基准已公开。
...
<details>
  <summary>Details</summary>
Motivation: 解决混合方法研究中定量和定性数据对齐的挑战，探索LLMs在生成合成调查响应中的潜力。

Method: 使用BREQ问卷和访谈数据，通过LLMs生成预测响应，分析其与人类响应的对齐程度。

Result: LLMs能捕捉整体响应模式但变异性较低，访谈数据和提示设计对结果有显著影响。

Conclusion: LLMs在结合定性数据生成定量响应方面有潜力，但需优化提示设计和模型设置以提高有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有评估方法需要昂贵标注数据，且难以处理开放式生成任务。本文旨在提供一种更高效的方法来评估LLM的多语言性能。

Method: 提出基于“翻译后评估”的框架，从信息和同理心两个维度评估LLM的跨语言一致性。

Result: 发现LLM在三十种语言中存在明显不一致性，某些语系和文字表现尤其差。

Conclusion: 跨语言评估需多维度一致性，本文框架可用于未来多语言LLM基准测试。
...
<details>
  <summary>Details</summary>
Motivation: 印度许多公民因法律意识不足和法律信息获取困难而无法有效行使权利，因此需要一种高效的法律辅助工具。

Method: 模型通过检索定制数据库中的法律信息生成准确回答，并在印度法律领域数据集（如印度宪法、BNS、BNSS等）上进行了微调。

Result: 模型在AIBE测试中得分60.08%，优于GPT-3.5 Turbo和Mistral 7B，避免了幻觉问题，表现出高效和可靠性。

Conclusion: Legal Assist AI展示了在实际法律场景中的应用潜力，未来将扩展数据集以覆盖更多语言和案例查询。
...
<details>
  <summary>Details</summary>
Motivation: 解决推理优化模型在简单问题上过度思考导致的低效问题。

Method: CoThink方法：指令模型生成大纲，推理模型完成细节，动态调整推理深度。

Result: 在三个数据集上，CoThink减少22.3%的token生成，准确率仅下降0.42%。

Conclusion: CoThink能有效提升推理效率，并可能揭示LLM的效率扩展规律。
...
Method: 采用两阶段方法：1）滑动窗口同步重叠标记；2）First-Fit-Decreasing算法打包短文本以减少填充和截断。

Result: 在多种模型架构和语料领域上，SP方法在99%的情况下优于基线方法。

Conclusion: SP策略有效解决了数据打包中的截断和上下文不连续问题，显著提升了模型性能。
...
Method: 引入VRAG-RL框架，模型通过视觉感知令牌自主采样推理轨迹，并基于动作空间（如裁剪和缩放）优化。

Result: 框架解决了视觉信息推理不足和检索查询不准确的问题，优化了模型性能。

Conclusion: VRAG-RL通过强化学习策略优化视觉语言模型，提升了视觉丰富信息检索与推理的实际应用效果。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型（LLM）在关键应用中的快速部署，亟需稳健的安全基准测试方法。

Method: JBDistill利用少量开发模型和现有越狱攻击算法生成候选提示池，并通过提示选择算法筛选有效子集作为安全基准。

Result: 实验表明，JBDistill在13种不同评估模型上表现优异，显著优于现有基准。
...
<details>
  <summary>Details</summary>
Motivation: 探索如何将语音克隆技术应用于代表性不足的语言，如瑞士德语方言。

Method: 预处理瑞士播客数据集，自动转录并标注方言类别，微调XTTSv2模型。

Result: 模型在人类和自动化评估中表现良好，CMOS得分-0.28，SMOS得分3.8。

Conclusion: 展示了语音克隆技术对少数语言的适应潜力。
...
<details>
  <summary>Details</summary>
Motivation: RAG系统直接传递私有文档给LLMs易受MIA攻击，需保护隐私。

Method: 提出Mirabel框架，基于查询与目标文档的高相似性检测MIA，并采用检测-隐藏策略。

Result: 实验证明Mirabel能有效防御多种MIA方法，且不影响数据实用性。

Conclusion: Mirabel为私有RAG系统提供了一种高效、通用的MIA防御方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究表明，强化学习训练的LLMs在数学任务中仅优化推理路径而非推理能力，而监督微调可以。本文从科学信息提取（SciIE）角度探讨这一问题，提出改进方法。

Method: 1. MimicSFT：使用结构化推理模板，无需高质量思维链数据；2. R²GRPO：结合相关性和规则诱导奖励的强化学习。

Result: 实验表明，两阶段训练显著提升推理能力，R²GRPO结合MimicSFT在关系提取任务中超越基线LLMs和专用监督模型。

Conclusion: 通过两阶段训练（MimicSFT和R²GRPO），LLMs在SciIE任务中的推理能力得到显著提升，为相关领域提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 尽管指令微调的LLM在未见任务上表现良好，但在需要领域知识的任务中仍存在困难。本文希望通过专门针对CA领域的微调解决这一问题。

Method: 通过分析现有CA研究，设计了105个CA任务的自然语言指令，并构建了一个CA专用基准。利用自指导过程生成了52k条CA相关指令，用于训练CA专用LLM。

Result: 实验表明，CA专用指令微调显著提升了LLM在CA任务（包括未见任务）上的表现，同时不影响其在通用NLP任务上的性能。

Conclusion: 专门针对领域的指令微调可以有效提升LLM在特定任务上的能力，同时保持其通用性。
...
Method: 提出R1-Router框架，动态决定检索时机和目标KB，并引入Step-GRPO算法优化推理行为。

Result: 在开放域QA基准测试中，R1-Router性能提升超过7%，减少不必要检索，提高效率。

Conclusion: R1-Router通过动态检索和强化学习，显著提升了多模态检索增强生成的效率和准确性。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的方法因参数知识有限，难以覆盖多样化和领域特定的查询，导致生成的SQL准确性不足。

Method: 构建一个综合知识库，基于问题和相关数据库模式及其知识，支持检索和生成所需知识，适用于不同数据集和领域。

Result: 在多个文本到SQL数据集上验证，包括重叠和非重叠数据库场景，显著优于基线方法。

Conclusion: 提出的知识库方法有效提升了文本到SQL的准确性，具有广泛适用性和可重用性。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM主要依赖参数化内存和临时激活内存，缺乏统一和结构化的内存架构，限制了长期知识演化和多模态集成能力。

Method: 提出MemOS，通过MemCube标准化内存抽象，统一管理参数化、激活和明文三种内存类型，支持跟踪、融合和迁移。

Result: MemOS建立了以内存为中心的执行框架，具有强可控性、适应性和可演化性。
...
<details>
  <summary>Details</summary>
Motivation: 传统注意力机制在长上下文建模中存在计算冗余问题，所有令牌消耗相同计算资源，而实际注意力权重稀疏，仅少数令牌对预测有显著贡献。

Method: 将概率序列建模重新定义为监督学习任务，分析注意力稀疏性，提出分组编码策略，并设计动态分组注意力（DGA）来显式减少冗余。

Result: DGA显著降低计算成本，同时保持竞争力性能。

Conclusion: DGA通过分组编码策略有效减少注意力计算冗余，为长上下文建模提供高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型（LRMs）在复杂任务中表现优异，但在简单任务中因过度思考导致计算效率低下，浪费资源。

Method: 引入Think-Bench基准和新效率指标，从推理过程、结果质量和思维链（CoT）特性多维度评估LRMs。

Result: 多数LRMs在简单问题上过度思考，生成冗长推理链，部分模型CoT质量高但效率低。

Conclusion: Think-Bench为LRMs研究提供了坚实基础，推动效率优化。
...
<details>
  <summary>Details</summary>
Motivation: 术中低血压（IOH）与不良后果相关，但预测困难，需整合静态和动态数据。

Method: 采用两阶段训练策略：领域自适应预训练和任务微调，结合多模态融合技术。

Result: 在实验中，IOHFuseLM优于基线方法，能更准确识别IOH事件。

Conclusion: IOHFuseLM在临床决策支持中具有潜力，代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: 解决多语言和跨语言环境下事实核查声明检索的挑战，尤其是在资源有限语言和全球性议题中。

Method: 采用负样本选择（监督学习）和重排序（无监督学习）策略，并在47种语言的数据集上评估。

Result: LLM重排序效果最佳，其次是基于句子相似性的负样本采样。跨语言检索具有独特特性。

Conclusion: 跨语言检索需特殊处理，LLM重排序和负样本采样是有效策略。
...
Conclusion: LoKI通过结合LLM知识存储机制与微调目标，实现了任务专业化与通用能力保留的最佳权衡。
...
TLDR: 论文提出EULER模型，通过生成高质量的错误解决方案来增强大语言模型（LLMs）的数学推理能力，实验显示其性能优于基线模型4%以上。


<details>
  <summary>Details</summary>
Motivation: 从错误中学习能提升LLMs在监督微调（SFT）中的表现，但现有方法难以针对每个数学问题生成错误解决方案。

Method: EULER模型优化错误暴露模型，增加自生成错误的概率，同时利用优质LLM的解决方案规范生成质量。

Result: 实验证明EULER在多个数学问题数据集上有效，性能提升超过4%，并能生成更具挑战性和教育意义的错误。

Conclusion: EULER通过高质量错误生成显著提升了LLMs的数学推理能力，为训练和推理过程提供了支持。
...
TLDR: RAD框架通过自推测解码识别Transformer中的冗余层，并用SSM组件替换，结合自蒸馏提升性能。


<details>
  <summary>Details</summary>
Motivation: 优化混合模型（Transformer和SSM）的性能与效率，解决Transformer组件的冗余问题。

Method: 使用自推测解码诊断冗余层，选择性替换为SSM组件，并进行针对性自蒸馏。

Result: 在数学和编码任务上显著超越基线模型，收敛速度提升2倍，GSM8K和CRUX得分显著提高。

Conclusion: RAD为混合模型的蒸馏提供了高效优化和性能增强的新途径。
...
TLDR: 该研究重新评估了BERT类模型在识别论点任务中的泛化能力，发现模型依赖词汇捷径而非真正任务对齐，但特定预训练和联合训练能提升性能。


<details>
  <summary>Details</summary>
Motivation: 识别论点是自动化话语分析的关键任务，但现有模型在泛化能力上表现不佳，需重新评估其实际效果。

Method: 评估了四种Transformer模型（包括一种增强对比预训练的模型）在17个英文句子级数据集上的表现。

Result: 模型依赖词汇捷径，泛化能力有限，但在特定预训练和联合训练下表现有所提升。

Conclusion: 模型需改进以真正理解任务，特定训练方法能增强泛化能力。
...
TLDR: InComeS框架通过压缩和选择机制提升LLMs处理编辑上下文的能力，解决了现有模型编辑方法在复杂语义场景中的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有模型编辑方法在需要深层语义理解的复杂场景中表现不佳，且受限于LLMs的上下文窗口。

Method: 提出InComeS框架，通过压缩编辑上下文为KV缓存，并动态选择相关信息。

Result: 实验证明InComeS在多样化的编辑任务中高效且有效。

Conclusion: InComeS通过灵活的设计提升了LLMs的编辑能力，适用于复杂场景。
...
TLDR: 本文提出了一种高效且通用的数据选择方法，通过多步骤流程实现高质量数据筛选，适用于多用途模型微调。


<details>
  <summary>Details</summary>
Motivation: 现有数据选择方法计算成本高或适用范围有限，需要一种高效且通用的解决方案。

Method: 采用多步骤流程：数据分组、质量评估、难度评分，并结合任务分类和聚类算法保证多样性。

Result: 实现了高性能微调且计算开销低。

Conclusion: 该方法为多用途模型微调提供了一种高效且通用的数据选择策略。
...
TLDR: NeoDiff是一种新型扩散模型，结合离散和连续扩散模型的优势，通过Poisson扩散过程和自适应时间预测器实现更精细的噪声控制和语义感知生成。


<details>
  <summary>Details</summary>
Motivation: 离散和连续扩散模型各有局限性：离散模型缺乏细粒度控制，连续模型无法捕捉语义差异。NeoDiff旨在整合两者的优势。

Method: NeoDiff采用Poisson扩散过程实现灵活噪声控制，并使用时间预测器自适应调节去噪进度。优化推理调度以提升性能。

Result: 实验表明，NeoDiff在文本生成任务中优于基线模型，包括非自回归和自回归扩散方法。
...
TLDR: 论文提出了一种基于随机方法评估LLM提示敏感性的框架ReliableEval，强调标准基准测试的局限性，并通过实验验证了即使顶级模型如GPT-4o和Claude-3.7-Sonnet也存在显著的提示敏感性。


<details>
  <summary>Details</summary>
Motivation: 标准基准测试通常使用单一提示评估LLM性能，忽视了提示敏感性，可能导致评估结果不可靠。

Method: 提出ReliableEval方法，通过随机采样意义保留的提示扰动，定义可靠评估标准，并估计所需提示重采样次数。

Result: 实验表明，即使是GPT-4o和Claude-3.7-Sonnet等顶级模型也存在显著的提示敏感性。

Conclusion: ReliableEval提供了一种模型、任务和指标无关的LLM评估方法，增强了评估的鲁棒性和意义。
...
TLDR: 论文提出了一种名为反向偏好优化（RPO）的方法，用于解决大型语言模型在复杂指令跟随任务中的多约束对齐问题，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 处理复杂指令时，现有方法基于约束满足数量选择偏好对，但会引入噪声，导致选中的响应可能不完美，而被拒绝的响应可能在某些方面表现更好。

Method: RPO通过动态反转指令中的约束，确保选中响应完美，减少噪声，同时扩大选中与被拒绝响应的差距，明确优化方向。

Result: 在Sysbench和Multi-IF基准测试中，RPO分别比DPO基线平均提升4.6和2.5分（Llama-3.1 8B），且70B参数的RPO模型超越GPT-4o。

Conclusion: RPO是一种简单有效的方法，能够显著提升语言模型在多约束指令跟随任务中的性能，且具有良好的扩展性。
...
TLDR: 提出了一种名为TabXEval的两阶段评估框架，结合结构对齐和语义句法比较，用于全面评估表格，并通过TabXBench验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 传统指标难以捕捉表格的结构和内容差异，需要一种更全面的评估方法。

Method: TabXEval框架分两阶段：TabAlign结构对齐和TabCompare语义句法比较，使用TabXBench进行验证。
...
TLDR: 论文研究了如何结合推测解码和量化技术来加速大语言模型的内存受限推理，提出了一种分层框架，显著提升了4位量化模型的性能。


<details>
  <summary>Details</summary>
Motivation: 推测解码和量化技术各自能优化大语言模型的推理效率，但结合使用时，4位量化模型的性能提升受到推测解码计算负载的限制。

Method: 提出了一种分层框架，利用小模型将树状推测转化为序列推测，以充分利用量化模型的内存访问优势。

Result: 实验表明，该方法在4位量化的Llama-3-70B模型上实现了2.78倍的加速，优于EAGLE-2的1.31倍。

Conclusion: 分层框架有效结合了推测解码和量化的优势，显著提升了量化模型的推理效率。
...
TLDR: 提出了一种无需训练和提示的方法C²TU，用于揭示中文社交媒体中的伪装毒性内容，通过子串匹配和语义过滤实现高效识别。


<details>
  <summary>Details</summary>
Motivation: 社交媒体中伪装毒性内容（如同音字替换）的增加对内容审核提出挑战，现有方法主要针对英文，中文领域尚未解决。

Method: C²TU基于子串匹配识别候选毒性词，结合BERT和LLMs过滤非毒性内容并修正伪装词。

Result: 在两个中文毒性数据集上表现优异，F1分数和准确率分别比最佳竞争对手高出71%和35%。

Conclusion: C²TU为中文伪装毒性内容识别提供了高效且无需训练的解决方案。
...
TLDR: 论文探讨了预训练语言模型是否能在抽象语义单元上进行推理，提出了一种框架将模型从词级提升到句子级，并在多个领域验证了其性能。


<details>
  <summary>Details</summary>
Motivation: 人类推理基于高级抽象（如句子和概念），而语言模型通常逐词生成。研究旨在探索模型是否能在语义单元上进行类似推理。

Method: 提出一种框架，通过自回归预测连续句子嵌入，探索了语义嵌入和上下文嵌入两种范式，并比较了离散化和连续化推理方式。

Result: 在数学、逻辑、常识和规划四个领域中，上下文嵌入在连续推理下性能接近Chain-of-Thought，同时推理效率提升一倍。

Conclusion: 研究表明预训练语言模型可以在潜在嵌入空间中实现抽象结构化推理，并展示了可扩展性和模块化适应性。
...
TLDR: JQL是一种高效的多语言数据筛选方法，利用轻量级标注器提升数据质量和多样性，显著优于现有启发式方法。


<details>
  <summary>Details</summary>
Motivation: 高质量多语言训练数据稀缺，现有方法依赖启发式过滤，限制了跨语言迁移和扩展性。

Method: JQL利用预训练多语言嵌入的轻量级标注器，系统化筛选高质量多语言数据。

Result: 在35种语言上验证，JQL显著优于Fineweb2等现有方法，提升下游模型训练质量和数据保留率。

Conclusion: JQL为多语言数据筛选提供了实用方案，提升了多语言数据集开发标准。
...
TLDR: 研究分析了TTS系统对句法边界的敏感性，发现系统在句法模糊的句子中难以准确生成语调短语边界，需依赖逗号等表面标记；而在简单句法结构中，系统能利用更深层次的句法线索。通过微调模型，可以提升语调模式对底层结构的反映。


<details>
  <summary>Details</summary>
Motivation: 探讨TTS系统在生成语调短语边界时对句法结构的依赖程度，尤其是在句法模糊的句子中的表现。

Method: 采用心理语言学启发的方法，分析TTS系统在生成语调短语边界时的表现，并针对句法模糊句子进行模型微调。

Result: TTS系统在句法模糊的句子中依赖表面标记（如逗号）生成语调短语边界，而在简单句法结构中能利用更深层次的句法线索。微调模型后，语调模式能更好地反映底层结构。

Conclusion: TTS系统在生成语调短语边界时对句法结构的敏感性有限，但通过微调可以提升其表现。
...
TLDR: BioHopR是一个新的生物医学领域多跳推理基准，填补了现有基准在多跳推理评估上的空白，并揭示了当前模型的性能差距。


<details>
  <summary>Details</summary>
Motivation: 现有基准无法评估生物医学领域中的多跳推理能力，尤其是涉及一对多和多对多关系的查询。

Method: 基于PrimeKG构建BioHopR，包含1跳和2跳推理任务，评估了包括O3-mini、GPT4O、HuatuoGPT-o1-70B和Llama-3.3-70B在内的多种模型。

Result: O3-mini在1跳任务中表现最佳（37.93%精度），但所有模型在多跳任务中性能显著下降。

Conclusion: BioHopR为生物医学多跳推理设定了新标准，揭示了专有模型与开源模型之间的差距，并推动未来生物医学LLM的发展。
...
TLDR: 论文提出了一种基于LLM生成Python代码的方法，用于解决表格数据问答任务，通过多步骤处理实现70.50%的得分。
...
Method: 利用LLM生成自然语言指令并转换为Python代码，分步骤处理表格内容、生成指令、翻译为代码、执行及错误处理。

Result: 在子任务1中取得了70.50%的得分。

Conclusion: 该方法通过优化提示和多步骤处理，有效提升了表格数据问答任务的性能。
...
TLDR: 论文研究了非正式表达的词汇规范化问题，针对未分词语言（如日语）提出了三项贡献：创建多领域数据集、开发基于预训练模型的方法，并进行多视角实验。


<details>
  <summary>Details</summary>
Motivation: 解决用户生成文本中非正式表达的处理问题，尤其是在未分词语言中缺乏全面评估的挑战。

Method: 基于最先进的预训练模型开发规范化方法，并进行多视角实验。

Result: 实验表明，仅编码器和仅解码器方法在准确性和效率上均表现良好。

Conclusion: 研究为未分词语言的词汇规范化提供了有效方法和数据集，展示了预训练模型的潜力。
...
TLDR: 本文综述了129项研究，探讨了自然语言处理（NLP）在循证医学（EBM）中的应用，重点介绍了NLP如何支持EBM的五个核心步骤，并提出了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 由于医学文献数量庞大且增长迅速，人工整理成本高昂，亟需利用NLP技术来识别、评估、综合和传播EBM中的证据。

Method: 通过系统回顾129项研究，分析了NLP在EBM中的应用，涵盖EBM的五个核心步骤。

Result: NLP在EBM中发挥了关键作用，能够优化证据提取、综合、评估和总结，提升临床决策效率。

Conclusion: NLP有潜力通过改进证据管理和临床工作流程来革新EBM，但仍需进一步研究解决现有局限性。
...
TLDR: 提出了一种名为Fragment-Shot Prompting的新方法，用于低资源语言的翻译，并通过实验验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 解决LLMs在低资源语言翻译中的挑战，特别是提示工程的问题。

Method: 引入Fragment-Shot Prompting和Pivoted Fragment-Shot，基于句法覆盖分段输入并检索翻译示例。

Result: 实验表明，该方法对低资源语言翻译有效，且模型推理能力越强，翻译质量越高。

Conclusion: 提示工程对低资源到高资源语言翻译改进有限，但新方法显著提升了低资源语言间的翻译质量。
...
TLDR: 360-LLaMA-Factory开源项目通过引入序列并行技术，广泛应用于多个模型和公司框架。


<details>
  <summary>Details</summary>
Motivation: 提升LLaMA-Factory的性能和适用性，支持更高效的模型训练。

Method: 在LLaMA-Factory中实现不同的序列并行模式。

Result: 项目被广泛认可并应用于多个模型和公司框架。

Conclusion: 序列并行技术为模型训练提供了高效解决方案。
...
TLDR: ToxEdit是一种动态检测和缓解大型语言模型中毒性内容的方法，通过自适应层间路径有效减少毒性，同时保持模型的通用能力。


<details>
  <summary>Details</summary>
Motivation: 现有的大型语言模型解毒方法依赖实体定位，对无明确实体的对抗输入无效，且容易过度编辑，影响模型性能。

Method: ToxEdit通过动态检测毒性激活模式，并自适应调整层间计算路径来缓解毒性。

Result: 实验表明，ToxEdit在解毒性能和保持模型通用能力上优于现有方法。

Conclusion: ToxEdit提供了一种更精确的毒性缓解方法，同时增强了模型的安全性评估。
...
TLDR: 论文研究了大型语言模型（LLMs）在反事实（假设性知识冲突）场景中的逻辑推理能力，发现其性能下降27%。提出了一种名为Self-Segregate的提示方法，将性能差距缩小至11%，并提升整体准确率7.5%。


<details>
  <summary>Details</summary>
Motivation: 探究LLMs在知识冲突场景中的推理能力退化现象。

Method: 引入CounterLogic数据集（1,800个例子，9种逻辑模式），评估11种LLMs在6个数据集上的表现，并提出Self-Segregate提示方法。

Result: LLMs在反事实场景中平均准确率下降27%，Self-Segregate方法将差距缩小至11%，整体准确率提升7.5%。

Conclusion: 研究为理解和提升LLMs在现实应用中的推理能力提供了实用见解，尤其是在需要独立于事实知识进行逻辑推理的场景。
...
TLDR: 提出了一种优化MoE模型的方法，通过正交性和方差损失提升专家专业化，显著提高性能。
...
Result: 实验显示方法显著提升专家专业化，性能提升达23.79%，且保持负载均衡。

Conclusion: 方法简单有效，无需架构修改即可提升MoE模型性能。
...
TLDR: 本文探讨了NLP在社会挑战中的作用，强调负责任的研究方向。


<details>
  <summary>Details</summary>
Motivation: 随着LLM的发展，NLP领域需要更负责任地部署技术以应对社会挑战。

Method: 通过跨学科分析社会目标和新兴风险，提出研究方向。

Result: 指出了NLP4SG研究中的挑战和潜在方向。

Conclusion: 呼吁确保NLP研究的负责任和公平进展。
...
TLDR: 论文提出了一种两阶段方法（监督微调+强化学习）来提升多模态推理能力，显著优于单一方法，并在多个基准测试中达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 尽管RL在多模态大语言模型（MLLMs）中表现出自我修正能力，但研究发现这些能力在RL训练前已存在，且不一定与推理性能提升相关。因此，论文探索如何更有效地结合监督微调（SFT）和RL来提升推理能力。

Method: 采用两阶段方法：1）使用结构化思维链推理模式进行监督微调（SFT）；2）通过GRPO进行强化学习（RL）进一步优化。

Result: 实验表明，该方法在多个多模态推理基准测试中显著优于单一SFT或RL方法，7B和3B模型均达到最先进水平，性能提升显著。

Conclusion: 论文为构建先进多模态推理模型提供了实用指导，并开源了代码。
...
TLDR: Text2Grad将自由形式的文本反馈转化为细粒度的梯度信号，通过局部调整模型策略，优于传统的标量奖励RL和仅提示基线。


<details>
  <summary>Details</summary>
Motivation: 传统RLHF使用粗粒度的标量奖励，掩盖了成功或失败的细节原因，导致学习缓慢且不透明。Text2Grad旨在通过细粒度的文本反馈改进模型优化。

Method: Text2Grad通过三个组件实现：(1) 反馈标注管道，将批评与标记跨度配对；(2) 细粒度奖励模型，预测跨度奖励并生成解释性批评；(3) 跨度级策略优化器，反向传播自然语言梯度。

Result: 在摘要、代码生成和问答任务中，Text2Grad均优于标量奖励RL和仅提示基线，任务指标更高且可解释性更强。

Conclusion: 自然语言反馈转化为梯度信号是一种强大的细粒度策略优化方法。
...
TLDR: 论文研究了LLMs如何处理虚假预设，并探讨了语言因素对其响应的影响。研究发现LLMs难以识别虚假预设，且表现因条件而异。


<details>
  <summary>Details</summary>
Motivation: 虚假预设可能嵌入误导信息，而LLMs可能无法像人类一样检测和纠正这些误导，尤其是在政治背景下。

Method: 基于语言预设分析的系统方法，实验使用了新数据集，测试了GPT-4-o、LLama-3-8B和Mistral-7B-v03三种模型。

Result: 模型难以识别虚假预设，表现因语言构造、政党等因素而异。

Conclusion: 语言预设分析是揭示LLMs强化政治错误信息的有效工具。
...
TLDR: Pangu Embedded是一种高效的LLM推理器，通过两阶段训练框架实现快速和慢速推理能力，显著降低计算成本和延迟。
...
Result: 在多个基准测试中表现优异，优于同类模型，提供快速响应和先进推理能力。

Conclusion: Pangu Embedded展示了开发强大且可实际部署的LLM推理器的潜力。
...
TLDR: RAG-Zeval是一个端到端框架，通过强化学习训练评估器，以规则引导的推理任务形式评估RAG系统的忠实性和正确性，显著降低计算成本并提升性能。
...
Method: 采用强化学习训练评估器，引入基于排名的奖励机制，通过质量控制的响应生成排名参考，无需人工标注。

Result: RAG-Zeval在性能上优于基线方法，与人类判断的相关性最强，且模型参数更少。
...
TLDR: MM-UPT是一种基于GRPO的无监督后训练框架，通过自奖励机制提升多模态大语言模型（MLLMs）的推理能力，无需外部监督。


<details>
  <summary>Details</summary>
Motivation: 传统监督方法（如SFT和RL）依赖昂贵的人工标注数据，而现有无监督方法复杂且难以迭代。MM-UPT旨在提供一种简单有效的无监督后训练方案。

Method: MM-UPT基于GRPO算法，采用多数投票的自奖励机制替代传统奖励信号，并利用MLLM自身生成的合成问题进一步提升性能。

Result: 实验显示，MM-UPT显著提升了Qwen2.5-VL-7B的推理能力（如MathVista上66.3%→72.9%，We-Math上62.9%→68.7%），且优于现有无监督基线，接近监督GRPO的结果。

Conclusion: MM-UPT为无外部监督下MLLMs的持续自主增强提供了新范式。
...
TLDR: EvolveSearch结合监督微调（SFT）和强化学习（RL），无需外部人工标注数据，提升大语言模型（LLM）在开放搜索领域的性能，实验显示其在多跳问答任务上平均提升4.7%。


<details>
  <summary>Details</summary>
Motivation: 当前主流方法（如SFT和RL）在开放搜索领域存在数据效率低或收敛快的问题，限制了LLM的搜索能力。

Method: 提出EvolveSearch框架，迭代结合SFT和RL，无需外部人工标注数据，自我进化提升搜索能力。

Result: 在七个多跳问答基准测试中，EvolveSearch性能持续提升，平均超越当前最优方法4.7%。

Conclusion: EvolveSearch为开放搜索领域的自我进化能力提供了新思路。
...
TLDR: 提出一种两阶段知识蒸馏框架，提升小型多模态大语言模型在低资源场景下检测多模态新闻上下文错误的能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖大量标注数据或昂贵API调用，不适用于低资源场景，需更高效且低成本的方法。

Method: 通过多教师模型生成预测和解释作为知识，采用两阶段蒸馏（LoRA微调和DPO）训练学生模型。

Result: 实验表明，仅用不到10%标注数据即可达到最先进性能。

Conclusion: 该方法显著降低了标注成本，提升了模型在复杂案例中的表现。
...
TLDR: 提出了一种任务自适应的推理框架，结合微调和强化学习，动态生成可变长度的推理链，显著提升了情感理解任务的性能。


<details>
  <summary>Details</summary>
Motivation: 当前方法依赖固定长度的推理链，无法适应情感任务的复杂性。

Method: 使用DeepSeek-R1生成可变长度推理链，结合微调和强化学习设计复合奖励函数。

Result: 在情感、情感、幽默和讽刺任务中，Acc和F1分数均有提升，高级任务提升尤为显著。

Conclusion: 通过自适应深度分析，弥合了固定推理链与情感复杂性之间的差距。
...
TLDR: ClaimPKG框架通过结合知识图谱（KGs）和大型语言模型（LLMs）提升声明验证的推理能力，实现端到端的解决方案。


<details>
  <summary>Details</summary>
Motivation: 现有声明验证方法依赖非结构化文本，难以有效利用KGs的结构化知识，而LLMs在多步推理和KG适应方面存在不足。

Method: ClaimPKG使用轻量级LLM生成伪子图指导子图检索模块，再利用通用LLM处理子图生成最终结果。

Result: 在FactKG数据集上，ClaimPKG性能领先基线9%-12%，并在非结构化数据集上展示零样本泛化能力。

Conclusion: ClaimPKG成功整合KGs与LLMs，显著提升声明验证性能，具有广泛适用性。
...
TLDR: 研究探讨大型语言模型（LLMs）与人类大脑在句子理解层面的计算原理是否一致，发现模型性能提升会使其表征架构更接近大脑的层次结构。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs与人类大脑在语言处理上的计算原理是否相似，尤其是模型规模扩展是否导致大脑类似模式的出现。

Method: 通过比较14种公开LLMs的分层表征与人类在自然叙事故事中的fMRI数据，构建句子级神经预测模型。

Result: 模型性能提升使其表征架构更接近大脑的层次结构，尤其在高级语义抽象层面表现出更强的功能和解剖对应。

Conclusion: LLMs的表征架构在性能提升过程中逐渐与人类大脑的语言处理层次对齐。
...
TLDR: 提出了一种基于LLM代理的统一检索增强生成（RAG）系统框架Agent-UniRAG，支持单跳和多跳查询，并引入合成数据集SynAgent-RAG以适配小型开源LLM。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统多针对单跳或多跳查询单独设计，限制了实际应用。

Method: 设计了一个可训练的LLM代理框架Agent-UniRAG，根据输入复杂度分步处理RAG任务，并开发了合成数据集SynAgent-RAG。

Result: 在多个RAG基准测试中表现与闭源及大型开源LLM相当。

Conclusion: Agent-UniRAG提升了RAG系统的效果与可解释性，适用于多种查询类型。
...
TLDR: Fusion Steering是一种激活引导方法，通过动态注入特定提示的激活增量提升LLM在QA任务中的事实准确性。


<details>
  <summary>Details</summary>
Motivation: 传统方法局限于单层或固定层操作，无法灵活调整。Fusion Steering旨在通过动态、分段的激活控制提升模型表现。

Method: 采用动态注入提示特定的激活增量，优化权重以平衡事实对齐和流畅性，支持全层和分段引导。

Result: 在260个SimpleQA提示上，分段引导的准确率达25.4%，显著优于基线（3.5%）和全层引导（16.2%）。

Conclusion: 分段动态干预策略有效，全网络激活控制具有潜力，未来可结合稀疏表示提升可解释性和扩展性。
...
TLDR: 论文提出了一种基于层间语言相似性的专家分配算法（LayerMoE），用于在扩展新语言时减少专家数量并减轻旧语言性能下降的问题。


<details>
  <summary>Details</summary>
Motivation: 构建强大的多语言大语言模型（LLM）需要不断扩展新语言，但现有方法参数成本高且难以避免对旧语言性能的影响。

Method: 通过分析LLM不同层的语言表征相似性，动态分配新专家数量，并在高相似层添加分类器引导路由。

Result: 实验表明，该方法在单次扩展和持续扩展场景下分别减少60%和33.3%的专家数量，性能优于现有基线。

Conclusion: LayerMoE通过层间相似性优化专家分配，有效降低了扩展成本并缓解了旧语言遗忘问题。
...
TLDR: PISCES是一种新框架，通过直接编辑参数空间中的方向，精确地从语言模型中擦除概念，效果优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前方法在移除语言模型中的敏感或受版权保护知识时效果有限，PISCES旨在提供更精确的擦除方案。

Method: PISCES使用解耦模型分解MLP向量为可解释特征，并通过自动解释技术识别目标概念，从参数中移除。

Result: 在Gemma 2和Llama 3.1上，PISCES将目标概念准确率降至7.7%，擦除特异性和鲁棒性分别提升31%和38%。

Conclusion: 基于特征的参数编辑为语言模型知识擦除提供了更精确可靠的方法。
...
TLDR: 论文提出Self-Error-Instruct (SEI)框架，通过分析错误类型并生成针对性训练数据，提升大语言模型在数学推理中的表现。
...
Method: 通过分析模型在GSM8K和MATH数据集上的错误案例，生成错误关键词并聚类，利用GPT-4o生成针对性训练数据，并通过迭代微调优化模型。

Result: 实验表明，SEI框架显著提升了模型在数学推理任务中的表现，包括域内和域外数据集。

Conclusion: SEI框架通过错误泛化和针对性训练数据生成，有效提升了大语言模型的数学推理能力。
...
TLDR: 提出了一种针对双向扩散模型的块状近似KV缓存机制和置信感知并行解码策略，显著提升了扩散LLM的推理速度，同时保持了生成质量。


<details>
  <summary>Details</summary>
Motivation: 开源扩散LLM的实际推理速度落后于自回归模型，且并行解码时生成质量下降。

Method: 引入块状近似KV缓存机制和置信感知并行解码策略。

Result: 在LLaDA和Dream模型上实现了最高27.6倍的吞吐量提升，且准确性损失极小。

Conclusion: 填补了扩散LLM与自回归模型之间的性能差距，为其实际部署铺平了道路。
...
TLDR: CoTalk是一种AI辅助的标注方法，通过顺序标注和多模态界面优化标注效率和全面性。


<details>
  <summary>Details</summary>
Motivation: 现有密集标注方法在优化人力标注效率方面不足，需系统性改进。

Method: 采用顺序标注减少冗余，结合多模态界面（读+说）提升效率。

Result: 标注速度（0.42单位/秒）和检索性能（41.13%）优于并行方法。

Conclusion: CoTalk在固定预算下显著提升标注效率和实用性。
...
TLDR: 该论文研究了大型语言模型（LLMs）在生成预测时如何错误地整合无关上下文，揭示了其内部机制中的结构化错误，即基于类别的（错误）泛化。


<details>
  <summary>Details</summary>
Motivation: 探讨LLMs在生成预测时错误整合无关上下文的规律性及其内部机制，以更细致地理解其行为。

Method: 通过行为分析和机制解释性实验，研究了Llama-3、Mistral和Pythia模型在39种事实回忆关系类型中的表现。

Result: 发现模型通过低层构建抽象类别表示，并在高层细化答案；特征选择由两个竞争电路控制，最终输出取决于它们的相对影响。

Conclusion: LLMs通过形式化训练可以表现出基于抽象的泛化能力，但依赖于不可靠的上下文线索，被称为“随机变色龙”。
...
TLDR: SKG2Data利用空间知识图谱指导多模态数据合成，提升多模态大语言模型的空间感知能力。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型的空间感知能力不足，需要一种能确保合成数据符合空间常识的方法。
...
TLDR: 论文提出了一种通过组合原子任务的链式思维（CoT）数据来提升大型语言模型（LLM）在未见组合任务上的零样本性能的方法，并通过拒绝采样微调（RFT）进一步优化。


<details>
  <summary>Details</summary>
Motivation: 解决现有方法依赖大量标注数据且泛化能力有限的问题，目标是实现组合式泛化，即通过组合原子推理技能解决更复杂的未见任务。

Method: 修改原子任务的CoT格式使其可组合，训练原子CoT模型，并通过多任务学习或模型合并提升零样本性能，最后用RFT微调少量组合数据。

Result: 在字符串操作和自然语言技能组合任务上，使用可组合CoT训练的模型优于多任务学习和持续微调的基线方法。

Conclusion: 可组合CoT训练能有效提升LLM在组合任务上的泛化能力，且在小规模数据下表现更优。
...
TLDR: 研究探讨了大型语言模型（LLM）在简体中文和繁体中文中的表现差异，发现任务和提示语言会影响模型偏向性。


<details>
  <summary>Details</summary>
Motivation: 了解LLM在简体中文和繁体中文中的表现差异，以避免因文化背景不同导致的潜在危害。

Method: 设计了两个基准任务（区域术语选择和区域名称选择），评估了11种商业和开源LLM的表现。

Result: 发现LLM在任务中表现偏向性：简体中文在术语选择中占优，繁体中文在名称选择中占优。

Conclusion: 需进一步分析LLM偏向性，并提供了开源基准数据集以促进未来研究。
...
TLDR: 论文提出了一种端到端的信息搜索代理训练范式，包括数据构建、轨迹采样、监督微调和强化学习四个阶段，并在WebDancer代理中实现，在GAIA和WebWalkerQA基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决复杂现实问题需要深入的信息搜索和多步推理，现有代理系统（如Deep Research）展示了自主多步研究的潜力。

Method: 提出四阶段训练范式：浏览数据构建、轨迹采样、监督微调（冷启动）和强化学习（泛化增强），并在WebDancer代理中实现。

Result: 在GAIA和WebWalkerQA基准测试中表现优异，验证了训练范式的有效性。

Conclusion: 该训练范式为开发更强大的代理模型提供了系统化路径，代码和演示将在GitHub发布。
...
TLDR: 研究探讨了奖励噪声对后训练大型语言模型（LLM）推理能力的影响，发现模型对高噪声具有强鲁棒性，且仅奖励关键推理短语即可达到与严格验证相当的性能。


<details>
  <summary>Details</summary>
Motivation: 现实场景中后训练LLM时奖励模型常存在噪声，研究旨在分析噪声对模型性能的影响。

Method: 通过手动翻转奖励函数输出引入噪声，并设计推理模式奖励（RPR）仅奖励关键推理短语。

Result: 模型在40%噪声下仍能快速收敛，RPR策略使Qwen-2.5-7B模型达到70%以上准确率，接近无噪声训练。

Conclusion: 研究强调预训练阶段提升基础能力的重要性，并为后训练技术提供新思路。
...
TLDR: GuessArena是一个基于对抗性游戏交互的自适应评估框架，旨在解决传统静态基准测试在适应性和细粒度评估上的不足。


<details>
  <summary>Details</summary>
Motivation: 传统静态基准测试缺乏对不同应用领域的适应性，且无法全面评估领域知识和上下文推理能力。

Method: 采用类似“猜猜我是谁”游戏的交互结构，结合动态领域知识建模和渐进式推理评估。

Result: 在五个垂直领域（金融、医疗、制造、信息技术和教育）中，GuessArena能有效区分LLM的领域知识覆盖和推理链完整性。
...
TLDR: AutoL2S框架动态调整LLMs的推理路径长度，减少冗余推理，提升效率。


<details>
  <summary>Details</summary>
Motivation: 解决LLMs在简单问题上生成过长推理路径的问题，降低推理成本和延迟。

Method: 提出AutoL2S框架，通过训练LLMs动态选择长短推理路径，并使用<EASY>标记简化推理。

Result: 推理路径长度减少57%，性能未受影响。
...
TLDR: 提出了一种无需训练的ViTA方法，通过模拟神经科学中的星形胶质细胞，提升预训练模型的解释性，使其更符合人类感知。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型精度高但解释性差，现有方法适用性有限，需探索新途径提升解释性。
...
TLDR: 论文探讨了DeepFake检测的局限性，提出多类别和归因模型的重要性，并通过实验验证了对比方法在跨数据集泛化中的有效性。


<details>
  <summary>Details</summary>
Motivation: 由于DeepFake生成技术的普及，现有二元检测模型无法区分不同篡改方法，亟需研究归因模型以提高检测的可信度和可解释性。

Method: 利用五种骨干模型在六个DeepFake数据集上进行实验，比较二元与多类别模型的泛化能力，并评估归因模型和对比方法的性能。
...
TLDR: 论文提出了一种硬件-算法协同设计框架CIM-NET，通过优化架构和引入伪卷积算子CIM-CONV，显著减少了矩阵-向量乘法操作，提升了边缘设备上的视频去噪效率。


<details>
  <summary>Details</summary>
Motivation: 现有DNN模型未考虑计算内存（CIM）芯片的架构限制，导致在边缘设备上的实时性和能效表现不佳。

Method: 提出CIM-NET架构和CIM-CONV算子，结合大感受野操作和滑动处理，优化CIM芯片上的矩阵-向量乘法加速。

Result: CIM-NET将矩阵-向量乘法操作减少至1/77，同时保持接近FastDVDnet的去噪性能（PSNR 35.11 dB vs. 35.56 dB）。

Conclusion: 该框架为边缘设备上的高效视频去噪提供了可行解决方案，平衡了性能和计算效率。
...
TLDR: 论文提出了一种仅使用非配对数据学习共享表示的方法，通过谱嵌入技术实现跨模态关系捕捉，并在多个任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖配对数据，但配对数据难以获取，因此探索如何利用非配对数据学习共享表示。

Method: 基于随机游走矩阵的谱嵌入技术，独立构建单模态表示，实现跨模态共享表示。

Result: 在计算机视觉和自然语言处理任务中表现优异，支持检索、生成、算术、零样本和跨域分类。

Conclusion: 首次证明非配对数据可学习通用跨模态嵌入，为跨模态表示学习提供了新思路。
...
TLDR: UniDB++是一种无需训练的采样算法，通过精确求解反向时间SDE，显著提升UniDB的计算效率，减少误差累积，并在图像恢复任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: UniDB框架虽然实现了高保真图像生成，但其依赖迭代Euler采样方法导致推理速度慢且计算成本高，现有加速技术无法解决其独特挑战。

Method: UniDB++通过推导反向时间SDE的闭式解，减少Euler近似的误差，并采用数据预测模型和SDE-Corrector机制提升稳定性。

Result: UniDB++在图像恢复任务中表现优异，推理速度提升20倍，且能兼容现有扩散桥加速方法。

Conclusion: UniDB++填补了SOC驱动扩散桥模型的理论通用性与实际效率之间的差距，代码已开源。
...
TLDR: LLMs在3D虚拟角色控制中表现良好，能够生成高层次运动计划，但在精确身体部位定位和多步骤运动方面存在困难。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs在人类运动知识中的应用，通过3D虚拟角色控制验证其能力。

Method: 将运动指令分解为高层次计划（步骤）和低层次计划（身体部位位置），并通过插值生成动画。

Result: LLMs擅长解释高层次运动，但难以精确定位身体部位，且在多步骤运动中表现不佳。

Conclusion: LLMs在创意运动和文化特定运动模式方面有潜力，但需改进精确空间定位能力。
...
TLDR: 提出了一种物理引导的混合专家（MoE）框架，用于解决荧光LiDAR在散射介质中的计算挑战，显著提升了深度和荧光寿命的估计精度。


<details>
  <summary>Details</summary>
Motivation: 荧光LiDAR在散射介质中面临信号解析困难，传统方法难以有效分离光子飞行时间和荧光寿命，限制了其应用。

Method: 采用物理引导的MoE框架，结合基于证据的Dirichlet批评器（EDCs）和决策网络，动态融合专家预测。

Result: 在模拟的荧光LiDAR数据中，深度估计的归一化均方根误差（NRMSE）为0.030，荧光寿命的NRMSE为0.074。
...
TLDR: 提出了一种新的无监督视觉特征学习方法SOP，通过多个语义相似的表示（SEs）替代单一原型，提升训练性能。


<details>
  <summary>Details</summary>
Motivation: 现有自监督学习方法依赖单一原型编码隐藏簇的特征，限制了性能。

Method: 采用SOP策略，每个原型由多个互补的SEs表示，并引入非参数化损失函数和SOP-MIM任务。

Result: 在多个基准测试中表现优异，检索任务达到SOTA，复杂编码器性能提升显著。

Conclusion: SOP策略有效提升了无监督视觉特征学习的性能，尤其在复杂任务中表现突出。
...
TLDR: FAR框架通过用LSTM等序列模块替换预训练Transformer中的注意力机制，提升推理效率，同时保持模型性能。


<details>
  <summary>Details</summary>
Motivation: Transformer的注意力机制在推理时效率低，尤其是在资源有限的设备上。研究发现推理时的序列映射较简单，可用更廉价函数替代。

Method: 提出FAR框架，用多头LSTM替换注意力模块，通过块级蒸馏和全局结构剪枝优化。
...
TLDR: 论文分析了视觉语言模型（VLMs）在认知能力上的局限性，提出通过认知科学方法评估其表现，并发现改进方向。
...
Method: 采用认知科学方法，评估VLMs在感知、注意力和记忆等核心认知轴上的表现，并通过视觉-文本解耦分析改进模型。

Result: 发现VLMs在空间理解等任务上表现不足，但通过生成文本标题推理或针对性微调可显著提升性能。

Conclusion: 研究揭示了VLMs的认知局限，提出了改进方向，并验证了简单有效的解决方案。
...
TLDR: 提出了一种基于流匹配模型的等变求解器（Eda），用于点云组装任务，能够高效学习等变分布并处理非重叠输入。
...
Method: 通过理论分析，提出学习相关向量场是实现等变分布的关键，并构建了等变路径以保证训练效率。

Result: Eda在实际数据集中表现优异，能够处理非重叠输入。
...
TLDR: 论文提出了一种新任务：Alpha合成图像的逐层分解，并提出了DiffDecompose框架和AlphaBlend数据集来解决透明/半透明层分解的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有图像分解方法在透明/半透明层分解任务中表现不佳，主要由于掩码先验依赖、静态对象假设和数据缺乏。

Method: 提出了DiffDecompose框架，基于扩散Transformer，通过上下文分解和层位置编码克隆技术实现层分解。
...
TLDR: 论文提出了一种结合对象检测、大语言模型（LLM）和检索增强生成（RAG）的混合方法，用于精准农业中的作物病害检测与诊断。


<details>
  <summary>Details</summary>
Motivation: 传统农业资源利用效率低且对环境造成挑战，精准农业通过先进技术优化农业流程。

Method: 结合视觉和语言模型，利用YOLOv8进行病害检测，RAG提供上下文感知诊断，减少LLM的幻觉问题。

Result: 系统能实时检测咖啡叶病害并提供治疗方案，减少农药使用，支持环保农业。
...
TLDR: CAT-LVDM是一种针对LVDMs的鲁棒性训练框架，通过结构化噪声注入提升视频生成质量，显著减少语义漂移和时间不一致性。


<details>
  <summary>Details</summary>
Motivation: LVDMs在噪声较多的视频-文本数据集上容易产生语义漂移和时间不一致性，需要一种鲁棒性训练方法。

Method: 提出BCNI和SACN两种噪声注入方法，分别针对语义一致性和低频平滑性进行优化。

Result: BCNI在WebVid-2M等数据集上平均减少FVD 31.9%，SACN在UCF-101上提升12.3%。

Conclusion: CAT-LVDM为多模态噪声下的视频扩散模型提供了可扩展且理论支持的训练方法。
...
TLDR: 论文研究了大型视觉语言模型（LVLMs）中幻觉非存在对象的问题，提出了一种基于图像标记共现图和GNN的缓解方法。


<details>
  <summary>Details</summary>
Motivation: 发现LVLMs会幻觉非存在对象，假设这是由于训练中视觉先验导致的标记共现关联。

Method: 构建图像标记共现图，使用GNN和对比学习聚类标记，提出抑制视觉缺失标记影响的生成方法。

Result: 幻觉主要与输入中主导的标记簇相关，提出的方法有效减少了幻觉并保持表达能力。

Conclusion: 通过修改潜在图像嵌入抑制视觉缺失标记，可有效缓解幻觉问题。
...
TLDR: DCLIP是一种基于CLIP模型的改进版本，通过元教师-学生蒸馏框架提升多模态图像-文本检索能力，同时保留零样本分类性能。
...
Method: 采用跨模态Transformer教师模型和轻量级学生模型，通过双向跨注意力机制和混合损失函数进行训练。

Result: DCLIP显著提升了检索指标（Recall@K, MAP），同时保留了94%的零样本分类性能。
...
TLDR: 论文提出了一种新的Benign-to-Toxic（B2T）越狱范式，通过优化对抗图像从良性条件诱导毒性输出，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有Toxic-Continuation范式在缺乏显性毒性信号时效果不佳，需探索更有效的越狱方法。

Method: 优化对抗图像，使其在良性条件下诱导模型生成毒性输出，无需依赖已有毒性输入。

Result: B2T方法优于现有方法，适用于黑盒场景，并能与文本越狱互补。

Conclusion: 揭示了多模态对齐中未充分探索的漏洞，为越狱方法提供了新方向。
...
TLDR: 提出一种无需标准训练即可分析计算CNN权重和阈值的算法，仅需10张MNIST图像即可确定参数，实验表明其能快速识别手写数字。


<details>
  <summary>Details</summary>
Motivation: 探索无需传统训练的CNN构建方法，减少计算资源需求。

Method: 基于10张MNIST图像分析计算CNN权重、阈值及通道数，并用C++实现。

Result: 未经训练的CNN能快速识别半数以上的测试图像。

Conclusion: CNN可通过纯分析计算直接用于分类任务，无需训练。
...
TLDR: 提出了一种基于CNN的新框架，用于高效分类十种常见芸苔属种子，解决了种子图像纹理相似性的挑战，准确率达93%。
...
Method: 采用自定义CNN架构，针对种子图像纹理相似性问题优化，并与多种预训练架构对比。

Result: 实验结果表明，所提模型在芸苔属种子数据集上达到93%的高准确率。

Conclusion: 该CNN框架为种子分类提供了高效解决方案，有助于提升种子质量管理和产量预估。
...
TLDR: 提出了一种新颖的深度学习框架，用于自动评估蝶枕软骨联合（SOS）融合，通过双模型架构和知识蒸馏提升诊断准确性。


<details>
  <summary>Details</summary>
Motivation: 蝶枕软骨联合融合是正畸学和法医人类学中的重要诊断标志，传统方法依赖人工裁剪和预处理，效率低且不一致。

Method: 采用双模型架构（教师模型和学生模型），教师模型通过手动裁剪图像训练，学生模型通过知识蒸馏和新的损失函数学习空间特征。

Result: 框架实现了稳健的诊断准确性，无需额外预处理工具，提升了骨骼成熟评估的效率和一致性。

Conclusion: 该框架为临床提供了一种高效、一致的端到端解决方案，适用于多样化临床环境。
...
TLDR: 本文提出了一种基于自监督学习的预训练模型，用于解决脑血肿CT图像中多实例学习因实例数量增加而难以训练的问题，显著提升了分类性能。


<details>
  <summary>Details</summary>
Motivation: 在脑血肿CT图像的多实例学习中，当每个包（bag）中的实例数量增加到256时，传统深度学习方法难以有效学习。本文旨在解决这一问题。

Method: 采用自监督学习的预训练模型作为多实例学习器的下游任务，以克服原始任务中的虚假相关性。

Result: 在脑血肿CT的低密度标记分类任务中，准确率提升了5%至13%，F1分数提升了40%至55%。

Conclusion: 自监督预训练模型在多实例学习中表现优异，显著提升了分类性能，解决了实例数量增加带来的学习困难。
...
TLDR: 论文提出了一种基于扩散模型的AI动作捕捉技术，用于生成连续动作序列，解决了传统方法中动作预定义的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前AI动作捕捉技术依赖预定义视频序列，无法处理未观察到的动作。本文旨在通过虚拟人应用实现更灵活的动作捕捉。

Method: 提出扩散模型的动作补全技术，结合门模块和位置-时间嵌入模块，生成平滑连续的动作序列。

Result: 在Human3.6M数据集上，MDC-Net在ADE、FDE和MMADE上优于现有方法，模型更小（16.84M），生成动作更自然。

Conclusion: MDC-Net在动作捕捉中表现出色，同时提出从动作序列提取传感器数据的方法。
...
TLDR: 提出了一种名为EaqVLA的优化框架，通过编码对齐量化解决VLA模型的量化问题，实现了更低的量化损失和计算加速。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型的计算和存储成本高昂，量化是优化方法，但现有量化方法因token对齐问题难以应用。

Method: 提出编码对齐量化框架EaqVLA，包括多粒度对齐分析和混合精度量化方法。

Result: EaqVLA在端到端动作控制中量化损失最小，计算加速显著。

Conclusion: EaqVLA有效解决了VLA模型的量化问题，优于现有方法。
...
TLDR: 提出了一种厚度感知的3D网格神经网络（T-EMNN），解决了现有方法忽略物体厚度的问题，同时保持了计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有3D静态分析方法主要关注表面拓扑和几何，忽略了物体厚度及其对表面行为的影响。

Method: 提出T-EMNN框架，整合物体厚度并保持计算效率；引入数据驱动坐标以编码空间信息并保持E(3)-等变性或不变性。

Result: 在工业数据集上验证了T-EMNN在预测节点级3D变形方面的优越性能，有效捕捉厚度效应。

Conclusion: T-EMNN是一种高效且准确的厚度感知3D分析方法。
...
TLDR: 通过仅增强训练早期未学习的数据部分，提升图像分类器的泛化能力，优于全数据集增强方法。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型增强方法在确保生成多样性和扩展数据量（10-30倍）以提升性能方面存在不足。

Method: 分析双层CNN，证明仅增强30%-40%未学习数据可促进特征学习速度的均匀性，避免噪声放大。

Result: 在CIFAR-10、CIFAR-100和TinyImageNet上，性能提升高达2.8%，且SGD表现优于SOTA优化器SAM。

Conclusion: 该方法可与现有强弱增强策略结合，进一步提升性能。
...
TLDR: 论文介绍了一个新的光学幻觉数据集，用于研究机器学习和人类感知的局限性，并探讨了视觉概念对模型准确性的影响。


<details>
  <summary>Details</summary>
Motivation: 在安全关键领域（如自动驾驶和医疗诊断）中，机器学习算法的可靠性至关重要。光学幻觉为研究人类和机器感知的局限性提供了独特视角，但目前相关数据集稀缺。

Method: 作者创建了一个包含交织动物对的光学幻觉数据集，重点研究了凝视方向和眼睛线索等视觉概念对模型的影响。

Result: 研究发现，视觉概念（如凝视方向）对模型准确性有显著影响，为研究人类与机器视觉的偏差和对齐提供了基础。

Conclusion: 该数据集为研究视觉学习和偏差提供了新工具，代码和数据集已公开，便于进一步研究。
...
TLDR: 提出了一种一步式视频散景框架，通过多平面图像表示和预训练模型的3D先验，实现高质量、可控的视频散景效果。
...
Method: 利用多平面图像（MPI）表示和逐步扩展的深度采样函数，结合单步视频扩散模型和预训练模型的3D先验，实现深度感知的散景合成。

Result: 实验表明，该方法能生成高质量、可控的散景效果，并在多个评估基准上达到最先进性能。

Conclusion: 该方法解决了视频散景的时间一致性和深度感知问题，为可控光学效果提供了新思路。
...
TLDR: 论文提出了一种基于生物启发的无监督学习框架，利用运动边界信号从视频中学习物体中心视觉表征，无需标签或相机标定。


<details>
  <summary>Details</summary>
Motivation: 受婴儿通过观察运动获取物体理解的启发，研究旨在开发一种无监督学习方法，利用运动边界作为物体分组的信号。

Method: 通过现成的光流和聚类算法生成基于运动的实例掩码，并利用对比学习训练视觉编码器。

Result: 在多个下游任务（如深度估计和3D物体检测）中表现优于现有监督和自监督基线，并展示了对新场景的强泛化能力。
...
TLDR: BaryIR提出了一种多源表示学习框架，通过分解多源退化图像的潜在空间为连续重心空间和源特定子空间，提升全合一图像恢复的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有全合一图像恢复方法对分布外退化和图像表现脆弱，限制了其实际应用。

Method: 引入多源潜在最优传输重心问题，学习连续重心映射，将潜在表示传输到重心空间，同时设计传输成本以保持子空间表示对比和正交性。

Result: BaryIR在实验中表现优异，尤其在真实数据和未见退化上展现更强的泛化能力。

Conclusion: BaryIR通过学习统一退化无关信息和特定退化语义，提升了全合一图像恢复的泛化性能。
...
TLDR: 本文提出了一种基于几何特征的提示生成方法（GeomPrompt），用于优化SAM模型在植物根系图像分割中的表现，显著提高了自动分割的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 植物根系图像（如rhizotron图像）的手动标注耗时且主观，传统自动化方法效果不佳。通过改进SAM模型的提示生成方式，可以提升分割的敏感性和特异性。

Method: 提出GeomPrompt方法，利用几何特征生成提示点，直接与SAM模型集成，实现自动化的高精度分割。

Result: GeomPrompt结合SAM显著提升了植物根系图像的分割效果，减少了所需提示点的数量。

Conclusion: GeomPrompt为科学图像分析提供了一种高效、自动化的解决方案，开源工具geomprompt进一步促进了其应用。
...
TLDR: 本文提出了一种通过学习查询特定的特征空间变换来改进视觉语言模型在实例检索任务中的性能的方法。


<details>
  <summary>Details</summary>
Motivation: 大规模预训练的视觉语言模型在图像检索任务中表现不佳，尤其是在大规模图像集合中的实例检索任务。

Method: 通过学习查询特定的线性特征空间变换，并将其应用于图像嵌入，以提升检索性能。

Result: 该方法在性能上优于现有最先进方法，且计算成本更低。

Conclusion: 查询特定的特征空间变换是一种高效且有效的大规模图像检索方法。
...
TLDR: DORI是一个专注于评估视觉语言模型在物体方向理解能力的新基准，揭示了现有模型在方向感知上的显著局限性。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言基准未能独立评估物体方向理解能力，常将其与位置关系和场景理解混淆，因此需要专门的评估工具。

Method: DORI通过11个数据集的67个物体类别，设计了四个维度的方向理解任务，评估15种先进模型。

Result: 最佳模型在粗粒度任务中准确率为54.2%，细粒度任务中降至33.0%，模型在角度估计和复合旋转任务中表现不佳。

Conclusion: DORI揭示了模型在3D空间表示上的不足，为改进机器人控制、3D场景重建和人类-AI交互提供了方向。
...
TLDR: DiffPhy是一个通过微调预训练视频扩散模型实现物理正确和逼真视频生成的通用框架，利用LLM和MLLM指导生成。


<details>
  <summary>Details</summary>
Motivation: 当前视频扩散模型在生成视觉效果上表现优异，但在物理效果合成上仍有挑战，需解决复杂物理运动、交互和动力学的学习问题。

Method: 提出DiffPhy框架，利用LLM从文本提示中推理物理上下文，并通过MLLM作为监督信号，引入新训练目标以确保物理正确性和语义一致性。

Result: 在公共基准测试中，DiffPhy在多样物理相关场景中实现了最先进的生成效果。
...
TLDR: 提出了一种利用低分辨率粗标签作为空间参考的新框架，通过回归有符号距离变换图实现边界感知分割，并引入可扩展的类条件分割策略，显著提升了高分辨率脑MRI分割的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 高分辨率脑MRI分割面临标注数据不足和计算需求高的挑战，需要一种无需额外标注成本且高效的方法。

Method: 利用低分辨率粗标签作为参考，回归有符号距离变换图实现边界感知分割，并采用类条件分割策略逐类处理。

Result: 在合成和真实数据集上验证了方法的优越性能和可扩展性。

Conclusion: 该方法在减少内存消耗的同时，能够泛化到未见过的解剖类别，优于传统分割方法。
...
TLDR: MedBridge是一个轻量级多模态适应框架，通过重新利用预训练的视觉语言模型（VLMs）来提高医学图像诊断的准确性，无需大量资源。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言基础模型在自然图像分类上表现优异，但在医学图像上因领域差异表现不佳，而训练医学基础模型又需要大量资源。

Method: MedBridge包含三个关键组件：1）Focal Sampling模块提取高分辨率局部区域；2）Query Encoder（QEncoder）注入可学习查询；3）混合专家机制利用多种VLMs的互补优势。

Result: 在五个医学影像基准测试中，MedBridge在跨领域和领域内适应任务中表现优异，尤其在多标签胸部疾病诊断中AUC提升6-15%。

Conclusion: MedBridge有效利用基础模型，实现了准确且数据高效的医学诊断。
...
TLDR: 论文提出OMCRG任务，旨在在线生成同步的言语和非言语听众反馈，并引入OmniResponse模型解决多模态同步问题。


<details>
  <summary>Details</summary>
Motivation: OMCRG任务模拟真实对话中的多模态反馈，解决音频与面部反应同步的挑战。

Method: 提出OmniResponse模型，结合Chrono-Text和TempoVoice组件，利用文本作为中间模态。

Result: OmniResponse在语义内容、视听同步和生成质量上显著优于基线模型。

Conclusion: OmniResponse为OMCRG任务提供了高效解决方案，并发布了ResponseNet数据集支持未来研究。
...
TLDR: 论文提出了一种称为“矩核”的简单卷积核形式，用于实现旋转和反射等对称性的等变性，并证明了所有等变核必须采用这种形式。该方法简化了传统依赖表示理论的复杂数学方法。


<details>
  <summary>Details</summary>
Motivation: 旋转和反射等对称性在生物医学图像分析中至关重要，但传统方法依赖复杂的数学理论（如表示理论），限制了其广泛应用。论文旨在简化这些对称性的实现方法。

Method: 提出“矩核”作为简单卷积核形式，由空间位置的径向对称函数乘以$x$的分量幂或单位矩阵构成。使用标准卷积模块实现等变神经网络。

Result: 成功实现了多种生物医学图像分析任务，包括分类（输出正交变换不变）、3D图像配准（输出向量变换）和细胞分割（椭圆定义的二次形式矩阵变换）。

Conclusion: 矩核提供了一种简单且通用的方法来实现对称性等变性，避免了传统方法的数学复杂性，为生物医学图像分析提供了高效工具。
...
TLDR: 论文探讨了扩散模型（DMs）中的对抗训练（AT）与分类器中的AT的根本区别，提出了一种无需假设噪声模型的方法，显著提升了模型的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 研究扩散模型中对抗训练与分类器中对抗训练的本质差异，以提升模型对噪声、异常值和数据损坏的鲁棒性。

Method: 通过添加随机噪声（类似随机平滑）或对抗噪声（类似对抗训练）来无缝集成到扩散训练中，无需假设噪声模型。

Result: 在低维和高维空间中验证了方法的有效性，并在CIFAR-10、CelebA和LSUN Bedroom等标准基准测试中表现出色。

Conclusion: 扩散模型中的对抗训练通过保持扩散过程与数据分布的对齐，显著提升了模型的鲁棒性和适应性。
...
TLDR: 论文提出了一种融合卫星和无人机（UAS）图像的超分辨率框架，用于精准农业中的覆盖作物生物量和氮含量估算。


<details>
  <summary>Details</summary>
Motivation: 卫星和无人机数据在精准农业中各具优势与局限，卫星覆盖广但分辨率低，无人机分辨率高但成本高。研究旨在结合两者优势。

Method: 通过超分辨率方法融合卫星和无人机数据，光谱扩展UAS RGB数据至植被红边和近红外区域，生成高分辨率Sentinel-2图像。

Result: 生物量和氮含量估算精度分别提高了18%和31%，且无人机数据仅需部分采集。

Conclusion: 该框架轻量且可扩展，适用于农场低成本使用，即使无卫星数据时仍有效。
...
TLDR: LoopGNN是一种图神经网络架构，通过利用视觉相似关键帧的团来提高闭环检测的精度和效率。
...
Method: 提出LoopGNN，利用图神经网络在视觉相似关键帧团中传播特征编码，实现高精度闭环检测。

Result: 在TartanDrive 2.0和NCLT数据集上表现优于传统方法，且计算效率更高。

Conclusion: LoopGNN在保持高召回率的同时提升精度，且对特征编码类型鲁棒。
...
TLDR: 提出了一个新的基准FRAMES-VQA，用于评估VQA任务中的鲁棒微调，涵盖多模态分布偏移。
...
Method: 利用十个现有VQA基准，分为ID、近OOD和远OOD数据集，计算马氏距离量化分布偏移，并分析模态交互与重要性。

Result: 提供了对单模态和多模态偏移的深入分析，为开发更鲁棒的微调方法提供指导。
...
TLDR: MMTBENCH是一个包含500个真实世界多模态表格的基准测试，用于评估视觉语言模型在复杂多模态表格推理任务中的性能，揭示了现有模型的不足。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在文本和图像理解方面表现优异，但在复杂多模态表格推理任务中的性能尚未被充分探索。

Method: 引入MMTBENCH基准测试，包含500个多模态表格和4021个问题对，覆盖多种问题类型、推理类型和表格类型。

Result: 评估显示现有模型在视觉推理和多步推理问题上存在显著性能差距。

Conclusion: MMTBENCH为未来多模态表格研究提供了高质量资源，并强调了改进视觉与语言处理架构的紧迫性。
...
TLDR: 该论文提出了一种通过逆向生成建模理解场景的方法，利用组合式生成模型从自然图像中推断场景结构和对象。
...
Result: 方法能够从训练数据之外的图像中推断场景结构和对象，并支持零样本多对象感知。

Conclusion: 组合式逆向生成建模为场景理解和多对象感知提供了鲁棒且通用的解决方案。
...
TLDR: SANSA利用SAM2的潜在语义结构，通过最小任务特定修改，将其重新用于少样本分割，实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 解决少样本分割任务中需要跨图像识别语义相关对象并生成准确分割掩码的问题，同时克服SAM2因任务特定线索而受限的问题。

Method: 提出SANSA框架，显式利用SAM2的潜在语义结构，通过最小任务特定修改重新调整其功能。

Result: 在少样本分割基准测试中表现最优，支持多种交互方式，且比现有方法更快、更紧凑。

Conclusion: SANSA成功将SAM2重新用于少样本分割，展示了其潜在语义结构的有效性。
...
TLDR: ALTER框架通过统一层剪枝、专家路由和模型微调，显著提升扩散模型的推理效率，同时保持生成质量。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在推理时计算开销大，现有加速方法未能充分利用时间动态性，且剪枝与微调策略存在不匹配问题。

Method: ALTER采用超网络动态生成层剪枝决策和时间步路由，统一优化剪枝、路由和微调过程。

Result: ALTER在20步推理中仅用25.9%的MACs，实现3.64倍加速，生成质量与原模型相当。

Conclusion: ALTER为资源受限环境下的扩散模型部署提供了高效解决方案。
...
TLDR: HDRSDR-VQA是一个大规模视频质量评估数据集，用于比较HDR和SDR内容在真实观看条件下的质量。


<details>
  <summary>Details</summary>
Motivation: 现有数据集通常只关注单一动态范围格式或使用有限评估协议，无法直接比较HDR和SDR版本。HDRSDR-VQA填补了这一空白。

Method: 数据集包含960个视频，来自54个源序列，涵盖9种失真级别。通过145名参与者的主观研究和22,000多次成对比较，生成JOD评分。
...
TLDR: UniMoGen是一种基于UNet的扩散模型，用于骨架无关的运动生成，支持多样角色（如人类和动物）的运动生成，具有高效性和可控性。
...
Result: 在100style数据集上表现优于现有方法，同时在100style和LAFAN1数据集上均实现高效性能。

Conclusion: UniMoGen为角色动画提供了灵活、高效且可控的运动生成解决方案。
...
TLDR: 提出了一种针对开放词汇语义分割（OVSS）的测试时适应（TTA）方法MLMP，通过多级多提示熵最小化优化视觉语言模型的分割性能，无需额外数据或标签。


<details>
  <summary>Details</summary>
Motivation: 现有TTA方法主要关注图像分类，而密集预测任务如OVSS被忽视，因此需要专门的分割TTA方法。

Method: MLMP方法整合中间视觉编码器层特征，并在全局CLS标记和局部像素级别使用多提示模板进行熵最小化。

Result: 实验表明，MLMP在82种测试场景中显著优于直接应用分类TTA基线。

Conclusion: MLMP为OVSS提供了一种高效、即插即用的TTA解决方案，并建立了标准化评测基准。
...
TLDR: 研究发现FFN层是ViT推理延迟的主要来源，提出了一种通道空闲机制，通过结构重参数化优化FFN层，显著降低延迟并保持或提升精度。
...
Result: RePaViT系列模型在延迟显著降低的同时，精度保持或提升，尤其在大型模型中表现更优。
...
TLDR: 论文提出了一种细粒度噪声注入技术（FPAN），通过概率性地向token嵌入添加更大噪声，显著减少扩散模型对训练数据的复制，同时保持图像质量。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在生成高质量图像方面表现出色，但其容易复制训练数据的特性引发了隐私问题，尤其是涉及敏感或私人信息时。现有方法效果有限，需进一步改进。

Method: 提出FPAN方法，通过分析不同噪声量的影响，概率性地向token嵌入添加更大噪声，以减少数据复制。

Result: FPAN平均减少28.78%的数据复制，优于基线模型和现有噪声添加方法（提升26.51%），与其他方法结合可进一步减少16.82%的复制。

Conclusion: FPAN是一种有效的隐私保护技术，能显著减少扩散模型的数据复制问题，同时保持图像质量，且与其他方法兼容。
...
TLDR: 论文提出了MultiStAR基准和MSEval指标，用于评估多模态大语言模型在抽象视觉推理中的多阶段表现，发现现有模型在复杂规则检测阶段仍存在挑战。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在抽象视觉推理中表现不足，且现有基准和指标未能充分评估推理过程的多阶段性和中间步骤的正确性。

Method: 引入基于RAVEN的MultiStAR基准和MSEval指标，对17种代表性模型进行实验。

Result: 现有模型在基础感知任务中表现良好，但在复杂规则检测阶段表现不佳。

Conclusion: MultiStAR和MSEval填补了评估空白，揭示了模型在复杂推理中的局限性。
...
TLDR: 论文提出两种新策略（WAAttack和SubAttack）改进基于梯度的对抗攻击，通过加权梯度和自适应步长策略以及子集分解，提高攻击效果和隐蔽性。
...
Method: 1. WAAttack：加权梯度和自适应步长策略；2. SubAttack：分解点云并聚焦关键区域。
...
TLDR: HLIP是一种用于3D医学影像的可扩展预训练框架，通过分层注意力机制解决了计算效率问题，并在多个基准测试中取得了显著性能提升。
...
Method: HLIP采用轻量级分层注意力机制（切片、扫描和研究层次），并直接在大规模未筛选临床数据集上进行训练。

Result: 在多个基准测试中表现优异，如Rad-ChestCT（+4.3% AUC）、Pub-Brain-5（+32.4% ACC）等。

Conclusion: HLIP证明直接在未筛选临床数据集上进行预训练是3D医学影像语言-图像预训练的有效方向。
...
TLDR: GETReason框架通过提取地理空间、时间和事件信息，提升图像上下文理解的深度，并引入GREAT评估指标。
...
Method: 提出GETReason框架，结合地理空间、时间和事件信息，并引入GREAT评估指标。

Result: 多智能体方法通过推理加权指标，成功将图像与事件背景关联。

Conclusion: GETReason框架有效提升了图像上下文理解的深度和准确性。
...
TLDR: 论文提出Cross-DINO方法，通过结合深度MLP网络和Cross Coding Twice Module（CCTM）提升小目标检测性能，并引入Category-Size（CS）软标签和Boost Loss损失函数。
...
Method: 结合深度MLP网络聚合短长程信息，使用CCTM增强小目标细节，引入CS软标签和Boost Loss提升预测分数。

Result: 在多个数据集上验证，Cross-DINO在COCO上达到36.4% APs，优于DINO（+4.4%），且参数和计算量更少。

Conclusion: Cross-DINO有效提升了DETR类模型在小目标检测中的性能，具有高效性和优越性。
...
TLDR: EPiC是一种高效精确的相机控制学习框架，通过自动构建高质量锚视频，无需昂贵的相机轨迹标注，显著减少训练参数和步骤。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖点云估计和相机轨迹标注，导致锚视频不准确且资源消耗大。EPiC旨在解决这些问题。

Method: 通过基于首帧可见性的视频掩码生成高质量锚视频，并引入轻量级Anchor-ControlNet模块集成到预训练VDMs中。

Result: EPiC在RealEstate10K和MiraData上实现SOTA性能，支持精确3D相机控制，并具备零样本泛化能力。

Conclusion: EPiC提供了一种高效、精确且泛化能力强的相机控制解决方案，显著优于现有方法。
...
TLDR: 本文提出了一种名为HS-GS的新方法，结合3D高斯泼溅和扩散模型，用于高光谱场景的3D显式重建和新视角合成，解决了NeRF在训练和渲染速度上的限制。


<details>
  <summary>Details</summary>
Motivation: 高光谱成像（HSI）在农业中有广泛应用，但现有的NeRF方法在训练和渲染速度上存在不足。

Method: 结合3D高斯泼溅和扩散模型，引入波长编码器和KL散度损失函数，优化高光谱场景的重建和渲染。

Result: 在Hyper-NeRF数据集上的实验表明，HS-GS方法在性能上优于现有方法。

Conclusion: HS-GS为高光谱场景的3D重建和新视角合成提供了高效且高质量的解决方案。
...
TLDR: 论文提出了一种改进的少样本医学图像分割方法，通过关注弱特征和边界优化，显著提升了分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有原型方法因随机采样或局部平均导致边界模糊，需关注弱特征以改善分割边界。

Method: 设计了支持自预测模块（SSP）识别弱特征，硬原型生成模块（HPG）生成硬原型，多相似性图融合模块（MSMF）优化分割，并引入边界损失。

Result: 在三个公开医学图像数据集上实现了最先进的性能。

Conclusion: 该方法通过关注弱特征和边界优化，显著提升了少样本医学图像分割的准确性。
...
TLDR: CAST是一种半监督知识蒸馏框架，通过压缩预训练的视觉基础模型，利用有限标注和大量未标注数据训练紧凑模型。


<details>
  <summary>Details</summary>
Motivation: 实例分割需要昂贵的像素级标注和大模型，CAST旨在通过半监督学习降低成本并提升性能。

Method: CAST分为三个阶段：域适应、蒸馏和微调，核心是实例感知像素级对比损失。

Result: 在Cityscapes和ADE20K上，CAST的学生模型比教师模型性能提升3.4 AP和1.5 AP，优于现有半监督方法。

Conclusion: CAST通过对比损失和多阶段训练，显著提升了半监督实例分割的性能和效率。
...
TLDR: 本文提出了一种新方法，通过最大化参考面部的效用，改进了基于扩散模型的图像修复中的身份保留问题。
...
Method: 1) 提出Composite Context，融合参考面部的高、低层次信息；2) 设计Hard Example Identity Loss，改进身份学习效率；3) 提出无需训练的多参考输入适应方法。

Result: 在FFHQ-Ref和CelebA-Ref-Test等基准测试中，方法显著提升了修复质量，身份保留效果达到最优。

Conclusion: 该方法通过充分利用参考面部信息，显著提升了面部修复和身份保留的性能。
...
TLDR: AlignGen提出了一种跨模态先验对齐机制，通过可学习令牌、鲁棒训练策略和选择性跨模态注意力掩码，解决了文本与参考图像不对齐时生成结果偏向文本先验的问题。
...
TLDR: 论文介绍了LiDARDustX数据集，专为高粉尘环境下的感知任务设计，填补了现有数据集的空白。


<details>
  <summary>Details</summary>
Motivation: 现有数据集主要关注结构化城市环境，缺乏对高粉尘等特殊场景的覆盖，限制了算法在这些场景中的验证。

Method: LiDARDustX数据集包含30,000帧LiDAR数据，由六种不同LiDAR传感器采集，附带3D边界框标注和点云语义分割。80%以上为粉尘场景。

Result: 建立了评估3D检测和分割算法的基准，分析了粉尘对感知精度的影响及其原因。
...
TLDR: BD Open LULC Map (BOLM) 提供高分辨率卫星图像的像素级 LULC 标注，填补南亚/东亚地区数据空白，支持深度学习模型和领域适应任务。


<details>
  <summary>Details</summary>
Motivation: 解决南亚/东亚发展中国家因资金有限、基础设施多样和人口密集导致的标注卫星数据稀缺问题。

Method: 使用高分辨率 Bing 卫星图像（2.22 米/像素）标注 Dhaka 大都市区及周边 4,392 平方公里的 LULC 数据，并通过 GIS 专家验证。

Result: BOLM 包含 11 个 LULC 类别，覆盖 8.91 亿像素，并利用 DeepLab V3+ 在五大类上进行了性能基准测试。

Conclusion: BOLM 为南亚/东亚地区提供了可靠的 LULC 数据集，支持深度学习模型的训练和领域适应任务。
...
TLDR: InfoSAM提出了一种基于信息论的参数高效微调方法，通过保留预训练模型中的领域不变关系，提升SAM在专业领域的性能。
...
Result: 实验证明InfoSAM显著提升了SAM在真实任务中的性能，尤其在专业领域表现优越。

Conclusion: InfoSAM为SAM的参数高效微调提供了鲁棒的蒸馏框架，适用于专业场景。
...
TLDR: 论文提出了一种基于伪标签的半监督计数框架，通过点区域（P2R）方案替代点对点（P2P）方案，解决了伪标签置信度传播问题。


<details>
  <summary>Details</summary>
Motivation: 点检测方法在密集人群定位和计数中表现优异，但标注成本高。本文旨在通过半监督框架减少标注需求。

Method: 提出点特定激活图（PSAM）分析训练问题，并设计P2R方案，通过分割局部区域共享伪点置信度。

Result: 实验表明，P2R在半监督计数和无监督域适应中有效解决了PSAM发现的问题。

Conclusion: P2R方案显著提升了伪标签的可靠性，为半监督计数提供了高效解决方案。
...
TLDR: UniTalk是一个专为主动说话人检测任务设计的新数据集，强调挑战性场景以提升模型泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准（如AVA）主要基于老电影，存在显著领域差距，而UniTalk专注于多样且困难的真实场景。

Method: UniTalk包含44.5小时视频，涵盖48,693个说话人身份，标注帧级主动说话人，覆盖多语言、嘈杂背景和拥挤场景。

Result: 在UniTalk上，现有模型表现远未饱和，但在真实数据集（如Talkies和ASW）上泛化能力更强。

Conclusion: UniTalk为主动说话人检测提供了新基准，助力开发更具适应性和鲁棒性的模型。
...
TLDR: 论文提出了一种结合第一人称和第三人称视图的框架E3VQA和M3CoT，以提升大型视觉语言模型在多视角问答中的表现。


<details>
  <summary>Details</summary>
Motivation: 第一人称视图虽能捕捉用户注意力和手-物交互细节，但视野狭窄且缺乏全局上下文，导致在空间或上下文要求高的查询中失败。

Method: 引入E3VQA基准和多视角训练技术M3CoT，通过结合第一人称和第三人称视图提供互补信息。

Result: M3CoT显著提升了模型性能（GPT-4o提升4.84%，Gemini 2.0 Flash提升5.94%）。

Conclusion: 多视角输入能有效提升模型推理能力，但仍有局限性。
...
TLDR: 论文提出了一种名为Cross-modal RAG的新框架，通过分解查询和图像为子维度组件，实现子查询感知的检索和生成，显著提升了检索和生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有检索增强生成方法在复杂查询下表现不佳，因为单一图像无法包含所有所需元素。

Method: 提出混合检索策略（稀疏和稠密检索器结合），并利用多模态大语言模型选择性生成图像。

Result: 在多个数据集上显著优于现有基线，同时保持高效。

Conclusion: Cross-modal RAG为解决复杂查询下的图像生成问题提供了有效方案。
...
TLDR: TiUE是一种新型的T2I扩散模型蒸馏方法，通过共享编码器特征和并行采样显著提升推理效率，同时保持图像质量和多样性。


<details>
  <summary>Details</summary>
Motivation: 解决T2I扩散模型在推理速度与图像质量之间的权衡问题，尤其是单步模型的多样性和质量不足。

Method: 提出时间无关的统一编码器TiUE，共享编码器特征并引入KL散度正则化噪声预测。

Result: TiUE在推理效率和图像质量上优于LCM、SD-Turbo和SwiftBrushv2等现有方法。

Conclusion: TiUE为T2I扩散模型提供了一种高效且高质量的蒸馏方案。
...
TLDR: 论文提出A2Seek数据集和A2Seek-R1框架，用于无人机视角下的异常检测，通过推理机制和优化策略显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 无人机视角下的异常检测面临动态视角、尺度变化和复杂场景的挑战，现有方法难以适应。

Method: 提出A2Seek数据集和A2Seek-R1框架，包括GoT引导的微调、A-GRPO优化策略和模拟无人机行为的注意力机制。

Result: A2Seek-R1在预测准确率和异常定位上分别提升22.04%和13.9%，泛化能力强。

Conclusion: A2Seek数据集和框架有效解决了无人机视角异常检测的挑战，性能显著提升。
...
TLDR: 本文提出了一种基于扩散模型的文档去扭曲方法DvD，通过坐标级去噪和时间变体条件细化机制，显著提升了文档结构的保留能力，并在多个基准测试中取得了最优性能。


<details>
  <summary>Details</summary>
Motivation: 文档去扭曲技术虽已取得进展，但在保留文档结构方面仍具挑战性。扩散模型的最新进展激发了其在文档去扭曲中的应用潜力。
...
TLDR: 论文提出了一种视频检索增强生成（VRAG）方法，通过显式全局状态条件化，显著减少了长期复合错误并提高了世界模型的时空一致性。


<details>
  <summary>Details</summary>
Motivation: 现有长视频生成模型由于复合错误和内存机制不足，缺乏有效的世界建模能力，影响了未来规划和动作选择的准确性。

Method: 通过动作条件和自回归框架增强图像到视频模型，并提出VRAG方法，结合显式全局状态条件化。

Result: VRAG显著减少了长期复合错误，提高了时空一致性，而传统自回归生成和检索增强生成效果较差。
...
TLDR: 本文提出D-Fusion方法，通过视觉一致性样本优化扩散模型的提示-图像对齐问题。


<details>
  <summary>Details</summary>
Motivation: 扩散模型生成的图像与文本提示之间存在不对齐问题，且现有直接偏好优化（DPO）方法因视觉不一致性受限。

Method: D-Fusion通过掩码引导的自注意力融合生成视觉一致性样本，并保留去噪轨迹以支持DPO训练。

Result: 实验表明D-Fusion能有效提升不同强化学习算法中的提示-图像对齐效果。

Conclusion: D-Fusion解决了视觉不一致性问题，显著改善了扩散模型的提示-图像对齐能力。
...
TLDR: 提出了一种基于事件相机的前向视角人体姿态估计方法D-EventEgo，通过头部姿态估计和动态对象分割提升性能。


<details>
  <summary>Details</summary>
Motivation: 解决传统RGB相机在低光或运动模糊环境下人体姿态估计的不足，利用事件相机的优势。

Method: 先估计头部姿态，再生成身体姿态；引入动态分割模块去除干扰。

Result: 在合成数据集上，动态环境中五项指标中有四项优于基线。

Conclusion: D-EventEgo在事件相机人体姿态估计任务中表现出色。
...
TLDR: 论文提出PeO-HOI方法，通过原型嵌入优化解决直播中人类-物体交互检测的对象偏差问题，显著提升了检测性能。


<details>
  <summary>Details</summary>
Motivation: 直播中人类与物体的交互行为检测存在对象偏差问题，现有方法过于关注物体而忽略交互。

Method: 结合物体检测与跟踪技术提取特征，采用原型嵌入优化减少对象偏差，建模时空上下文后通过预测头获取结果。

Result: 在VidHOI和BJUT-HOI数据集上，PeO-HOI的检测准确率显著提升（如VidHOI上37.19%@full）。

Conclusion: PeO-HOI有效解决了直播中HOI检测的对象偏差问题，提升了性能。
...
TLDR: PanoWan利用预训练的文本到视频模型，通过纬度感知采样和旋转语义去噪等技术，实现了高质量全景视频生成，并贡献了一个高质量全景视频数据集PanoVid。


<details>
  <summary>Details</summary>
Motivation: 现有全景视频生成模型难以利用预训练生成先验，导致生成质量受限。

Method: PanoWan采用纬度感知采样、旋转语义去噪和填充像素解码技术。

Result: PanoWan在全景视频生成中达到最先进性能，并在零样本下游任务中表现稳健。

Conclusion: PanoWan通过高效利用预训练模型和新技术，显著提升了全景视频生成的质量和多样性。
...
TLDR: GL-PGENet是一种用于多退化彩色文档图像增强的新架构，通过全局与局部参数化生成机制和分层增强框架，实现了高效且鲁棒的增强效果。
...
Method: 采用分层增强框架、双分支局部细化网络和修改的NestUNet架构，结合两阶段训练策略（大规模预训练和任务特定微调）。

Result: 在DocUNet和RealDAE数据集上分别达到0.7721和0.9480的SSIM分数，具有卓越的跨域适应性和计算效率。
...
TLDR: 提出了一种新型的BE-ToF成像范式，通过突发模式发射光脉冲并联合优化编码函数与深度重建网络，解决了传统iToF系统的相位缠绕和低信噪比问题。
...
Method: 采用突发模式发射光脉冲，估计整个突发周期的相位延迟，并提出端到端可学习框架，联合优化编码函数与深度重建网络。

Result: 通过仿真和原型实验验证了BE-ToF的有效性和实用性。

Conclusion: BE-ToF为长距离高保真深度成像提供了一种可行的解决方案。
...
TLDR: 论文介绍了一个名为“Guess the Age of Photos”的网页平台，通过两种游戏化模式让用户估计历史照片的年代，评估了用户表现和平台满意度。
...
Method: 平台基于Python、Flask、Bootstrap和PostgreSQL构建，使用10,150张1930-1999年的照片数据集，设计了两种游戏模式并评估用户表现。

Result: 用户对平台的满意度为4.25/5分，相对比较（65.9%准确率）优于绝对年份猜测（25.6%准确率），且较早年代的照片更容易识别。

Conclusion: 该平台不仅作为教育工具有效，还为研究人类对图像时间线索的感知提供了资源，并可用于生成计算机视觉模型的标注数据。
...
TLDR: 论文提出了一种名为平衡令牌修剪（BTP）的方法，用于减少大型视觉语言模型（LVLMs）中的图像令牌数量，以降低计算开销，同时保持模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有的令牌修剪方法通常忽略了修剪对当前层和后续层的联合影响，导致修剪决策不理想。

Method: BTP方法通过一个小型校准集将修剪过程分为多个阶段，早期阶段关注修剪对后续层的影响，后期阶段则注重保持局部输出的一致性。

Result: 实验表明，BTP方法在多个基准测试中表现优异，实现了78%的压缩率，同时平均保留了96.7%的原始模型性能。

Conclusion: BTP是一种有效的令牌修剪方法，能够在显著减少计算开销的同时保持模型性能。
...
TLDR: OmniAD是一个结合视觉和文本推理的多模态框架，用于细粒度异常检测与分析，性能优于现有模型。
...
Method: 结合Text-as-Mask Encoding进行视觉推理，并通过Visual Guided Textual Reasoning整合视觉感知；采用SFT与GRPO结合的集成训练策略。
...
TLDR: LatentMove是一个基于DiT的框架，专注于生成高度动态的人体动画，解决了复杂非重复运动中不自然变形的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理复杂、非重复的人体运动时容易产生不自然的变形，因此需要一种更鲁棒的解决方案。

Method: 提出了LatentMove框架，结合条件控制分支和可学习的脸/身体标记，并引入Complex-Human-Videos数据集和两个评估指标。

Result: 实验表明，LatentMove显著提升了人体动画质量，尤其在处理快速复杂运动时表现优异。

Conclusion: LatentMove推动了图像到视频生成的边界，代码、数据集和评估指标将开源。
...
TLDR: AquaMonitor是首个大型水生无脊椎动物计算机视觉数据集，用于环境监测，包含2.7M图像和多模态数据，定义了三个基准任务。


<details>
  <summary>Details</summary>
Motivation: 现有物种识别数据集缺乏标准化采集协议，且未聚焦于水生无脊椎动物，AquaMonitor填补了这一空白。

Method: 通过两年监测收集图像，结合DNA序列、干质量和尺寸测量，构建多模态数据集。

Result: 数据集包含2.7M图像、43,189标本，支持监测、分类和少样本学习任务。

Conclusion: AquaMonitor为水生生物多样性监测提供了实用基准，可直接应用于立法水质评估。
...
TLDR: SERA是一个基于LLM的框架，通过针对性场景推荐修复自动驾驶系统的失败案例，提升其鲁棒性和泛化能力。
...
Method: SERA分析性能日志，识别失败模式，动态检索语义对齐场景，并通过LLM机制优化推荐。

Result: 实验表明，SERA在安全关键条件下显著提升了自动驾驶基线的关键指标。

Conclusion: SERA通过少样本微调实现针对性适应，展示了其有效性和泛化能力。
...
TLDR: 提出一种结合临床增强动态软标签和医学图形对齐的新方法，改进医学视觉语言处理中的对比损失应用，并通过否定硬负样本提升模型对临床语言的理解。


<details>
  <summary>Details</summary>
Motivation: 通用领域架构（如CLIP）直接应用于医学数据时面临否定处理和数据不平衡的挑战，需改进以提升临床理解和适用性。

Method: 集成临床增强动态软标签和医学图形对齐，引入否定硬负样本，优化医学CLIP训练流程。

Result: 在零样本、微调分类和报告检索等任务中达到最优性能，并通过CXR-Align基准验证模型对临床语言的理解能力。

Conclusion: 该方法易于实现且能有效推广到对比学习框架，提升医学视觉语言处理能力，推动医学影像中临床语言理解的进步。
...
TLDR: 本文提出了一种新的AI任务，通过多模态（视觉、语音、音频）时间模式量化电影中的物化现象，并发布了MObyGaze数据集。
...
Method: 基于电影研究和心理学，定义了物化的结构化分类，构建了MObyGaze数据集，并提出了多种学习任务和模型基准测试。

Result: 展示了任务的可行性，并提供了数据集和代码。

Conclusion: 为多模态物化研究提供了新工具和基准。
...
TLDR: 提出了一种基于GPU数据调度算法的高效无人机图像特征匹配方法，通过矩阵带约简（MBR）和GPU加速级联哈希提升匹配效率。


<details>
  <summary>Details</summary>
Motivation: 特征匹配在运动恢复结构（SfM）中耗时严重，研究旨在通过GPU加速和优化数据调度提高无人机图像匹配效率。

Method: 1. 使用图像检索技术选择匹配对；2. 基于MBR生成紧凑图像块以减少冗余IO；3. 在GPU加速级联哈希框架下执行特征匹配，并结合几何约束和RANSAC优化。

Result: 在大规模无人机数据集上测试，速度比基于KD-Tree的方法快77.0至100.0倍，且精度与传统的BA方法相当。
...
TLDR: 本文提出了一个挑战性基准数据集UAVPairs和训练流程，用于大规模无人机图像的匹配对检索，通过几何相似性和多场景结构优化训练样本和损失函数，显著提升了检索精度和3D重建质量。
...
Method: 构建UAVPairs数据集，提出批量非平凡样本挖掘策略和排名列表损失函数，优化训练流程。

Result: 实验表明，使用UAVPairs和排名列表损失的模型在检索精度和3D重建质量上显著优于现有方法。
...
TLDR: 论文提出了一种新的无监督域自适应（UDA）框架RLGLC，通过结合域对齐和目标域可区分性增强约束，解决了传统对抗性方法忽视目标域特征可区分性的问题。
...
Method: 提出RLGLC框架，结合域对齐和可区分性增强约束，使用AR-WWD处理类别不平衡和语义维度加权，并通过局部一致性机制保留目标域细粒度信息。

Result: 在多个基准数据集上的实验表明，RLGLC优于现有方法，验证了同时保证可迁移性和可区分性的必要性。

Conclusion: 论文通过理论和实验证明了在对抗性UDA中同时优化可迁移性和可区分性的重要性，RLGLC框架为未来研究提供了新方向。
...
TLDR: 论文提出ELE-SAM模型，改进SAM以解决电力传输走廊危险分割任务，通过上下文感知提示适配器和高保真掩码解码器提升性能，并构建ELE-40K数据集验证其效果。


<details>
  <summary>Details</summary>
Motivation: 电力传输走廊危险分割（PTCHS）对保障电力传输安全至关重要，但现有SAM模型在复杂场景中表现不佳，尤其是对精细结构目标的处理。
...
Conclusion: ELE-SAM在PTCHS任务中表现优异，验证了方法的有效性，并提供了开源代码和数据集。
...
TLDR: 提出首个基于扩散模型的非自回归视频预测框架ARFree，解决自回归方法中的误差传播问题。


<details>
  <summary>Details</summary>
Motivation: 现有长期视频预测方法依赖自回归机制，导致远未来帧误差传播严重。

Method: ARFree直接预测未来帧组，包含运动预测模块和训练方法以提升运动连续性和上下文一致性。

Result: 在两个基准数据集上，ARFree优于多种最先进的视频预测方法。

Conclusion: ARFree为非自回归视频预测提供了有效解决方案，性能显著提升。
...
TLDR: 论文介绍了首个科学插图生成基准SridBench，评估了AI模型在生成科学插图时的表现，发现即使顶级模型如GPT-4o-image也落后于人类表现。


<details>
  <summary>Details</summary>
Motivation: 科学插图生成需要高水平的语义理解和结构准确性，但目前缺乏相关基准。自动化这一过程具有重要实用价值。

Method: 构建了包含1,120个实例的SridBench基准，涵盖13个学科，每个样本从六个维度评估。

Result: 实验显示，顶级模型在文本/视觉清晰度和科学正确性方面表现不佳，落后于人类。
...
TLDR: 本文提出了一种针对ISS上IMAGIN-e任务中地球观测图像机械散焦的盲去模糊方法，利用Sentinel-2数据在GAN框架下训练恢复模型，显著提升了图像质量。


<details>
  <summary>Details</summary>
Motivation: 解决IMAGIN-e任务中地球观测图像的机械散焦问题，适应空间边缘计算限制，提升图像质量。

Method: 利用Sentinel-2数据估计散焦核，并在GAN框架下训练恢复模型，无需参考图像。

Result: 在Sentinel-2图像上，SSIM提升72.47%，PSNR提升25.00%；在IMAGIN-e图像上，NIQE提升60.66%，BRISQUE提升48.38%。

Conclusion: 该方法在IMAGIN-e任务中成功部署，证明了其在空间环境中的实际应用价值，支持高分辨率图像处理。
...
TLDR: 本文研究了文本到图像扩散模型（如Stable Diffusion）在360度全景图像生成中的适应性，揭示了注意力模块中不同矩阵的作用，并提出了一种高效的方法UniPano。


<details>
  <summary>Details</summary>
Motivation: 探索预训练扩散模型在全景图像生成中的适应性机制，解决视角与全景图像之间的领域差距问题。

Method: 通过分析注意力模块中矩阵的行为，提出UniPano框架，优化了全景生成的效率和性能。

Result: UniPano在性能上优于现有方法，同时显著减少了内存使用和训练时间。

Conclusion: 注意力模块中的值矩阵和输出权重矩阵在全景生成中起关键作用，UniPano为未来研究提供了高效基线。
...
TLDR: FaceEditTalker是一个统一框架，支持在生成高质量音频同步说话视频的同时进行可控的面部属性编辑。


<details>
  <summary>Details</summary>
Motivation: 现有音频驱动说话头生成技术忽视了面部属性编辑的重要性，而这一能力对个性化数字头像、在线教育内容和品牌数字客服等应用至关重要。

Method: 方法包括图像特征空间编辑模块（提取语义和细节特征以控制属性）和音频驱动视频生成模块（融合编辑特征与音频引导的面部标志驱动扩散生成器）。

Result: 实验表明，该方法在唇同步精度、视频质量和属性可控性上优于现有技术。

Conclusion: FaceEditTalker实现了时间一致性、视觉保真度和身份保持，为面部属性编辑提供了高效解决方案。
...
TLDR: 论文提出cdViews方法，通过自动选择关键和多样化的2D视图，利用2D视觉语言模型（LVLMs）以零样本方式解决3D场景理解任务，并在3D-QA任务中取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 探索如何利用大型视觉语言模型（LVLMs）解决3D场景理解任务，尤其是在3D训练数据有限的情况下，避免直接训练3D模型。

Method: 提出cdViews方法，包括viewSelector（选择关键视图）和viewNMS（增强视图多样性），通过2D模型（如LLAVA-OV）处理3D点云的采样视图。

Result: 在ScanQA和SQA基准测试中，cdViews实现了最优性能，仅依赖2D模型且无需微调。

Conclusion: 2D LVLMs是当前解决3D任务最有效的替代方案，避免了资源密集型的3D LVLMs。
...
TLDR: 该论文提出了一种基于低维属性表示的框架，用于连接视觉工具感知和语言任务理解，显著提升了工具选择任务的准确性。
...
Method: 使用视觉编码器（ResNet或ViT）从工具图像中提取属性，并通过微调的语言模型（GPT-2、LLaMA、DeepSeek）从任务描述中推导所需属性。

Result: 该方法在工具选择任务中达到74%的准确率，显著优于直接工具匹配（20%）和小型多模态模型（21%-58%），且接近GPT-4o（73%）的性能。

Conclusion: 该工作提供了一种参数高效且可解释的解决方案，模拟了人类工具认知，推动了认知科学和实际应用的发展。
...
TLDR: 提出了一种名为FgB2I的方法，通过细粒度文本作为桥梁改进脑到图像重建，解决了现有方法中细节和语义不一致的问题。
...
Method: FgB2I分为三个阶段：细节增强、解码细粒度文本描述和基于文本的脑到图像重建，利用大视觉语言模型生成细粒度描述，并设计三种奖励指标指导解码。
...
TLDR: 论文提出了一种新颖的RGB-T目标检测框架，通过基础-辅助检测器架构和模态交互模块，有效应对极端模态不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 现实场景中，RGB和热红外（TIR）模态可能因环境或技术问题出现极端不平衡，导致模型训练和测试性能下降。

Method: 提出基础-辅助检测器架构，引入模态交互模块动态加权模态，并利用模态伪退化模拟训练数据不平衡。

Result: 实验显示，该方法显著降低了缺失率（55%），并在多种基线检测器上提升了性能。

Conclusion: 该方法增强了模型在极端模态退化下的鲁棒性，为RGB-T目标检测提供了可靠解决方案。
...
TLDR: Q-VDiT是一个专为视频DiT模型设计的量化框架，通过Token-aware Quantization Estimator（TQE）和Temporal Maintenance Distillation（TMD）解决了量化中的信息丢失和优化目标不对齐问题，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 视频DiT模型参数量大、计算复杂度高，难以部署在边缘设备上，现有量化方法在视频生成任务中表现不佳。

Method: 提出TQE补偿量化误差，TMD保持帧间时空相关性，优化整体视频上下文。

Result: W3A6 Q-VDiT的场景一致性达到23.40，性能提升1.9倍。

Conclusion: Q-VDiT为视频DiT模型量化提供了高效解决方案，性能显著提升。
...
TLDR: S2AFormer提出了一种高效的Vision Transformer架构，通过Strip Self-Attention（SSA）和Hybrid Perception Blocks（HPBs）结合CNN的局部感知与Transformer的全局建模能力，显著降低计算开销同时保持准确性。


<details>
  <summary>Details</summary>
Motivation: Vision Transformer（ViT）的计算需求随token数量呈二次增长，限制了其实际效率。现有方法结合卷积与自注意力，但自注意力的计算复杂度仍是瓶颈。

Method: 设计了SSA，通过降低K和V的空间维度并压缩Q和K的通道维度来减少计算开销，同时引入HPBs整合CNN与Transformer的优势。

Result: 在ImageNet-1k、ADE20k和COCO等基准测试中，S2AFormer在GPU和非GPU环境下均表现出显著的精度提升和高效性。

Conclusion: S2AFormer在效率和效果间取得了最优平衡，是高效Vision Transformer的有力候选方案。
...
TLDR: 研究探讨了视觉语言模型（VLMs）中图像与文本绑定的机制，提出Binding ID概念，并通过实验验证其有效性。
...
Method: 使用合成数据集和任务，分析VLMs如何为图像和文本分配Binding ID，实现跨模态关联。

Result: 实验表明，VLMs为图像和文本分配独特的Binding ID，支持上下文关联。
...
TLDR: 综述了无需训练的开放词汇语义分割方法，利用现有多模态分类模型，涵盖历史、方法分支及未来方向。


<details>
  <summary>Details</summary>
Motivation: 传统方法需要大量计算资源和标注数据，而开放词汇语义分割要求模型超越已学类别，数据标注成本高。因此转向无需训练的方法。

Method: 利用现有多模态分类模型（如CLIP），分为纯CLIP、辅助视觉基础模型和生成方法三大分支，综述30多种方法。

Result: 总结了当前研究的局限性和潜在问题，并提出了未来研究的未探索方向。

Conclusion: 本文为新手提供了入门指南，并希望激发对该领域的兴趣。
...
TLDR: 提出Look & Mark（L&M）策略，通过结合放射科医生的注视点和标注框，显著提升多模态大语言模型在胸片报告生成中的性能，减少临床错误。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在医学图像分析中存在幻觉和临床错误，限制了其实际应用的可靠性。

Method: 提出L&M策略，结合放射科医生的注视点（Look）和标注框（Mark），通过上下文学习提升模型性能，无需重新训练。

Result: L&M显著提升模型性能，如CXR-LLaVA提升1.2%，LLaVA-Med提升9.2%，并减少临床错误（平均每报告减少0.43个错误）。
...
TLDR: 本文提出了一种基于Hadamard乘积的自适应跨通道模块（ACH）和轻量级网络Hadaptive-Net，在视觉任务中实现了推理速度与精度的平衡。


<details>
  <summary>Details</summary>
Motivation: 尽管Hadamard乘积在增强网络表示能力和维度压缩方面具有潜力，但其实际应用尚未被系统探索。本文旨在填补这一空白。

Method: 通过分析Hadamard乘积在跨通道交互和通道扩展中的优势，提出自适应跨Hadamard模块（ACH）和轻量级网络Hadaptive-Net。

Result: 实验证明，Hadaptive-Net在视觉任务中实现了前所未有的推理速度与精度平衡。

Conclusion: 本文提出的ACH模块和Hadaptive-Net为Hadamard乘积的实际应用提供了有效解决方案。
...
TLDR: GoMatching++是一种参数和数据高效的方法，将现成的图像文本检测器转化为视频文本检测器，通过冻结图像检测器并引入轻量级可训练跟踪器，显著提升了视频文本检测性能。
...
Method: 提出GoMatching++，包括重新评分机制和LST-Matcher，以增强图像检测器在视频中的表现。

Result: 在多个基准测试中创下新记录，并显著降低训练成本。

Conclusion: GoMatching++和新基准ArTVideo将推动视频文本检测的未来发展。
...
TLDR: GradTrack利用医生的注视轨迹（包括注视点、持续时间和时间顺序）提升弱监督语义分割性能，通过注视轨迹图生成和轨迹注意力机制，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 医学影像中的弱监督语义分割（WSSS）难以有效利用稀疏标注，而现有基于注视的方法未充分利用注视数据中的丰富信息。

Method: 提出GradTrack框架，包含注视轨迹图生成和轨迹注意力两个关键组件，通过多级注视监督逐步优化特征。

Result: 在Kvasir-SEG和NCI-ISBI数据集上，GradTrack的Dice分数分别提升3.21%和2.61%，显著缩小与全监督模型的差距。

Conclusion: GradTrack通过充分利用注视数据，显著提升了WSSS性能，为医学影像分割提供了新思路。
...
TLDR: StateSpaceDiffuser通过结合状态空间模型（Mamba）的序列表示，解决了扩散模型在长上下文任务中视觉一致性快速丢失的问题，显著提升了长期记忆能力。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型因缺乏持久环境状态，导致在长序列任务中视觉一致性迅速崩溃。

Method: 引入StateSpaceDiffuser，将扩散模型与状态空间模型结合，以整合整个交互历史的序列表示。

Result: 实验表明，StateSpaceDiffuser在2D迷宫导航和复杂3D环境中均能保持视觉一致性，显著优于纯扩散模型。

Conclusion: 状态空间表示与扩散模型结合，有效兼顾了视觉细节和长期记忆能力。
...
TLDR: 该研究开发了YH-OSI系统，基于多模态大模型（MLLM）实现珊瑚礁生态监测的智能框架，提升了检测、分割和分类的准确性。


<details>
  <summary>Details</summary>
Motivation: 珊瑚礁生态监测面临人工分析效率低和复杂水下场景分割精度不足的挑战，亟需高效解决方案。

Method: 系统采用“目标检测-语义分割-先验输入”框架，结合目标检测模块和多模态模型，完成像素级分割和分类。

Result: 目标检测模块mAP@0.5达0.78，分类准确率88%，并能提取核心生态指标。

Conclusion: YH-OSI系统为未来集成到水下机器人奠定了基础，支持全流程自动化监测。
...
TLDR: HeadCLIP是一种零样本异常检测方法，通过自适应文本和图像编码器，结合可学习提示和动态调整的注意力头权重，显著提升了工业与医疗领域的异常检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有零样本异常检测方法在领域适应方面存在局限，未能全面调整模型组件或忽略领域特性。

Method: HeadCLIP通过可学习提示调整文本编码器，动态调整图像编码器的注意力头权重，并引入联合异常分数以利用像素级信息进行图像级检测。

Result: 在工业和医疗领域，HeadCLIP在像素级和图像级的异常检测得分上均优于现有方法，最高提升4.9%p和3.0%p。
...
TLDR: 论文提出了一种名为HDGS的深度监督框架，通过多尺度深度一致性提升稀疏视图下的新视角合成质量。


<details>
  <summary>Details</summary>
Motivation: 稀疏视图条件下，现有方法（如NeRF和3DGS）因几何线索不足导致重建质量下降，表现为模糊细节和结构伪影。深度渲染质量是关键因素。
...
TLDR: 论文提出了一种统一域自适应图像恢复（UDAIR）框架，通过域适应策略和对比学习机制，解决了多退化模式图像恢复中的域差距问题。


<details>
  <summary>Details</summary>
Motivation: 现有All-in-One图像恢复方法在真实场景中性能下降明显，主要由于训练数据与测试数据分布不一致。

Method: 设计了代码本学习退化模式，结合跨样本对比学习和动态域适应策略，通过测试时自适应机制优化特征对齐。

Result: 在10个开源数据集上，UDAIR实现了最先进的性能，并验证了对未知条件的鲁棒性。

Conclusion: UDAIR框架有效解决了域差距问题，提升了真实场景中的图像恢复性能。
...
TLDR: 本文提出了一种基于合成数据集生成和生成式AI的方法，用于自动修复数字化自色照片中的绿色缺陷，解决了现有方法在颜色还原和效率上的不足。


<details>
  <summary>Details</summary>
Motivation: 早期视觉艺术（尤其是彩色照片）的保存因老化和不当存储导致的模糊、划痕、颜色渗出和褪色等问题而面临挑战，需要高效的自动修复方法。

Method: 通过合成数据集生成模拟绿色缺陷，并利用生成式AI和精心设计的损失函数（改进的ChaIR方法）进行修复。

Result: 该方法能够高效修复绿色缺陷，减少时间需求，同时更准确地还原原始颜色。
...
TLDR: 论文提出ReCAD框架，用于自动检测和修正CAD程序中的错误，确保3D对象与参考图像一致，并创建了包含20K程序-图像对的数据集CADReview。


<details>
  <summary>Details</summary>
Motivation: 设计师在3D对象原型设计中需反复检查和修正CAD程序，耗时且低效。现有MLLMs在多几何组件识别和空间几何操作上表现不佳。

Method: 提出ReCAD框架，通过检测程序错误并提供修正反馈，同时构建CADReview数据集支持任务。

Result: 实验表明ReCAD显著优于现有MLLMs，展示了在设计应用中的潜力。

Conclusion: ReCAD框架有效解决了CAD程序与参考图像一致性问题，为设计自动化提供了新工具。
...
TLDR: IKIWISI是一种交互式视觉模式生成工具，用于评估视觉语言模型在视频对象识别中的表现，尤其在没有真实标签时。通过热图显示对象存在与否，并引入“间谍对象”检测模型幻觉。


<details>
  <summary>Details</summary>
Motivation: 传统评估方法依赖真实标签，而IKIWISI旨在通过人类直觉评估模型可靠性，揭示机器与人类感知的差异。

Method: IKIWISI将模型输出转化为二值热图（绿色表示对象存在，红色表示不存在），并加入“间谍对象”检测模型幻觉。

Result: 15名参与者认为IKIWISI易用，其评估结果与客观指标一致，且仅需少量热图单元即可得出结论。

Conclusion: IKIWISI不仅补充传统评估方法，还揭示了改进视觉语言模型与人类感知对齐的机会。
...
TLDR: 提出了一种统一的方法，通过3D扫描植物推断其参数化表示，适用于重建、分割和骨架化等多种任务。


<details>
  <summary>Details</summary>
Motivation: 植物3D重建因器官复杂性和自遮挡问题具有挑战性，现有方法多专注于特定任务或逆向建模。

Method: 使用基于L系统的虚拟植物训练递归神经网络，从3D点云推断参数化树状表示。

Result: 在合成植物上验证了方法的有效性，结果与现有技术相当。

Conclusion: 该方法为植物3D重建提供了一种统一且高效的解决方案。
...
TLDR: 论文提出了一种名为Progressive Data Dropout的新训练范式，通过减少有效训练轮次（降至基线的12.4%）来降低训练成本，同时不牺牲准确性，甚至提升高达4.82%。


<details>
  <summary>Details</summary>
Motivation: 当前机器学习依赖大数据集训练，成本高昂。尽管模型压缩研究较多，但数据利用效率的研究较少，标准训练方法仍采用均匀采样，效率低下。

Method: 结合硬数据挖掘和dropout的简单方法，提出Progressive Data Dropout，无需修改模型架构或优化器即可应用。

Result: 有效训练轮次降至基线的12.4%，准确性提升高达4.82%。

Conclusion: 该方法简单易用，适用于标准训练流程，有望成为新的训练标准。
...
TLDR: 提出了一种基于任务驱动的自动化LiDAR系统设计框架，通过六维设计空间和流生成模型实现高效设计。
...
Method: 在六维设计空间中表示LiDAR配置，通过流生成模型学习任务特定隐式密度，利用期望最大化方法合成新系统。

Result: 在3D视觉任务中验证了方法的有效性，适用于人脸扫描、机器人跟踪和物体检测等应用。

Conclusion: 该框架实现了高效、约束感知的LiDAR系统自动化设计，具有广泛的应用潜力。
...
TLDR: 论文提出VME和CDSI数据集，解决卫星图像中车辆检测的地理偏差问题，提升中东地区和全球的检测精度。
...
Result: VME显著提升中东地区检测精度，CDSI提升全球检测性能。
...
TLDR: 提出了一种新框架，通过解耦身份相关与无关特征并引入特征融合机制，提升文本到图像生成的质量和文本对齐。


<details>
  <summary>Details</summary>
Motivation: 当前方法在生成定制化图像时难以分离身份相关与无关信息，导致过拟合或身份丢失。

Method: 框架包括隐式-显式前景-背景解耦模块（IEDM）和基于专家混合（MoE）的特征融合模块（FFM），结合三种损失函数。

Result: 实验表明，该方法提升了生成图像质量、场景适应灵活性和输出多样性。

Conclusion: 新框架有效解决了身份信息解耦问题，显著提升了文本到图像生成的表现。
...
TLDR: 论文提出了一种多领域数据集压缩方法（MDDC），通过引入领域感知模块（DAM）提升压缩数据的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现代数据集通常是多领域的，现有数据集压缩方法未充分考虑这一点。

Method: 提出DAM模块，利用可学习的空间掩码嵌入领域特征，并采用基于频率的伪领域标签。

Result: DAM在域内、域外和跨架构性能上均优于基线方法。

Conclusion: MDDC和DAM有效提升了数据集压缩的泛化能力。
...
TLDR: PacTure是一个新框架，通过未纹理化的3D网格、文本描述和可选图像提示生成基于物理的渲染（PBR）材质纹理。它解决了现有方法在全局一致性和分辨率上的限制。


<details>
  <summary>Details</summary>
Motivation: 现有方法在生成纹理时存在全局不一致性和低分辨率问题，PacTure旨在通过新技术解决这些问题。

Method: 引入视图打包技术，将多视图映射视为2D矩形装箱问题，提高分辨率；同时采用多视图多域生成框架降低推理成本。

Result: 实验表明，PacTure在生成PBR纹理的质量和训练/推理效率上优于现有方法。

Conclusion: PacTure通过创新技术显著提升了纹理生成的全局一致性和效率。
...
TLDR: CcDPO是一种多级偏好优化框架，通过从序列上下文到局部细节的视觉线索增强多图像任务中的感知能力，显著减少幻觉并提升性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLMs）在多图像理解中存在跨模态不对齐问题，导致幻觉（如上下文遗漏、混淆和误解）。现有方法（如DPO）仅优化单一图像参考，忽视整体上下文建模。

Method: 提出CcDPO框架，包括上下文级优化（重新评估认知偏差并整合全局序列偏好）和细节级优化（通过区域目标视觉提示和多模态偏好监督关注细粒度细节）。构建MultiScope-42k数据集支持优化。

Result: 实验表明，CcDPO显著减少幻觉，并在单图像和多图像任务中实现一致的性能提升。

Conclusion: CcDPO通过多级优化有效解决了MLLMs在多图像任务中的局限性，为跨模态对齐提供了新思路。
...
TLDR: SRRL是一种基于自反思强化学习的扩散模型算法，通过多轮反思去噪过程实现逻辑图像生成，性能优于GPT-4o。


<details>
  <summary>Details</summary>
Motivation: 现有图像生成方法在逻辑为中心的生成任务中表现不佳，SRRL受CoT和RL启发，旨在解决这一问题。

Method: SRRL将去噪轨迹视为CoT步骤，引入条件引导的前向过程，实现多轮反思迭代。

Result: 实验表明SRRL在逻辑图像生成任务中表现优异，甚至超越GPT-4o。

Conclusion: SRRL首次将CoT引入图像生成任务，为逻辑图像生成提供了新思路。
...
TLDR: 提出了一种基于变分自编码器（VAE）的无回放增量学习方法，解决了增量学习中数据增长和灾难性遗忘的问题。
...
Result: 实验表明，该方法在内存效率上比相关方法高一个数量级，同时达到SOTA准确率。
...
TLDR: GeoDrive通过将3D几何条件显式整合到驾驶世界模型中，提升了空间理解和动作可控性，显著优于现有模型。
...
Result: 实验表明，GeoDrive在动作准确性和3D空间感知上显著优于现有模型，支持交互式场景编辑。
...
TLDR: 提出了一种首创的在线自动雷达与相机系统几何校准方法，解决了雷达高度数据稀疏性和测量不确定性的挑战。


<details>
  <summary>Details</summary>
Motivation: 雷达高度数据的稀疏性和不确定性使得系统运行期间的自动校准成为长期难题。

Method: 采用双视角表示（正面视角和鸟瞰视角），提出选择性融合机制和多模态交叉注意力机制，并设计了抗噪声匹配器。
...
TLDR: SeeGround是一个零样本3D视觉定位框架，利用2D视觉语言模型，无需3D特定训练，通过混合输入格式和核心模块提升定位精度。
...
Method: 引入混合输入格式（查询对齐的渲染视图和空间丰富的文本描述），包含视角适应模块和融合对齐模块。

Result: 在ScanRefer和Nr3D上显著优于零样本基线（分别提升7.7%和7.1%），甚至媲美全监督方法。

Conclusion: SeeGround展示了在挑战性条件下强大的泛化能力。
...
TLDR: 论文提出了一种基于距离变换和分层混合的单域泛化方法，用于解决阿尔茨海默病检测中的类别不平衡和数据集多样性不足问题。
...
Method: 扩展了mixup方法，通过计算MRI扫描的距离变换，将其分层并组合不同样本的层以生成增强图像。

Result: 实验结果表明，该方法在ADNI和AIBL数据集上提升了泛化性能。

Conclusion: 提出的方法能够生成多样化的数据并保留大脑结构，有效改善了阿尔茨海默病检测的泛化能力。
...
TLDR: NeRFs被重新设计以学习多路径信号（如WiFi），从而推断环境，如室内平面图。


<details>
  <summary>Details</summary>
Motivation: 探讨是否可以通过多路径信号（如RF/音频）推断环境，尽管这些信号包含复杂的反射。

Method: 重新设计NeRFs，使其能够从多路径信号中学习，并以稀疏WiFi测量为例推断室内平面图。

Result: 学习到的隐含平面图效果良好，支持信号预测和基础光线追踪等应用。

Conclusion: NeRFs可以扩展至多路径信号领域，为环境推断提供新方法。
...
TLDR: 论文提出了一种几何感知的参数高效微调模块GEM，用于3D点云模型，显著减少了计算和存储成本，同时保持性能。
...
Method: GEM模块结合局部位置编码和轻量级潜在注意力机制，捕捉全局几何上下文。

Result: GEM仅更新1.6%参数，性能接近或超过全微调，训练时间和内存需求显著降低。
...
TLDR: 提出了一种基于学习的3D形状配准框架，无需标注对应关系即可处理显著非刚性变形和部分形状匹配问题。


<details>
  <summary>Details</summary>
Motivation: 解决传统方法在处理非刚性变形和部分形状匹配时的局限性，同时避免对标注数据的依赖。

Method: 结合深度学习提取的神经特征与几何配准流程，动态更新和过滤对应关系。

Result: 在多个基准测试中取得最优性能，并能处理未见过的复杂变形形状对。

Conclusion: 该框架在非刚性点云匹配和部分形状匹配中表现出色，具有广泛适用性。
...
TLDR: 论文提出了一个名为“next-event prediction (NEP)”的学习任务，旨在通过利用未来视频片段作为自监督信号，提升多模态大语言模型（MLLMs）的时间推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有任务（如视频问答或视频字幕生成）依赖人工标注或强模型，且容易混淆时间推理与空间信息。NEP任务旨在填补这一空白。

Method: 将视频分为过去和未来帧，MLLM以过去帧为输入，预测未来帧的事件摘要，从而促进时间推理。为此，构建了V1-33K数据集，并探索了多种视频指令调优策略。

Result: 实验验证了NEP作为一种可扩展且有效的训练范式，能够显著提升MLLMs的时间推理能力。

Conclusion: NEP为MLLMs的时间推理提供了一种新的自监督学习任务，并通过实验证明了其有效性。
...
TLDR: 论文提出了一种新的无监督领域自适应方法UniDA-SS，通过UniMAP框架解决类别设置未知时的语义分割问题，显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 传统UDA-SS方法假设源域和目标域的类别设置已知，这在现实中不切实际，导致性能下降。论文旨在解决这一问题。

Method: 提出UniMAP框架，包含两个关键组件：DSPD（域特定原型区分）和TIM（基于目标的图像匹配），以增强跨域共同类别的识别。

Result: 实验表明UniMAP在UniDA-SS基准上显著优于基线方法。

Conclusion: UniMAP为无监督领域自适应语义分割提供了一种有效的解决方案，尤其在类别设置未知时表现优异。
...
TLDR: SHTOcc方法通过稀疏头尾体素构建和分离学习，解决了3D占用预测中的长尾问题和几何分布问题，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法未探索体素的基本分布模式，导致结果不理想，尤其是长尾问题和几何分布问题。

Method: 提出SHTOcc，采用稀疏头尾体素构建和分离学习，平衡关键体素并减少模型对主导类别的偏向。
...
TLDR: 本文提出了一种结合可学习伪形态模块和监督对比学习的方法，用于提高阿尔茨海默病MRI检测的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决阿尔茨海默病MRI检测中因类别不平衡、协议差异和数据集多样性不足导致的泛化能力受限问题。

Method: 使用可学习伪形态模块生成形状感知的解剖学有意义增强，并结合监督对比学习提取鲁棒的类别特定表示。

Result: 在三个数据集上的实验表明，该方法在类别不平衡和成像协议差异下表现出更好的性能和泛化能力。

Conclusion: 该方法有效提升了阿尔茨海默病检测的泛化性，尤其在数据分布差异大的情况下表现优异。
...
TLDR: ProCrop是一种基于检索的图像裁剪方法，利用专业摄影作品指导裁剪决策，显著提升性能，并提供了一个大规模弱标注数据集。
...
Method: ProCrop通过融合专业照片和查询图像的特征，学习专业构图。同时，通过外绘专业图像生成242K弱标注数据集，提供多样高质量裁剪建议。

Result: ProCrop在监督和弱监督设置下均显著优于现有方法，新数据集使其性能甚至媲美全监督方法。
...
TLDR: 该论文研究了3D物体检测模型对3D对抗攻击的脆弱性，提出了一种生成非侵入式3D对抗物体的方法，并通过实验验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 3D物体检测是自动驾驶系统的关键组件，但其对对抗攻击的脆弱性尚未充分研究。通过生成3D对抗物体，可以评估模型的鲁棒性。

Method: 采用可微分渲染技术建模对抗物体与目标车辆的空间关系，引入遮挡感知模块增强视觉一致性，并设计BEV空间特征引导的优化策略。

Result: 实验表明，该方法能有效抑制先进3D物体检测器的预测，且生成的对抗物体在不同位置和距离下仍保持攻击效果。

Conclusion: 该方法为评估3D物体检测模型的鲁棒性提供了重要工具，对抗物体表现出强泛化能力。
...
TLDR: PathFL是一种多对齐联邦学习框架，通过图像、特征和模型聚合的三级对齐策略解决病理图像分割中的异质性问题。


<details>
  <summary>Details</summary>
Motivation: 病理图像分割在多中心应用中面临成像模态、器官和扫描设备等异质性带来的挑战，导致模型泛化能力受限。

Method: PathFL采用三级对齐策略：图像级（协同风格增强模块）、特征级（自适应特征对齐模块）和模型聚合级（分层相似性聚合策略）。

Result: 在四种异质性病理图像数据集上的评估表明，PathFL在性能和鲁棒性方面优于其他方法。

Conclusion: PathFL通过多级对齐策略有效解决了数据异质性，提升了病理图像分割的泛化能力。
...
TLDR: 提出了一种新范式，通过生成中间视觉思考步骤，使大型多模态模型（LMMs）能够在文本和视觉模态之间进行推理。
...
Method: 通过两种机制实现：（1）分解复杂视觉任务为可管理的子目标并逐步生成；（2）生成初始视觉假设后，通过文本推理分析其不足并优化。

Result: 在复杂多对象场景处理中，相对基线方法提升了50%（从38%到57%）。

Conclusion: 该方法为AI模型提供了类似人类视觉想象和迭代优化的能力，适用于多个领域。
...
TLDR: 论文提出了一种生成高质量多层透明图像的方法，包括数据集发布、合成管道和多层生成模型ART+，显著提升了透明图像生成的质量和可控性。


<details>
  <summary>Details</summary>
Motivation: 解决多层透明图像生成领域缺乏高质量数据集的问题，推动创意编辑的便捷性。

Method: 1. 发布PrismLayersPro数据集；2. 提出无需训练的合成管道；3. 开发多层生成模型ART+，包含LayerFLUX和MultiLayerFLUX技术。

Result: ART+在用户研究中60%情况下优于原ART模型，视觉质量接近FLUX.1-[dev]模型。
...
TLDR: RICO框架通过视觉重建优化图像描述，结合文本到图像模型和多模态大语言模型，迭代提升描述的准确性和完整性。RICO-Flash通过DPO学习生成类似RICO的描述，减少计算成本。实验显示其在CapsBench和CompreCap上优于基线约10%。


<details>
  <summary>Details</summary>
Motivation: 现有图像重描述方法依赖多模态大语言模型，但常因幻觉和细节缺失导致不准确和不完整。

Method: 利用文本到图像模型重建参考图像，通过MLLM识别差异并迭代优化描述；RICO-Flash通过DPO学习生成类似描述以减少计算成本。

Result: 在CapsBench和CompreCap上显著提升描述准确性和完整性，优于基线约10%。

Conclusion: RICO框架通过视觉重建和迭代优化有效解决了现有方法的局限性，RICO-Flash进一步降低了计算成本。
...
TLDR: RiverMamba是一种新型深度学习模型，用于全球河流流量和洪水预测，优于现有AI和物理模型。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习模型在水文学中局限于局部应用，未能利用水体的空间联系，需要新方法建模时空关系以提高预测能力。

Method: RiverMamba基于Mamba块，预训练长期再分析数据，整合ECMWF HRES气象预测，并通过时空建模处理其不准确性。

Result: RiverMamba在0.05°网格上提供长达7天的可靠预测，包括极端洪水事件，表现优于现有模型。

Conclusion: RiverMamba通过全球尺度建模和高效预测能力，为科学和业务应用提供了更优的河流流量和洪水预测解决方案。
...
TLDR: Sherlock是一个自校正和自我改进的训练框架，显著提升了视觉语言模型的推理能力，仅需少量标注数据即可实现高性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在复杂多模态任务中表现优异，但对推理错误敏感，依赖大量标注数据或验证器，且泛化能力有限。

Method: 引入轨迹级自校正目标、基于视觉扰动的偏好数据构建方法及动态β偏好调整，仅需20k随机标注数据即可实现自校正。

Result: 在八个基准测试中平均准确率达64.1（直接生成）和65.4（自校正后），优于其他模型且数据需求减少80%。

Conclusion: 自校正策略显著提升模型性能，减少对标注数据的依赖，为视觉语言模型的改进提供了新方向。
...
TLDR: OmniVQA框架通过构建大规模多模态指令数据库（MIDB）和数据集，解决了视频质量评估（VQA）领域数据稀缺的问题，并提出了互补训练策略和细粒度基准测试，实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 在视频质量评估领域，数据稀缺和数据集规模不足限制了数据缩放定律的应用潜力。

Method: 提出OmniVQA框架，构建OmniVQA-Chat-400K和OmniVQA-MOS-20K数据集，采用互补训练策略，并设计OmniVQA-FG-Benchmark进行细粒度评估。

Result: 模型在质量理解和评分任务中均达到最先进性能。

Conclusion: OmniVQA框架通过数据扩展和互补训练策略，显著提升了视频质量评估任务的性能。
...
TLDR: 利用深度学习从膝关节X光片中预测骨密度（BMD），并通过不确定性量化提高临床可信度。


<details>
  <summary>Details</summary>
Motivation: DXA设备有限，阻碍骨质疏松筛查，需利用广泛可用的膝关节X光片进行机会性BMD估计。

Method: 使用EfficientNet模型在OAI数据集上训练，比较两种测试时间增强（TTA）方法，并采用Split Conformal Prediction生成患者特异性预测区间。

Result: Pearson相关性为0.68（传统TTA），多样本方法生成更紧的置信区间，同时保持覆盖率。
...
TLDR: 论文提出3DLLM-Mem模型，通过动态记忆管理和融合技术，提升LLMs在3D环境中的空间-时间推理能力，并在3DMem-Bench基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型（LLMs）在动态多房间3D环境中规划和行动能力不足，主要原因是缺乏有效的3D空间-时间记忆建模。

Method: 提出3DLLM-Mem模型，利用工作记忆令牌选择性融合来自情景记忆的空间和时间特征，以优化任务相关信息处理。

Result: 3DLLM-Mem在3DMem-Bench测试中表现最佳，成功率达16.5%的提升。
...
TLDR: MultiFormer是一种基于CSI的无线感知系统，通过Transformer和特征融合网络提高多人姿态估计的准确性。


<details>
  <summary>Details</summary>
Motivation: 解决基于CSI的多人体姿态估计中的准确性和特征学习问题。

Method: 采用基于Transformer的时间-频率双令牌特征提取器和多阶段特征融合网络（MSFN）。

Result: 在公开和自收集数据集上表现优于现有方法，尤其对高动态关键点（如手腕、肘部）估计更准确。
...
TLDR: PRISM是一种新颖的视频数据集压缩方法，通过渐进式细化和插入稀疏运动帧，保留时空动态的相互依赖关系，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决大规模视频数据处理中的计算挑战，同时克服视频领域时空动态的复杂性。

Method: PRISM方法通过渐进式细化和插入帧，保留静态内容与动态运动的相互依赖关系，并考虑每帧的梯度关系。
...
TLDR: VTV-LLM是首个多模态大语言模型，用于通用视觉-触觉视频（VTV）理解，结合触觉感知与自然语言。通过VTV150K数据集和三阶段训练范式，实现了触觉推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法未能有效整合触觉信息，而触觉感知对理解物体的物理属性至关重要。

Method: 提出VTV-LLM模型，使用VTV150K数据集（150,000帧视频，100种物体，3种触觉传感器），采用三阶段训练范式（VTV增强、VTV-文本对齐、文本提示微调）。

Result: VTV-LLM在触觉视频理解任务中表现优异，支持特征评估、比较分析和场景决策等能力。
...
TLDR: 本文提出两种方法解决扩散模型在人类偏好对齐与多样性间的权衡：1）组合生成策略，2）ImageReFL微调方法，显著提升了生成图像的质量与多样性。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像生成方面表现优异，但与人类偏好的对齐仍具挑战性，且传统奖励微调方法会损害多样性。

Method: 1）组合生成策略：在生成后期应用奖励调优模型，保留基础模型以维持多样性；2）ImageReFL：结合真实图像和多种正则化损失进行微调。

Result: 实验表明，该方法在质量和多样性指标上优于传统奖励微调方法，用户研究也证实其更好地平衡了偏好与多样性。

Conclusion: 本文提出的方法有效解决了扩散模型在人类偏好对齐与多样性间的权衡问题，具有实际应用价值。
...
TLDR: 论文介绍了首个大规模阿拉伯语-英语视听深度伪造数据集ArEnAV，包含387k视频和765小时内容，用于解决多语言和语码转换的深度伪造检测挑战。


<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测研究多关注单语内容，忽视了多语言和语码转换的挑战，尤其是阿拉伯语和英语混合的情况。

Method: 通过整合四种文本到语音和两种唇同步模型，生成包含语码转换、方言变体和单语内容的数据集。

Result: 数据集包含387k视频和765小时内容，并在现有单语和多语数据集、先进检测模型及人类评估中进行了基准测试。

Conclusion: ArEnAV数据集为多语言多模态深度伪造检测研究提供了重要资源，推动了该领域的发展。
...
TLDR: SAM-R1是一种利用强化学习训练多模态大模型进行图像分割的新框架，无需依赖昂贵的人工标注推理数据。
...
Method: 结合任务特定的细粒度奖励和优化目标，利用Segment Anything Model（SAM）作为奖励提供者，训练多模态模型。
...
TLDR: 论文提出了一种无需训练的对抗防御方法TRIM，用于检测AI生成图像（AIGI），通过信息论方法量化特征偏移，显著优于现有防御方法。


<details>
  <summary>Details</summary>
Motivation: AI生成图像的恶意使用（如伪造和虚假信息）日益严重，现有检测器普遍易受对抗攻击，而防御方法稀缺。

Method: 提出TRIM方法，基于标准检测器，利用预测熵和KL散度量特征偏移，无需额外训练。

Result: 在多数据集和攻击下，TRIM显著优于现有防御方法，如ProGAN和GenImage上分别提升33.88%和28.91%。

Conclusion: TRIM是首个无需训练的对抗防御方法，有效解决了AIGI检测中的性能崩溃问题，同时保持原始检测精度。
...
TLDR: 提出了一种基于视频帧插值的数据增强方法PS4PRO，用于提升神经渲染在静态和动态场景中的重建性能。
...
Method: 设计轻量级高质量视频帧插值模型PS4PRO，通过像素级监督增强3D重建的光照监督。

Result: 实验表明，该方法显著提升了静态和动态场景的重建性能。

Conclusion: PS4PRO作为一种隐式世界先验，有效丰富了神经渲染的数据集，提升了重建质量。
...
TLDR: 论文提出新数据集OBER和框架ObjectClear，用于精确移除物体及其视觉影响（如阴影、反射），并通过注意力机制提升背景保真度。


<details>
  <summary>Details</summary>
Motivation: 现有扩散修复方法在移除物体及其视觉影响时易产生伪影或改变背景，需更精准的解决方案。

Method: 构建OBER数据集，提出ObjectClear框架，利用物体效应注意力机制学习注意力掩码，分离前景移除与背景重建。

Result: ObjectClear在复杂场景中表现优于现有方法，显著提升物体效应移除质量和背景保真度。

Conclusion: OBER数据集和ObjectClear框架为物体效应移除提供了有效工具，尤其在复杂场景中表现突出。
...
TLDR: 论文提出了一种名为Spiral的新型LiDAR扩散模型，能够同时生成深度、反射率图像和语义地图，解决了现有方法在跨模态一致性上的不足，并在实验中表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有基于范围视图的方法只能生成无标签的LiDAR场景，依赖预训练分割模型预测语义地图会导致跨模态一致性不佳。

Method: 提出Spiral模型，利用扩散模型同时生成深度、反射率图像和语义地图，并引入新的语义感知指标评估生成数据质量。

Result: 在SemanticKITTI和nuScenes数据集上，Spiral以最小的参数量实现了最先进的性能，优于结合生成和分割模型的两步方法。

Conclusion: Spiral生成的范围图像可用于下游分割训练的合成数据增强，显著减少LiDAR数据标注工作量。
...
TLDR: 提出了一种新任务：多人对话视频生成，并提出了MultiTalk框架解决多人生成中的音频绑定和指令跟随问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注单人动画，难以处理多流音频输入，存在音频与人物绑定错误及指令跟随能力不足的问题。

Method: 提出Label Rotary Position Embedding (L-RoPE)方法解决音频绑定问题，并通过部分参数训练和多任务训练保留基础模型的指令跟随能力。

Result: MultiTalk在多个数据集（如说话头部、说话身体和多人数据集）上表现优于其他方法。
...
TLDR: VScan是一个两阶段视觉令牌减少框架，通过全局和局部扫描合并令牌以及在语言模型中间层剪枝，显著加速推理并保持高性能。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型（LVLM）因视觉令牌序列较长导致计算成本高，难以实时部署。现有方法在视觉编码器输出层或语言模型早期层剪枝令牌，但效果有限。

Method: 提出VScan框架，包括视觉编码阶段的全局和局部扫描令牌合并，以及语言模型中间层的令牌剪枝。

Result: 在四个LVLM上验证，VScan显著加速推理（LLaVA-NeXT-7B预填充速度提升2.91倍，FLOPs减少10倍），性能保留95.4%。

Conclusion: VScan通过优化令牌处理流程，在加速推理的同时保持高性能，优于现有方法。
...
TLDR: 提出了一种无需训练的方法，通过视觉语言模型和跨域校正流反演策略生成风格化抽象图像，支持多轮生成，并在StyleBench上验证了其性能。


<details>
  <summary>Details</summary>
Motivation: 解决风格化抽象任务中如何在保留身份线索的同时实现风格化的问题，尤其是在分布外个体上的挑战。

Method: 使用视觉语言模型提取身份相关特征，结合跨域校正流反演策略动态重建结构，并通过风格感知时间调度实现高保真重建。

Result: 在多种风格化抽象任务（如LEGO、针织玩偶、南方公园）中表现出色，能够泛化到未见过的身份和风格。

Conclusion: 该方法在无需微调的情况下实现了高质量的风格化抽象生成，并通过StyleBench验证了其有效性。
...
TLDR: 论文提出了一种通过训练小型“代理模型”来降低视觉语言模型（VLM）训练成本的方法，称为零-shot嫁接。该方法通过继承目标大语言模型的浅层来共享嵌入空间和表示语言，训练后的视觉编码器可直接迁移到大模型上，性能接近完整训练，同时降低约45%的训练成本。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型通常将视觉编码器与大语言模型（如Llama-70B）配对，训练成本主要来自解码器。为降低成本，研究探索了先使用小语言模型训练视觉编码器再迁移到大模型的策略。

Method: 构建小型“代理模型”，通过继承目标大语言模型的浅层共享嵌入空间和表示语言。训练视觉编码器后，直接迁移到大模型（零-shot嫁接）。

Result: 嫁接后的模型性能优于编码器-代理模型对，部分基准测试中甚至与完整训练的目标大语言模型相当，同时训练成本降低约45%。

Conclusion: 零-shot嫁接是一种高效降低VLM训练成本的方法，性能接近完整训练，具有实际应用潜力。
...
### [263] [ChemHAS: Hierarchical Agent Stacking for Enhancing Chemistry Tools](https://arxiv.org/abs/2505.21569)
*Zhucong Li,Bowei Zhang,Jin Xiao,Zhijian Zhou,Fenglei Cao,Jiaqing Liang,Yuan Qi*

Main category: cs.LG

TLDR: 本文提出ChemHAS方法，通过优化LLM代理堆叠结构减少化学工具预测误差，在四项基础化学任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: LLM代理在化学任务中表现受限，因化学工具存在预测误差，需探索如何利用LLM代理减少这些误差。

Method: 提出ChemHAS方法，通过优化代理堆叠结构从有限数据中提升化学工具性能。

Result: ChemHAS在四项基础化学任务中达到最优性能，并识别出四种代理堆叠行为。
...
### [264] [Revisiting Bi-Linear State Transitions in Recurrent Neural Networks](https://arxiv.org/abs/2505.21749)
*M. Reza Ebrahimi,Roland Memisevic*

Main category: cs.LG

TLDR: 论文探讨了循环神经网络中隐藏单元的双线性操作，提出其作为状态跟踪任务的天然归纳偏置。


<details>
  <summary>Details</summary>
Motivation: 研究隐藏单元在计算中的主动作用，而非仅作为被动记忆存储。

Method: 理论分析和实证验证双线性操作在状态跟踪任务中的有效性。

Result: 双线性状态更新形成与任务复杂度对应的自然层次结构。

Conclusion: 双线性操作为隐藏状态演化提供了更自然的表示方式。
...
### [265] [Born a Transformer -- Always a Transformer?](https://arxiv.org/abs/2505.21785)
*Yana Veitsman,Mayank Jobanputra,Yash Sarrof,Aleksandra Bakalova,Vera Demberg,Ellie Pavlick,Michael Hahn*

Main category: cs.LG

TLDR: 研究探讨了预训练大语言模型（LLM）在序列任务中的理论限制，发现预训练会选择性增强某些能力，但无法克服长度泛化的根本限制。


<details>
  <summary>Details</summary>
Motivation: 探索Transformer架构在大规模预训练后是否仍受理论限制影响，以及预训练是否能克服这些限制。

Method: 通过检索和复制任务，结合C-RASP框架，分析预训练模型的长度泛化能力。

Result: 发现预训练模型在检索任务中存在诱导与反诱导不对称性，且这种不对称性可通过微调消除。

Conclusion: 预训练选择性增强某些能力，但无法突破长度泛化的根本限制。
...
### [266] [From Directions to Cones: Exploring Multidimensional Representations of Propositional Facts in LLMs](https://arxiv.org/abs/2505.21800)
*Stanley Yu,Vaidehi Bulusu,Oscar Yasunaga,Clayton Lau,Cole Blondin,Sean O'Brien,Kevin Zhu,Vasu Sharma*

Main category: cs.LG

TLDR: 论文扩展了概念锥框架，用于研究LLM中真实性的多维度几何结构，并通过因果干预验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM具有强大的对话能力，但常生成虚假信息，现有研究未能完全捕捉其内部几何结构。

Method: 扩展概念锥框架，识别多维度锥体，并通过因果干预验证其作用。

Result: 发现多维度锥体可因果性地调节LLM的真实性行为，且干预不影响无关行为。
...
### [267] [Let Me Think! A Long Chain-of-Thought Can Be Worth Exponentially Many Short Ones](https://arxiv.org/abs/2505.21825)
*Parsa Mirtaheri,Ezra Edelman,Samy Jelassi,Eran Malach,Enric Boix-Adsera*

Main category: cs.LG

TLDR: 论文探讨了推理时计算在提升大语言模型推理能力中的作用，重点比较了顺序扩展（如更长的思维链）和并行扩展（如多数投票）的优势，发现某些图连通性问题中顺序扩展具有指数级优势。


<details>
  <summary>Details</summary>
Motivation: 研究推理时计算的最优分配方式，以提升大语言模型的推理能力，解决顺序扩展与并行扩展的优劣问题。

Method: 通过理论分析和实验验证，比较顺序扩展与并行扩展在图连通性问题中的表现，涵盖多种语言模型和思维链策略。

Result: 在特定图连通性问题的分布中，顺序扩展比并行扩展具有指数级优势。
...
### [268] [Efficient Ensemble for Fine-tuning Language Models on Multiple Datasets](https://arxiv.org/abs/2505.21930)
*Dongyue Li,Ziniu Zhang,Lu Wang,Hongyang R. Zhang*

Main category: cs.LG

TLDR: 提出一种基于多数据集的语言模型微调集成方法，通过分组训练适配器并加权组合，显著提升效率与性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如QLoRA）在单数据集上高效，但多任务场景下缺乏高效微调方案。

Method: 将n个数据集分组为m个（m<<n），每组训练一个适配器，加权组合成集成模型，利用低秩适配的一阶近似快速估计性能。

Result: 在34B参数模型上，性能估计误差<5%，计算速度提升105倍；在Llama和GPT上，测试准确率比QLoRA高10%，FLOPs仅增9%。
...
### [269] [EnsemW2S: Enhancing Weak-to-Strong Generalization with Large Language Model Ensembles](https://arxiv.org/abs/2505.21959)
*Aakriti Agrawal,Mucong Ding,Zora Che,Chenghao Deng,Anirudh Satheesh,Bang An,Bayan Bruss,John Langford,Furong Huang*

Main category: cs.LG

TLDR: 论文提出了一种名为EnsemW2S的方法，通过集成多个弱专家模型来提升其对强学生模型的监督能力，显著改进了弱到强的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型（LLMs）接近或超越人类水平，需要开发能够用有限的人类水平数据监督和增强这些强大模型的方法。

Method: 采用基于token级别的集成策略，迭代结合多个弱专家模型，逐步改进其能力。

Result: 在分布内（ID）和分布外（OOD）数据集上，弱专家和学生模型分别实现了4%、3.2%和6%、2.28%的性能提升。

Conclusion: EnsemW2S方法有效提升了弱到强的泛化能力，为监督强大模型提供了新思路。
...
### [270] [Pitfalls of Rule- and Model-based Verifiers -- A Case Study on Mathematical Reasoning](https://arxiv.org/abs/2505.22203)
*Yuzhen Huang,Weihao Zeng,Xingshan Zeng,Qi Zhu,Junxian He*

Main category: cs.LG

TLDR: 论文分析了强化学习中验证器的可靠性问题，重点关注数学推理领域，发现规则和模型验证器各有缺陷，并提出改进方向。


<details>
  <summary>Details</summary>
Motivation: 验证器在强化学习中的可靠性及其对训练过程的影响尚不明确，需深入研究。

Method: 以数学推理为例，对比分析规则和模型验证器在静态评估和强化训练中的表现。

Result: 规则验证器易漏判等效答案，模型验证器易被攻击导致误判，均影响训练效果。

Conclusion: 需开发更稳健的验证器系统，以提升强化学习的可靠性。
...
### [271] [Train Sparse Autoencoders Efficiently by Utilizing Features Correlation](https://arxiv.org/abs/2505.22255)
*Vadim Kurochkin,Yaroslav Aksenov,Daniil Laptev,Daniil Gavrilov,Nikita Balagansky*

Main category: cs.LG

TLDR: KronSAE通过Kronecker积分解和mAND激活函数，解决了稀疏自编码器在大规模训练中的计算和内存问题。


<details>
  <summary>Details</summary>
Motivation: 稀疏自编码器在解释语言模型隐藏状态时表现出潜力，但大规模训练时计算和内存开销大。

Method: 提出KronSAE架构，利用Kronecker积分解降低计算和内存开销，并引入mAND激活函数提升性能。

Result: KronSAE显著减少了计算和内存需求，同时提高了模型的解释性和性能。
...
### [272] [Skywork Open Reasoner 1 Technical Report](https://arxiv.org/abs/2505.22312)
...
TLDR: Skywork-OR1通过强化学习显著提升了大型语言模型的推理能力，在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 探索强化学习在提升长链思维模型推理能力中的作用。

Method: 基于DeepSeek-R1-Distill模型系列，采用可扩展的强化学习方法。

Result: 32B和7B模型在多个基准测试中准确率显著提升，超越同类模型。

Conclusion: 缓解熵崩溃现象对提升测试性能至关重要，模型权重和训练数据已开源。
...
### [273] [Mitigating Overthinking in Large Reasoning Models via Manifold Steering](https://arxiv.org/abs/2505.22411)
*Yao Huang,Huanran Chen,Shouwei Ruan,Yichi Zhang,Xingxing Wei,Yinpeng Dong*

Main category: cs.LG

TLDR: 论文提出了一种名为Manifold Steering的新方法，通过低维流形投影减少大型推理模型（LRMs）中的过度思考现象，显著降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂任务中表现出过度思考现象，导致计算资源浪费，本文旨在从机制可解释性角度解决这一问题。

Method: 研究发现过度思考与激活空间的低维流形相关，提出Manifold Steering方法，将干预方向投影到低维流形以减少噪声干扰。

Result: 实验表明，该方法在数学基准测试中减少高达71%的输出令牌，同时保持或提升准确性，并在代码生成和知识问答任务中表现稳定。

Conclusion: Manifold Steering有效缓解了过度思考问题，具有跨领域适用性，为优化大型推理模型提供了新思路。
...
### [274] [Scaling Reasoning without Attention](https://arxiv.org/abs/2505.22425)
*Xueliang Zhao,Wei Wu,Lingpeng Kong*

Main category: cs.LG

TLDR: 提出了一种基于状态空间双层的无注意力语言模型，通过架构和数据创新解决了Transformer的低效性和缺乏结构化微调的问题，在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在复杂推理任务中取得进展，但仍受限于Transformer架构的低效性和缺乏针对高难度领域的结构化微调。

Method: 采用状态空间双层（SSD）架构消除自注意力需求，提出基于PromptCoT的两阶段课程微调策略。

Result: 在多个基准测试中，7B参数的模型表现优于同规模Transformer和混合模型，甚至超越更大规模的Gemma3-27B。
...
### [275] [The Entropy Mechanism of Reinforcement Learning for Reasoning Language Models](https://arxiv.org/abs/2505.22617)
...
TLDR: 论文研究了强化学习中策略熵崩溃的问题，提出了熵与性能的关系方程，并通过理论和实证分析揭示了熵动态变化的机制，最终提出了两种有效的熵管理方法。
...
Method: 通过理论推导和实证研究分析熵动态变化机制，提出Clip-Cov和KL-Cov两种熵管理技术。

Result: 实验证明所提方法能有效避免熵崩溃，提升探索能力和下游任务性能。

Conclusion: 熵管理对强化学习的持续探索和性能提升至关重要，提出的方法在实践中表现优异。
...
### [276] [Position: Uncertainty Quantification Needs Reassessment for Large-language Model Agents](https://arxiv.org/abs/2505.22655)
*Michael Kirchhof,Gjergji Kasneci,Enkelejda Kasneci*

Main category: cs.LG

TLDR: 论文探讨了大型语言模型（LLM）和聊天机器人代理在交互式环境中不确定性量化的局限性，提出了三种新的研究方向以改进不确定性表达。


<details>
  <summary>Details</summary>
Motivation: 传统的不确定性二分法（偶然性和认知性）在LLM代理与用户的交互中显得过于局限，需要更丰富的不确定性表达方式。

Method: 通过文献综述发现传统定义在交互场景中失效，提出了三种新方向：未明确性不确定性、交互式学习和输出不确定性。

Result: 传统不确定性定义在交互场景中矛盾且失去意义，新方向有望提升透明度和信任度。

Conclusion: 新提出的不确定性研究方向可以改善LLM代理的交互体验，使其更透明、可信和直观。
...
### [277] [Temporal Restoration and Spatial Rewiring for Source-Free Multivariate Time Series Domain Adaptation](https://arxiv.org/abs/2505.21525)
*Peiliang Gong,Yucheng Wang,Min Wu,Zhenghua Chen,Xiaoli Li,Daoqiang Zhang*

Main category: cs.LG

TLDR: TERSE是一种针对多变量时间序列（MTS）的无源域自适应（SFDA）方法，通过时空特征编码和任务设计，解决了现有方法忽视空间相关性的问题。


<details>
  <summary>Details</summary>
Motivation: 现有SFDA方法在多变量时间序列（MTS）上表现不佳，因为它们未考虑MTS数据固有的空间相关性。

Method: 提出TERSE方法，包含时空特征编码器、时间恢复和空间重连任务，以捕捉和转移跨域的时空依赖性。

Result: 在三个真实时间序列数据集上的实验验证了TERSE的有效性和通用性。
...
### [278] [Taming Transformer Without Using Learning Rate Warmup](https://arxiv.org/abs/2505.21910)
*Xianbiao Qi,Yelin He,Jiaquan Ye,Chun-Guang Li,Bojia Zi,Xili Dai,Qin Zou,Rong Xiao*

Main category: cs.LG

TLDR: 论文提出了一种新的优化策略，通过平滑权重更新来防止Transformer训练中的谱能量集中问题，从而避免模型崩溃，无需学习率预热。


<details>
  <summary>Details</summary>
Motivation: 大规模Transformer训练中，学习率预热等技术是必需的，但缺乏理论支持。本文旨在揭示模型崩溃现象的理论原因，并提出解决方案。

Method: 基于Weyl不等式，提出一种优化策略，通过动态限制学习率来平滑权重更新，防止谱能量集中。

Result: 实验证明，该方法能有效稳定训练ViT、Swin-Transformer和GPT，无需学习率预热。
...
### [279] [From Dormant to Deleted: Tamper-Resistant Unlearning Through Weight-Space Regularization](https://arxiv.org/abs/2505.22310)
*Shoaib Ahmed Siddiqui,Adrian Weller,David Krueger,Gintare Karolina Dziugaite,Michael Curtis Mozer,Eleni Triantafillou*

Main category: cs.LG

TLDR: 论文发现LLM的遗忘方法易受重新学习攻击，即使微调少量无关样本也会导致遗忘知识重新出现。在视觉分类器中，仅用保留集微调即可使遗忘集准确率从50%恢复至近100%。


<details>
  <summary>Details</summary>
Motivation: 研究LLM遗忘方法的脆弱性，探索遗忘知识重新出现的现象及其原因。

Method: 在视觉分类器中控制实验，测试多种遗忘方法，分析权重空间特性（如L2距离和线性模式连接性）。

Result: 遗忘集准确率可通过保留集微调恢复至近100%，而从头训练的模型准确率保持50%。权重空间特性可预测抗重新学习攻击能力。

Conclusion: 提出一类新方法，基于权重空间特性实现抗重新学习攻击的最佳效果。
...
### [280] [A Closer Look at Multimodal Representation Collapse](https://arxiv.org/abs/2505.22483)
*Abhra Chaudhuri,Anjan Dutta,Tu Bui,Serban Georgescu*

Main category: cs.LG

TLDR: 论文研究了多模态融合中的模态崩溃现象，并提出了一种通过显式基重分配防止崩溃的算法。


<details>
  <summary>Details</summary>
Motivation: 模态崩溃是多模态融合中常见的问题，模型倾向于依赖部分模态而忽略其他模态，本文旨在深入理解其成因并提出解决方案。

Method: 通过理论分析发现模态崩溃源于噪声特征与预测特征的纠缠，提出利用跨模态知识蒸馏和显式基重分配的方法。

Result: 实验验证了理论分析的正确性，提出的算法有效防止了模态崩溃，并适用于处理缺失模态的情况。

Conclusion: 研究揭示了模态崩溃的机制，并提出了一种实用的解决方案，为多模态融合领域提供了新的理论基础和方法。
...
### [281] [Understanding Adversarial Training with Energy-based Models](https://arxiv.org/abs/2505.22486)
*Mujtaba Hussain Mirza,Maria Rosaria Briglia,Filippo Bartolucci,Senad Beadini,Giuseppe Lisanti,Iacopo Masi*

Main category: cs.LG

TLDR: 论文通过能量模型（EBM）框架分析对抗训练（AT）中的灾难性过拟合（CO）和鲁棒过拟合（RO），提出Delta能量正则化器（DER）缓解问题，并探讨鲁棒分类器的生成能力。


<details>
  <summary>Details</summary>
Motivation: 研究对抗训练中的过拟合现象及其能量动态，探索鲁棒分类器的生成潜力。

Method: 通过能量视角分析样本能量变化，提出DER正则化器，并改进生成技术（局部类PCA和能量引导）。

Result: DER有效缓解CO和RO，鲁棒分类器在生成任务中表现竞争性（IS和FID）。

Conclusion: 能量视角为对抗训练提供新见解，DER和生成改进技术具有实际应用潜力。
...
### [282] [Physics-inspired Generative AI models via real hardware-based noisy quantum diffusion](https://arxiv.org/abs/2505.22193)
*Marco Parigi,Stefano Martina,Francesco Aldo Venturelli,Filippo Caruso*

Main category: quant-ph

TLDR: 论文提出两种基于量子扩散模型（QDMs）的物理启发协议，旨在利用量子特性提升生成AI性能，并通过实验验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有量子扩散模型算法受限于近量子设备的可扩展性，因此探索利用量子动力学和噪声资源提升模型性能。

Method: 1. 使用量子随机游走形式主义，结合量子与经典动力学；2. 利用IBM量子硬件的固有噪声生成图像。

Result: 第一种方法生成MNIST图像的FID更低；第二种方法仅用4个量子比特实现图像生成，验证了量子噪声作为资源的潜力。

Conclusion: 研究为量子生成AI的大规模算法开辟了新方向，展示了量子噪声作为资源的可能性。
...
### [283] [RenderFormer: Transformer-based Neural Rendering of Triangle Meshes with Global Illumination](https://arxiv.org/abs/2505.21925)
*Chong Zeng,Yue Dong,Pieter Peers,Hongzhi Wu,Xin Tong*

Main category: cs.GR

TLDR: RenderFormer是一种神经渲染管线，直接从三角形场景表示渲染图像，支持全局光照效果，无需逐场景训练或微调。
...
Method: 采用两阶段管线：视图无关阶段建模三角形间光传输，视图相关阶段将光线束转换为像素值。两阶段均基于Transformer架构。

Result: 在形状和光传输复杂度不同的场景中进行了验证，展示了RenderFormer的有效性。

Conclusion: RenderFormer提供了一种无需物理模拟的高效渲染方法，适用于复杂场景。
...
### [284] [STDR: Spatio-Temporal Decoupling for Real-Time Dynamic Scene Rendering](https://arxiv.org/abs/2505.22400)
*Zehao Li,Hao Jiang,Yujun Cai,Jianing Chen,Baolong Bi,Shuqin Gao,Honglong Zhao,Yiwei Wang,Tianlu Mao,Zhaoqi Wang*

Main category: cs.GR

TLDR: 论文提出STDR模块，通过解耦时空概率分布提升动态场景重建质量。
...
Method: STDR模块引入时空掩码、分离变形场和一致性正则化，解耦时空模式。
...
### [285] [Neural Face Skinning for Mesh-agnostic Facial Expression Cloning](https://arxiv.org/abs/2505.22416)
*Sihun Cha,Serin Yoon,Kwanggyoon Seo,Junyong Noh*

Main category: cs.GR

TLDR: 提出了一种结合全局和局部变形模型的方法，用于面部动画重定向，实现直观控制和细节表达克隆。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法在面部动画重定向中难以同时捕捉全局和局部细节，且局部变形方法复杂化整体控制。

Method: 通过预测目标面部网格的顶点权重，将全局潜在代码的影响局部化，并结合FACS混合形状监督潜在代码。

Result: 实验表明，该方法在表达保真度、变形传递准确性和适应性上优于现有方法。
...
### [286] [High-Fidelity Functional Ultrasound Reconstruction via A Visual Auto-Regressive Framework](https://arxiv.org/abs/2505.21530)
*Xuhang Chen,Zhuo Li,Yanyan Shen,Mufti Mahmud,Hieu Pham,Chi-Man Pun,Shuqiang Wang*

Main category: eess.IV

TLDR: fUS成像在神经血管映射中具有高时空分辨率，但数据稀缺和信号衰减限制了其实际应用。


<details>
  <summary>Details</summary>
Motivation: 解决fUS成像在神经血管映射中因数据稀缺和信号衰减导致的实际应用受限问题。

Method: 未明确提及具体方法，但提到数据稀缺和信号衰减是主要挑战。

Result: 数据稀缺和信号衰减限制了数据集多样性，并影响下游机器学习模型的公平性。

Conclusion: fUS成像的实际应用面临数据稀缺和信号衰减的挑战，需进一步解决。
...
### [287] [Image denoising as a conditional expectation](https://arxiv.org/abs/2505.21546)
*Sajal Chakroborty,Suddhasattwa Das*

Main category: eess.IV

TLDR: 论文提出了一种基于概率空间解释的数据驱动去噪方法，将真实图像恢复为条件期望，并通过核积分算子估计未知概率空间上的积分。


<details>
  <summary>Details</summary>
Motivation: 传统去噪方法通过投影到子空间估计真实图像，但这种方法在概率空间解释下可能不保证无偏和收敛。

Method: 将真实图像重新表述为再生核希尔伯特空间（RKHS）中线性方程的最小二乘解，利用核积分算子作为线性变换。

Result: 假设真实图像是紧致平面域上的连续函数，该方法在像素数趋于无穷时收敛，并可优化有限像素图像的去噪参数。

Conclusion: 该方法在理论和实际应用中均表现出良好的收敛性和去噪效果。
...
### [288] [Taylor expansion-based Kolmogorov-Arnold network for blind image quality assessment](https://arxiv.org/abs/2505.21592)
*Ze Chen,Shaode Yu*

Main category: eess.IV

TLDR: TaylorKAN通过泰勒展开作为可学习激活函数，提升了局部逼近能力，并在高维特征处理中表现优于其他KAN相关模型。


<details>
  <summary>Details</summary>
Motivation: KAN及其变体在高维特征处理中性能提升有限且计算成本高，需要改进。

Method: 提出TaylorKAN，利用泰勒展开作为激活函数，并整合网络深度减少和特征维度压缩以提高效率。
...
### [289] [Optimizing Deep Learning for Skin Cancer Classification: A Computationally Efficient CNN with Minimal Accuracy Trade-Off](https://arxiv.org/abs/2505.21597)
*Abdullah Al Mamun,Pollob Chandra Ray,Md Rahat Ul Nasib,Akash Das,Jia Uddin,Md Nurul Absur*

Main category: eess.IV

TLDR: 提出一种轻量级CNN模型，显著减少参数和计算量，同时保持高分类精度，适用于资源受限环境。


<details>
  <summary>Details</summary>
Motivation: 现有深度模型（如ResNet50）计算开销大，难以在资源受限环境中部署。

Method: 设计自定义CNN模型，大幅减少参数和FLOPs。

Result: 模型参数减少96.7%，FLOPs降至30.04百万，精度偏差小于0.022%。

Conclusion: 轻量级CNN在资源受限环境下更实用，平衡了模型复杂度和实际可行性。
...
### [290] [Laparoscopic Image Desmoking Using the U-Net with New Loss Function and Integrated Differentiable Wiener Filter](https://arxiv.org/abs/2505.21634)
*Chengyu Yang,Chengjun Liu*

Main category: eess.IV

TLDR: 论文提出了一种结合U-Net深度学习和可微维纳滤波器的ULW方法，用于去除腹腔镜手术中的烟雾，提升图像质量。


<details>
  <summary>Details</summary>
Motivation: 腹腔镜手术中烟雾导致视觉模糊，影响手术和计算机辅助技术，需有效去除烟雾的方法。

Method: 提出ULW方法，结合新损失函数（结构相似性、感知损失和均方误差）和可微维纳滤波器，优化图像重建。

Result: 在公开数据集上验证，ULW方法在视觉清晰度和量化指标上表现优异。
...
### [291] [STA-Risk: A Deep Dive of Spatio-Temporal Asymmetries for Breast Cancer Risk Prediction](https://arxiv.org/abs/2505.21699)
*Zhengbo Zhou,Dooman Arefan,Margarita Zuley,Jules Sumkin,Shandong Wu*

Main category: eess.IV

TLDR: 提出了一种基于Transformer的STA-Risk模型，通过空间和时间不对称性预测乳腺癌风险，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有乳腺癌风险预测模型性能有限，且忽视纵向影像中的细微组织变化。

Method: STA-Risk模型结合侧编码和时间编码，学习空间-时间不对称性，并使用定制的不对称损失函数。

Result: 在两个独立数据集上，STA-Risk在1-5年风险预测中优于四种SOTA模型。

Conclusion: STA-Risk通过捕捉空间和时间不对称性，显著提升了乳腺癌风险预测性能。
...
### [292] [Privacy-Preserving Chest X-ray Report Generation via Multimodal Federated Learning with ViT and GPT-2](https://arxiv.org/abs/2505.21715)
*Md. Zahid Hossain,Mustofa Ahmed,Most. Sharmin Sultana Samu,Md. Rakibul Islam*

Main category: eess.IV

TLDR: 论文提出了一种基于多模态联邦学习的胸片报告生成框架，通过分散训练保护数据隐私，评估了三种聚合策略，其中Krum Aggregation表现最佳。


<details>
  <summary>Details</summary>
Motivation: 传统集中式方法需要传输敏感数据，存在隐私问题，因此研究提出联邦学习框架以保护数据隐私。

Method: 使用Vision Transformer (ViT)作为编码器，GPT-2作为报告生成器，评估了FedAvg、Krum Aggregation和L-FedAvg三种联邦学习聚合策略。

Result: Krum Aggregation在ROUGE、BLEU、BERTScore和RaTEScore等指标上表现最优，联邦学习模型在生成临床相关且语义丰富的报告方面可媲美或超越集中式模型。
...
### [293] [MAMBO-NET: Multi-Causal Aware Modeling Backdoor-Intervention Optimization for Medical Image Segmentation Network](https://arxiv.org/abs/2505.21874)
*Ruiguo Yu,Yiyang Zhang,Yuan Tian,Yujie Diao,Di Jin,Witold Pedrycz*

Main category: eess.IV

TLDR: 提出了一种多因果感知建模后门干预优化网络（MAMBO-NET），用于解决医学图像分割中混淆因素的影响，显著提高了分割准确性。
...
Result: 在五个医学图像数据集上的实验表明，该方法显著降低了混淆因素的影响，提升了分割精度。

Conclusion: MAMBO-NET通过因果干预有效解决了医学图像分割中的混淆问题，为相关任务提供了新思路。
...
### [294] [Subspecialty-Specific Foundation Model for Intelligent Gastrointestinal Pathology](https://arxiv.org/abs/2505.21928)
...
TLDR: Digepath是一种专为胃肠道病理学设计的基础模型，通过双阶段迭代优化策略提升诊断准确性，尤其在稀疏分布的病变区域检测中表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统组织病理学诊断依赖病理学家的主观判断，存在可重复性低和诊断变异性的问题，亟需更精确的诊断方法。

Method: 开发Digepath模型，采用双阶段迭代优化策略（预训练与精细筛查结合），基于超过3.53亿个图像块和20万张胃肠道疾病切片进行训练。

Result: 在34项任务中，33项达到最先进水平，包括病理诊断、分子预测等，并在早期癌症筛查中实现99.6%的灵敏度。

Conclusion: Digepath填补了组织病理学实践的关键空白，为胃肠道疾病及其他病理亚专科提供了可转移的AI驱动精准病理学范式。
...
### [295] [Cascaded 3D Diffusion Models for Whole-body 3D 18-F FDG PET/CT synthesis from Demographics](https://arxiv.org/abs/2505.22489)
*Siyeop Yoon,Sifan Song,Pengfei Jin,Matthew Tivnan,Yujin Oh,Sekeun Kim,Dufan Wu,Xiang Li,Quanzheng Li*

Main category: eess.IV

TLDR: 提出了一种级联3D扩散模型框架，直接从人口统计学变量合成高保真3D PET/CT图像，用于肿瘤成像、虚拟试验和AI驱动的数据增强。
...
Method: 采用两阶段生成过程：首先用基于分数的扩散模型生成低分辨率图像，再用超分辨率残差扩散模型提升分辨率。

Result: 合成图像与真实数据在器官体积和SUV分布上高度一致，代谢摄取值偏差在3-5%以内。

Conclusion: 级联3D扩散模型为临床和研究提供了一种可扩展且准确的合成图像生成方法。
...
### [296] [Risk-Sensitive Conformal Prediction for Catheter Placement Detection in Chest X-rays](https://arxiv.org/abs/2505.22496)
*Long Hui*

Main category: eess.IV

TLDR: 提出了一种结合多任务学习和风险敏感共形预测的新方法，用于胸部X光片中导管和管线位置的检测，显著提升了临床可靠性。
...
Result: 实验显示总体覆盖率为90.68%，关键条件下覆盖率为99.29%，且无高风险误判。

Conclusion: 该方法在准确性和不确定性量化方面表现优异，适合临床部署。
...
### [297] [Surf2CT: Cascaded 3D Flow Matching Models for Torso 3D CT Synthesis from Skin Surface](https://arxiv.org/abs/2505.22511)
*Siyeop Yoon,Yujin Oh,Pengfei Jin,Sifan Song,Matthew Tivnan,Dufan Wu,Xiang Li,Quanzheng Li*

Main category: eess.IV

TLDR: Surf2CT是一种新颖的级联流匹配框架，能够仅通过外部体表扫描和简单人口统计数据生成完整的人体躯干3D CT图像。
...
Method: Surf2CT分为三个阶段：1) 表面补全，通过条件3D流匹配重建完整的SDF；2) 粗CT合成，从SDF和人口统计数据生成低分辨率CT；3) CT超分辨率，通过条件流模型提升分辨率。

Result: 在700组测试数据中，器官体积误差小（-11.1%至4.4%），肌肉/脂肪组成与真实数据强相关（0.67至0.96），肺定位偏差小（-2.5 mm）。
...
### [298] [Multipath cycleGAN for harmonization of paired and unpaired low-dose lung computed tomography reconstruction kernels](https://arxiv.org/abs/2505.22568)
...
TLDR: 提出一种多路径cycleGAN模型，用于CT核谐波化，以减少定量成像中的系统性变异性，并保持解剖学一致性。


<details>
  <summary>Details</summary>
Motivation: CT重建核影响空间分辨率和噪声特性，导致定量成像测量（如肺气肿量化）的系统性变异性。选择合适核对一致性分析至关重要。

Method: 使用多路径cycleGAN模型，结合配对和非配对数据训练，共享潜在空间，并为每个域定制判别器。在NLST数据集上评估性能。

Result: 模型减少了肺气肿评分的偏差（配对核p<0.05），消除了非配对核的混淆差异（p>0.05），并保持了肌肉和脂肪解剖学的高Dice分数。

Conclusion: 共享潜在空间的多路径cycleGAN能稳健地实现CT核谐波化，改善肺气肿量化并保持解剖学保真度。
...
### [299] [Comparative Analysis of Machine Learning Models for Lung Cancer Mutation Detection and Staging Using 3D CT Scans](https://arxiv.org/abs/2505.22592)
*Yiheng Li,Francisco Carrillo-Perez,Mohammed Alawad,Olivier Gevaert*

Main category: eess.IV

TLDR: 比较两种机器学习模型（FMCIB+XGBoost和Dinov2+ABMIL）在肺癌突变检测和分期任务中的表现，前者在突变检测中表现更优，后者在分期任务中展现潜力。


<details>
  <summary>Details</summary>
Motivation: 肺癌是全球癌症死亡的主要原因，非侵入性方法对检测关键突变和分期至关重要。

Method: 使用FMCIB+XGBoost（监督学习）和Dinov2+ABMIL（自监督学习）模型分析3D肺结节数据。

Result: FMCIB+XGBoost在KRAS和EGFR突变检测中表现更优（准确率0.846和0.883），Dinov2+ABMIL在分期任务中表现良好（准确率0.797）。

Conclusion: 监督学习在突变检测中更具临床价值，自监督学习在分期任务中潜力显著，但突变敏感性有待提升。
...
### [300] [Chest Disease Detection In X-Ray Images Using Deep Learning Classification Method](https://arxiv.org/abs/2505.22609)
*Alanna Hazlett,Naomi Ohashi,Timothy Rodriguez,Sodiq Adewole*

Main category: eess.IV

TLDR: 研究通过迁移学习技术，利用预训练的CNN模型对胸部X光图像进行分类，涵盖COVID-19、肺炎、结核病和正常病例，结果表现出高准确率和关键分类指标优异。


<details>
  <summary>Details</summary>
Motivation: 探索多分类模型在胸部X光图像分类中的性能，特别是针对COVID-19、肺炎、结核病和正常病例的诊断。

Method: 采用迁移学习技术，利用预训练的CNN模型进行微调，并结合Grad-CAM提高模型可解释性。

Result: 模型表现出高准确率和优异的分类指标（如精确率、召回率和F1分数）。

Conclusion: 该方法在临床应用中具有潜力，通过Grad-CAM提高了模型的透明度和可信度。
...
### [301] [From prosthetic memory to prosthetic denial: Auditing whether large language models are prone to mass atrocity denialism](https://arxiv.org/abs/2505.21753)
*Roberto Ulloa,Eve M. Zucker,Daniel Bultmann,David J. Simon,Mykola Makhortykh*

Main category: cs.CY

TLDR: 研究探讨大型语言模型（LLM）如何影响历史叙事传播，分析其对集体暴行记忆的塑造作用，揭示LLM可能引发记忆失真或否认的风险。


<details>
  <summary>Details</summary>
Motivation: 探究LLM在历史记忆传播中的双重作用：既是记忆传递的媒介，也可能助长历史否认主义。

Method: 对五种LLM（Claude、GPT、Llama、Mixtral和Gemini）进行对比审计，测试其在四种历史案例（如大屠杀）中对否认主义言论的回应。

Result: LLM对广泛记录的事件（如大屠杀）回应准确，但对较少被记录的事件（如柬埔寨种族灭绝）易受否认主义框架影响。

Conclusion: LLM虽扩展了“假体记忆”概念，但未受监管的使用可能强化历史否认主义，对数字记忆保存提出伦理挑战。
...
### [302] [Detecting Cultural Differences in News Video Thumbnails via Computational Aesthetics](https://arxiv.org/abs/2505.21912)
*Marvin Limpijankit,John Kender*

Main category: cs.CY

TLDR: 论文提出了一种两步法，通过聚类和比较美学特征来检测不同文化背景下图像风格的差异，并以中美YouTube缩略图为例验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 研究不同文化背景下图像风格的差异，特别是中美YouTube视频缩略图的美学特征差异，以揭示文化偏好并作为检测视觉宣传的基准。

Method: 采用两步法：首先基于内容将图像聚类为视觉主题，然后比较美学特征。测试数据为2400张来自中美各两个YouTube频道的缩略图，内容涉及COVID-19和乌克兰冲突。

Result: 结果显示，中国缩略图更随意和真实，而美国缩略图更正式和精心设计。具体表现为美国缩略图色彩较少、饱和度更高、更暗、细节更精细、对称性更低、更稀疏、变化较少且更近距离。

Conclusion: 这些差异反映了文化偏好，研究方法可作为检测视觉宣传的基准。
...
### [303] [Incorporating LLMs for Large-Scale Urban Complex Mobility Simulation](https://arxiv.org/abs/2505.21880)
*Yu-Lun Song,Chung-En Tsern,Che-Cheng Wu,Yu-Ming Chang,Syuan-Bo Huang,Wei-Chu Chen,Michael Chia-Liang Lin,Yu-Ta Lin*

Main category: cs.MA

TLDR: 该研究提出了一种结合大型语言模型（LLM）与基于代理建模（ABM）的创新城市交通模拟方法，通过生成合成人口数据和行为模拟提升模型多样性和真实性。


<details>
  <summary>Details</summary>
Motivation: 传统基于规则的ABM方法在模拟城市交通时缺乏多样性和真实性，因此研究提出利用LLM增强ABM以解决这一问题。

Method: 通过LLM生成合成人口数据、分配日常和偶尔活动地点，并模拟个性化路线，结合台北市的真实数据建模个体行为和大规模交通模式。

Result: 模拟结果提供了路线热图和交通方式指标等关键洞察，为城市规划者提供了决策支持。

Conclusion: 未来工作将集中于建立验证框架以确保模拟结果的准确性和可靠性。
...
### [304] [Synonymous Variational Inference for Perceptual Image Compression](https://arxiv.org/abs/2505.22438)
*Zijian Liang,Kai Niu,Changshuo Wang,Jin Xu,Ping Zhang*

Main category: cs.IT

TLDR: 提出了一种基于同义关系的变分推理方法（SVI），用于重新分析感知图像压缩问题，并引入了一种新的图像压缩方案（SIC）。


<details>
  <summary>Details</summary>
Motivation: 揭示语义与句法信息的集合-元素关系，并通过同义性视角重新分析感知图像压缩问题。

Method: 基于同义性标准构建理想同义集（Synset），通过最小化部分语义KL散度近似其后验表示，并提出SIC方案。

Result: 实验证明SIC编解码器在率失真感知性能上表现优异，验证了分析方法的有效性。

Conclusion: SVI和SIC方法为感知图像压缩提供了新的理论框架和实用方案。
...
### [305] [UI-Evol: Automatic Knowledge Evolving for Computer Use Agents](https://arxiv.org/abs/2505.21964)
*Ziyun Zhang,Xinyi Liu,Xiaoyi Zhang,Jun Wang,Gang Chen,Yan Lu*

Main category: cs.HC

TLDR: 论文提出UI-Evol模块，通过两阶段设计（Retrace和Critique）解决知识执行差距，显著提升任务性能和代理可靠性。
...
Method: 提出UI-Evol模块，包含Retrace（提取动作序列）和Critique（对比外部参考优化知识）两阶段。

Result: 在OSWorld基准测试中，UI-Evol显著提升任务性能并降低行为标准差。

Conclusion: UI-Evol有效解决知识执行差距，提升计算机使用代理的可靠性和任务表现。
...
### [306] [MapStory: LLM-Powered Text-Driven Map Animation Prototyping with Human-in-the-Loop Editing](https://arxiv.org/abs/2505.21966)
*Aditya Gunturu,Ben Pearman,Keiichi Ihara,Morteza Faraji,Bryan Wang,Rubaiat Habib Kazi,Ryo Suzuki*

Main category: cs.HC

TLDR: MapStory是一个基于LLM的地图动画创作工具，通过自然语言文本生成可编辑的地图动画序列。


<details>
  <summary>Details</summary>
Motivation: 旨在简化地图动画创作流程，降低专业门槛，支持用户快速迭代和创意探索。

Method: 利用代理架构分解脚本为动画构建块，结合LLM和网络搜索获取地理信息，提供交互式时间线编辑器。

Result: 专家访谈和可用性研究表明，MapStory能轻松创建地图动画，加快迭代速度，激发创意。

Conclusion: MapStory有效降低了地图故事创作的门槛，提升了创作效率和灵活性。
...
### [307] [Capability-Based Scaling Laws for LLM Red-Teaming](https://arxiv.org/abs/2505.20162)
*Alexander Panfilov,Paul Kassianik,Maksym Andriushchenko,Jonas Geiping*

Main category: cs.AI

TLDR: 研究探讨了大型语言模型在能力差距下的红队攻击有效性，发现攻击成功率与攻击者和目标模型的能力差距密切相关。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型能力和自主性的提升，红队测试成为确保安全部署的重要手段，但传统方法在能力差距较大时可能失效。

Method: 通过评估500多个攻击者-目标模型对，模拟人类红队攻击，分析能力差距对攻击成功率的影响。

Result: 发现三个趋势：能力更强的模型攻击更有效；目标能力超过攻击者时成功率骤降；攻击成功率与MMLU-Pro基准的社会科学表现相关。

Conclusion: 未来固定能力的攻击者（如人类）可能失效，开源模型的风险增加，需精确评估和控制模型的操控能力。
...
### [308] [R1-Code-Interpreter: Training LLMs to Reason with Code via Supervised and Reinforcement Learning](https://arxiv.org/abs/2505.21668)
*Yongchao Chen,Yueying Liu,Junwei Zhou,Yilun Hao,Jingquan Wang,Yang Zhang,Chuchu Fan*

Main category: cs.AI

TLDR: R1-Code-Interpreter通过多轮监督微调和强化学习，使LLM能自主生成代码查询，提升精确计算和符号操作任务的表现。


<details>
  <summary>Details</summary>
Motivation: 解决LLM在精确计算和符号操作任务中的不足，研究如何有效结合文本推理和代码生成。

Method: 采用多轮监督微调（SFT）和强化学习（RL）训练Qwen-2.5模型，探索不同策略和格式。

Result: R1-CI-14B模型在测试任务上准确率从44.0%提升至64.1%，接近GPT-4o的表现。
...
### [309] [Towards Safety Reasoning in LLMs: AI-agentic Deliberation for Policy-embedded CoT Data Creation](https://arxiv.org/abs/2505.21784)
...
TLDR: AIDSAFE提出了一种多智能体审议方法，通过迭代扩展安全策略的推理生成高质量CoT数据集，显著提升了LLMs的安全性和抗越狱能力。


<details>
  <summary>Details</summary>
Motivation: 现有安全措施存在过度拒绝和越狱漏洞问题，需要一种资源高效的方法生成高质量政策嵌入的CoT数据集。

Method: AIDSAFE利用多智能体审议迭代扩展安全策略推理，并通过数据精炼阶段去除冗余和欺骗性思维。此外，还引入信念增强生成偏好数据。

Result: AIDSAFE生成的CoT在政策遵循和推理质量上表现优异，显著提升了LLMs的安全泛化和抗越狱能力。

Conclusion: AIDSAFE为LLMs的安全训练提供了高效的数据生成方法，显著提升了模型的安全性和实用性。
...
### [310] [Modeling and Optimizing User Preferences in AI Copilots: A Comprehensive Survey and Taxonomy](https://arxiv.org/abs/2505.21907)
*Saleh Afzoon,Zahra Jahanandish,Phuong Thao Huynh,Amin Beheshti,Usman Naseem*

Main category: cs.AI

TLDR: 本文综述了AI副驾驶中偏好优化的研究，提出了一种基于交互阶段的分类法，并分析了偏好信号获取、用户意图建模和反馈整合的技术。


<details>
  <summary>Details</summary>
Motivation: 随着AI副驾驶能力的提升和普及，个性化成为确保其可用性、信任和生产力的关键，但现有研究在实时交互系统中的适应性仍不完善。

Method: 通过整合AI个性化、人机协作和大型语言模型适应的研究，提出了一个分阶段的偏好优化分类法，并分析了相关技术。

Result: 提出了统一的AI副驾驶定义和偏好优化策略分类法，总结了偏好信号获取和用户意图建模的技术。

Conclusion: 本文为设计自适应、偏好感知的AI副驾驶提供了结构化基础，并指出了未来研究方向。
...
### [311] [Rethinking the Unsolvable: When In-Context Search Meets Test-Time Scaling](https://arxiv.org/abs/2505.22290)
*Fanzeng Xia,Yidong Luo,Tinko Sebastian Bartels,Yaqi Xu,Tongxin Li*

Main category: cs.AI

TLDR: 通过结合上下文搜索提示和内部扩展，LLMs在复杂推理任务中取得突破性进展，挑战了现有评估范式对其能力的低估。


<details>
  <summary>Details</summary>
Motivation: 现有研究通过简单上下文学习评估LLMs的推理能力，忽视了高级技术，导致低估其潜力。本文旨在探索LLMs在超难任务中的真实能力。

Method: 采用上下文搜索提示和内部扩展相结合的方法，系统评估其在NP难任务和复杂规划基准上的表现。

Result: 实验显示成功率提升30倍，理论分析表明该方法显著扩展了可解问题的复杂度类别。

Conclusion: 研究挑战了LLMs在复杂任务中的限制假设，呼吁重新评估其推理能力，并提出更全面的评估策略。
...
### [312] [Efficiently Enhancing General Agents With Hierarchical-categorical Memory](https://arxiv.org/abs/2505.22006)
*Changze Qiao,Mingming Lu*

Main category: cs.AI

TLDR: 论文提出了一种无需参数更新的通用智能体EHC，通过分层记忆检索和任务导向经验学习模块，实现了高效学习和适应新环境的能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖计算成本高的端到端训练或缺乏持续学习能力的工具使用，限制了多模态智能体的通用性和适应性。

Method: EHC包含分层记忆检索（HMR）模块和任务类别导向经验学习（TOEL）模块，分别实现快速记忆检索和任务特征分类学习。

Result: 在多个标准数据集上的实验表明，EHC优于现有方法，达到最先进性能。

Conclusion: EHC作为一种通用智能体，能有效处理复杂多模态任务，展示了无需参数更新的持续学习潜力。
...
### [313] [Large-Area Fabrication-aware Computational Diffractive Optics](https://arxiv.org/abs/2505.22313)
*Kaixuan Wei,Hector A. Jimenez-Romero,Hadi Amata,Jipeng Sun,Qiang Fu,Felix Heide,Wolfgang Heidrich*

Main category: physics.optics

TLDR: 论文提出了一种制造感知的设计流程，用于通过直接写入灰度光刻和纳米压印复制制造的衍射光学元件，解决了模拟与制造设备之间的质量差距问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法多限于实验室原型，模拟与制造设备之间存在较大质量差距，阻碍了学习衍射光学系统的实际应用。

Method: 提出制造感知的设计流程，包括超分辨率神经光刻模型和分布式GPU计算框架，用于大规模衍射光学设计。

Result: 实现了模拟与制造原型之间的良好一致性，并在仅使用单个衍射光学元件的成像系统中获得高质量图像。
...
### [314] [Higher-Order Group Synchronization](https://arxiv.org/abs/2505.21932)
*Adriana L. Duncan,Joe Kileel*

Main category: stat.ML

TLDR: 本文提出了一种新颖的高阶群同步问题，基于超图处理高阶局部测量以获取全局估计，并提供了首个计算框架及其理论保证。


<details>
  <summary>Details</summary>
Motivation: 高阶群同步问题在计算机视觉和图像处理等领域有广泛应用，旨在解决传统群同步方法无法处理的高阶测量问题。

Method: 定义了高阶群同步问题及其数学基础，提出了基于消息传递算法的全局计算框架，并分析了其在噪声和异常值下的收敛性。

Result: 实验表明，该方法在旋转和角度同步任务中优于传统成对同步方法，且在模拟冷冻电镜数据中表现与标准工具相当。

Conclusion: 高阶群同步方法在处理高阶测量时具有优势，尤其在鲁棒性和性能上优于传统方法，为相关领域提供了新工具。
...
### [315] [Mitigating Audiovisual Mismatch in Visual-Guide Audio Captioning](https://arxiv.org/abs/2505.22045)
*Le Xu,Chenxing Li,Yong Ren,Yujie Chen,Yu Gu,Ruibo Fu,Shan Yang,Dong Yu*

Main category: cs.MM

TLDR: 提出了一种熵感知门控融合框架，通过跨模态不确定性量化动态调节视觉信息流，解决视听不对齐问题。


<details>
  <summary>Details</summary>
Motivation: 现有视觉引导音频字幕系统在真实场景（如配音或画外音）中常因视听不对齐而失效。

Method: 采用注意力熵分析识别并抑制误导性视觉线索，结合批量视听混洗技术生成合成不匹配训练对。

Result: 在AudioCaps基准测试中表现优于现有基线，尤其在不对齐场景下，推理速度提升约6倍。

Conclusion: 该框架有效提升模型对视听不对齐的鲁棒性和效率。
...
### [316] [Test-Time Immunization: A Universal Defense Framework Against Jailbreaks for (Multimodal) Large Language Models](https://arxiv.org/abs/2505.22271)
*Yongcan Yu,Yanbo Wang,Ran He,Jian Liang*

Main category: cs.CR

TLDR: 论文提出了一种通用防御框架TIM，用于自适应防御多种越狱攻击，通过训练检测令牌和安全微调实现高效防御。


<details>
  <summary>Details</summary>
Motivation: 现有防御方法通常针对特定类型的越狱攻击，无法应对多样化的对抗策略，因此需要一种通用且自适应的防御框架。

Method: TIM框架通过训练检测令牌识别越狱行为，并在检测到攻击时进行安全微调，同时解耦检测模块以避免性能下降。

Result: 实验证明TIM在LLMs和多模态LLMs上均能有效防御多种越狱攻击。

Conclusion: TIM是一种高效且通用的防御框架，能够自适应地应对多样化的越狱攻击策略。
...
### [317] [VideoMarkBench: Benchmarking Robustness of Video Watermarking](https://arxiv.org/abs/2505.21620)
*Zhengyuan Jiang,Moyang Guo,Kecen Li,Yuepeng Hu,Yupu Wang,Zhicong Huang,Cheng Hong,Neil Zhenqiang Gong*

Main category: cs.CR

TLDR: 论文提出了VideoMarkBench，首个系统性评估视频水印在去除和伪造攻击下鲁棒性的基准测试，揭示了当前方法的显著脆弱性。
...
Method: 研究引入了VideoMarkBench，使用三种最先进的视频生成模型生成统一数据集，涵盖三种视频风格、四种水印方法和七种检测聚合策略，评估了12种扰动类型在三种威胁模型下的表现。

Result: 研究发现当前水印方法存在显著脆弱性，亟需更鲁棒的解决方案。

Conclusion: 论文强调了视频水印鲁棒性研究的重要性，并提供了开源代码以促进未来研究。
...
### [318] [Visual Cues Support Robust Turn-taking Prediction in Noise](https://arxiv.org/abs/2505.22088)
*Sam O'Connor Russell,Naomi Harte*

Main category: cs.SD

TLDR: 研究探讨了预测性轮流模型（PTTMs）在噪声环境中的表现，发现其对噪声高度敏感，但通过多模态训练可提升性能。


<details>
  <summary>Details</summary>
Motivation: 为提升人机交互的自然性，需了解PTTMs在噪声中的表现。

Method: 分析PTTMs在不同噪声下的表现，并开发多模态模型以利用视觉线索。

Result: 多模态PTTM在噪声中表现优于纯音频模型，但对新噪声类型泛化能力有限。

Conclusion: 多模态训练可提升PTTMs在噪声中的鲁棒性，但需依赖准确转录且泛化能力待改进。
...
### [319] [Advancing Hearing Assessment: An ASR-Based Frequency-Specific Speech Test for Diagnosing Presbycusis](https://arxiv.org/abs/2505.22231)
*Stefan Bleeck*

Main category: cs.SD

TLDR: 本文提出了一种基于自动语音识别（ASR）的频率特异性语音测试方法，用于更精细地诊断听力损失对语音理解的影响，特别是在老年性耳聋中。


<details>
  <summary>Details</summary>
Motivation: 传统听力测试难以全面评估听力损失对语音理解的功能性影响，尤其是阈上缺陷和频率特异性感知问题。

Method: 利用ASR模拟中度倾斜听力损失的感知效果，通过控制声学退化处理语音刺激，并分析音素级混淆模式。

Result: 模拟听力损失导致特定音素混淆（如高频辅音替换）和音素删除，与老年性耳聋的声学线索退化一致。测试电池显示出诊断价值，能有效区分模拟正常听力与听力受损者。

Conclusion: ASR驱动的方法为开发客观、精细和频率特异性的听力评估工具提供了新途径，未来将验证人类参与者并探索高级AI模型的集成。
...
### [320] [Effective Context in Neural Speech Models](https://arxiv.org/abs/2505.22487)
*Yen Meng,Sharon Goldwater,Hao Tang*

Main category: cs.SD

TLDR: 论文提出两种测量语音Transformer有效上下文的方法，并分析了监督和自监督模型的表现。


<details>
  <summary>Details</summary>
Motivation: 研究语音模型实际使用的上下文长度（有效上下文），以优化模型性能。

Method: 提出两种测量有效上下文的方法，并应用于不同语音Transformer模型。

Result: 监督模型的有效上下文与任务性质相关，自监督模型的有效上下文较短且集中在早期层。

Conclusion: HuBERT等模型可在流式模式下运行，无需架构修改或微调。
...
### [321] [RESOUND: Speech Reconstruction from Silent Videos via Acoustic-Semantic Decomposed Modeling](https://arxiv.org/abs/2505.22024)
*Long-Khanh Pham,Thanh V. T. Tran,Minh-Tan Pham,Van Nguyen*

Main category: cs.SD

TLDR: RESOUND是一种新的唇语转语音（L2S）系统，通过分离声学和语义路径，结合语音单元和梅尔频谱图，生成清晰且富有表现力的语音。


<details>
  <summary>Details</summary>
Motivation: 现有L2S系统在准确性和自然度上存在挑战，主要由于对语言内容、口音和韵律的监督有限。

Method: RESOUND采用源-滤波器理论，分为声学路径（预测韵律）和语义路径（提取语言特征），并结合语音单元和梅尔频谱图生成波形。

Result: 在两个标准L2S基准测试中，RESOUND在多项指标上表现优异。

Conclusion: RESOUND通过分离和优化声学与语义表示，有效提升了L2S合成的性能。
...
### [322] [Learning Compositional Behaviors from Demonstration and Language](https://arxiv.org/abs/2505.21981)
*Weiyu Liu,Neil Nie,Ruohan Zhang,Jiayuan Mao,Jiajun Wu*

Main category: cs.RO

TLDR: BLADE是一个结合模仿学习和基于模型的规划的框架，用于长时程机器人操作，利用语言标注演示和大型语言模型提取抽象动作知识。


<details>
  <summary>Details</summary>
Motivation: 解决长时程机器人操作中动作规划和泛化能力的挑战。

Method: 通过语言标注演示提取动作知识，构建结构化高层动作表示库，包括视觉感知的预条件和效果，并实现神经网络策略。

Result: BLADE能自动恢复结构化表示，无需手动标注，显著提升对新情境的泛化能力。

Conclusion: BLADE在模拟和真实机器人实验中验证了其有效性，适用于复杂物体和约束场景。
...
### [323] [CogAD: Cognitive-Hierarchy Guided End-to-End Autonomous Driving](https://arxiv.org/abs/2505.21581)
*Zhennan Wang,Jianing Teng,Canqun Xiang,Kangliang Chen,Xing Pan,Lu Deng,Weihao Gu*

Main category: cs.RO

TLDR: CogAD是一种新型端到端自动驾驶模型，模仿人类驾驶员的层次认知机制，在感知和规划方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有端到端自动驾驶方法与人类认知原则不一致，CogAD旨在通过模仿人类认知机制解决这一问题。

Method: CogAD采用双重层次机制：全局到局部上下文处理（感知）和意图条件多模式轨迹生成（规划）。

Result: 在nuScenes和Bench2Drive数据集上，CogAD在端到端规划中表现最优，尤其在长尾场景和复杂现实条件下。

Conclusion: CogAD通过层次感知和多级规划，实现了更全面、鲁棒的自动驾驶性能。
...
### [324] [Vision-Language-Action Model with Open-World Embodied Reasoning from Pretrained Knowledge](https://arxiv.org/abs/2505.21906)
*Zhongyi Zhou,Yichen Zhu,Junjie Wen,Chaomin Shen,Yi Xu*

Main category: cs.RO

TLDR: ChatVLA-2是一种新型的混合专家VLA模型，通过三阶段训练流程保留VLM的核心能力，并在机器人任务中展现出卓越的数学推理和空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有端到端VLA系统在微调时会丢失VLM的核心能力，因此需要一种既能继承VLM知识又能实现可操作推理的通用VLA模型。

Method: 采用混合专家模型和三阶段训练流程，结合数学匹配任务验证模型能力。

Result: 模型展现出未明确训练的数学推理、OCR和空间推理能力，性能超越现有模仿学习方法。
...
### [325] [ForceVLA: Enhancing VLA Models with a Force-aware MoE for Contact-rich Manipulation](https://arxiv.org/abs/2505.22159)
...
TLDR: ForceVLA提出了一种新的端到端机器人操作框架，通过将力反馈作为VLA系统的第一模态，显著提升了接触密集型任务的表现。
...
Method: ForceVLA引入了FVLMoE模块，动态整合预训练的视觉-语言嵌入与实时6轴力反馈，并通过ForceVLA-Data数据集支持。
...
### [326] [LiDAR Based Semantic Perception for Forklifts in Outdoor Environments](https://arxiv.org/abs/2505.22258)
*Benjamin Serfling,Hannes Reichert,Lorenzo Bayerlein,Konrad Doll,Kati Radkhah-Lens*

Main category: cs.RO

TLDR: 提出了一种基于双LiDAR的语义分割框架，专为复杂户外环境中的自动叉车设计，提高了动态和静态障碍物的检测精度。


<details>
  <summary>Details</summary>
Motivation: 为工业物料搬运任务提供全面的场景理解，满足安全感知的自主叉车导航需求。

Method: 集成双LiDAR系统（前向和向下倾斜），利用高分辨率3D点云进行轻量级语义分割。

Result: 实验验证显示高分割精度，满足实时性要求，适用于动态仓库和场地环境。

Conclusion: 该框架在复杂户外环境中表现出色，适合安全感知的自主叉车导航。
...
### [327] [UP-SLAM: Adaptively Structured Gaussian SLAM with Uncertainty Prediction in Dynamic Environments](https://arxiv.org/abs/2505.22335)
*Wancai Zheng,Linlin Ou,Jiajie He,Libo Zhou,Xinyi Yu,Yan Wei*

Main category: cs.RO

TLDR: UP-SLAM是一种实时RGB-D SLAM系统，通过并行化框架解耦跟踪与映射，利用概率八叉树管理高斯基元，无需手工阈值。提出无训练不确定性估计器和时间编码器，提升动态环境下的鲁棒性和渲染质量。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS技术在动态环境中的实时性和鲁棒性不足，UP-SLAM旨在解决这些问题。

Method: 采用并行化框架、概率八叉树管理高斯基元，提出无训练不确定性估计器和时间编码器，结合DINO特征增强鲁棒性。

Result: 在定位精度（提升59.8%）和渲染质量（提升4.57 dB PSNR）上优于现有方法，保持实时性能。

Conclusion: UP-SLAM在动态环境中表现优异，提供高精度定位和高质量渲染，适用于实际场景。
...
### [328] [tenSVD algorithm for compression](https://arxiv.org/abs/2505.21686)
*Michele Gallo*

Main category: stat.CO

TLDR: 本文提出了一种基于张量的高效图像存储方法，旨在减少存储内存、传输带宽和处理能耗。通过将数据组织为高阶张量并应用Tucker模型进行压缩，该方法在计算时间和信息保留质量上表现优异。


<details>
  <summary>Details</summary>
Motivation: 张量为高维数据管理提供了强大框架，但现有方法在存储、传输和能耗方面存在不足。本研究旨在通过张量压缩技术解决这些问题。

Method: 将原始数据组织为高阶张量，应用Tucker模型进行压缩，并在R中实现。与基线算法对比，评估计算时间和信息保留质量。

Result: 实验结果表明，该方法在计算效率和信息保留方面优于基线算法，同时显著降低了能耗。

Conclusion: 基于张量的压缩方法在图像存储中具有高效性和可持续性，适用于多种应用场景。
...
### [329] [Scientific Paper Retrieval with LLM-Guided Semantic-Based Ranking](https://arxiv.org/abs/2505.21815)
*Yunyi Zhang,Ruozhen Yang,Siqi Jiao,SeongKu Kang,Jiawei Han*

Main category: cs.IR

TLDR: SemRank结合LLM引导的查询理解和基于概念的语义索引，显著提升科学论文检索的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有密集检索方法难以捕捉细粒度科学概念，而基于LLM的方法缺乏语料库知识支持，导致内容不可靠。

Method: 提出SemRank框架，通过LLM识别查询中的核心概念，并结合多粒度科学概念索引实现精确语义匹配。
...
### [330] [Complexity counts: global and local perspectives on Indo-Aryan numeral systems](https://arxiv.org/abs/2505.21510)
*Chundra Cathcart*

Main category: physics.soc-ph

TLDR: 本文研究了印欧语系语言（如印地语、古吉拉特语和孟加拉语）中1-99数字系统的非透明性，并探讨了其复杂性的成因。


<details>
  <summary>Details</summary>
Motivation: 印欧语系语言的数字系统与其他语言（如英语、汉语）不同，其1-99的数字形式无法通过简单规则构造，这种复杂性值得研究。

Method: 通过跨语言数据库数据，开发并应用量化数字系统复杂性的指标，分析印欧语系语言的复杂性及其成因。

Result: 印欧语系语言的数字系统复杂性显著高于世界其他语言，且内部差异明显；宗教、地理隔离等因素影响了复杂性的形成。

Conclusion: 尽管印欧语系数字系统高度复杂，但仍遵循高效通信的普遍原则；呼吁在跨语言数字系统研究中重视这一复杂性维度。
...
### [331] [Fluent but Culturally Distant: Can Regional Training Teach Cultural Understanding?](https://arxiv.org/abs/2505.21548)
*Dhruv Agarwal,Anya Shukla,Sunayana Sitaram,Aditya Vashistha*

Main category: physics.soc-ph

TLDR: 研究发现，即使是针对特定地区开发的LLMs，其文化价值观和实践与全球模型相比并未更贴近当地文化，甚至不如普通美国人更能代表印度文化。


<details>
  <summary>Details</summary>
Motivation: 解决LLMs在文化上的西方倾向问题，评估区域LLMs是否真正反映当地文化。

Method: 以印度为例，评估五个印度本地和五个全球LLMs，通过价值观和实践两个维度进行分析。

Result: 印度本地模型在文化价值观和实践上与全球模型无显著差异，甚至表现更差。

Conclusion: 需更多高质量、文化相关的数据来提升LLMs的文化代表性，文化评估应成为LLMs开发的核心要求。
...
### [332] [VietASR: Achieving Industry-level Vietnamese ASR with 50-hour labeled data and Large-Scale Speech Pretraining](https://arxiv.org/abs/2505.21527)
*Jianheng Zhuo,Yifan Yang,Yiwen Shao,Yong Xu,Dong Yu,Kai Yu,Xie Chen*

Main category: eess.AS

TLDR: VietASR提出了一种新的ASR训练流程，利用大量未标记数据和少量标记数据，通过多轮自监督学习提升低资源语言（如越南语）的语音识别性能。
...
Method: 采用多轮ASR偏置的自监督学习，在大规模未标记数据上进行预训练，再在小规模标记数据上微调。

Result: 仅用50小时标记数据和70,000小时未标记数据训练的轻量级模型，性能优于Whisper Large-v3和商业ASR系统。
...
### [333] [Evaluation of LLMs in Speech is Often Flawed: Test Set Contamination in Large Language Models for Speech Recognition](https://arxiv.org/abs/2505.22251)
*Yuan Tseng,Titouan Parcollet,Rogier van Dalen,Shucong Zhang,Sourav Bhattacharya*

Main category: eess.AS

TLDR: 研究发现LibriSpeech和Common Voice评估集中的大量数据出现在LLM预训练语料中，质疑了基于这些数据集的可靠性。实验表明，受污染的LLM更可能生成训练中见过的句子，导致输出偏差。
...
Method: 比较受污染和未受污染的LLM，分析其对测试句子的生成倾向及语音识别错误率的影响。

Result: 受污染的LLM更易生成训练见过的句子，语音识别错误率差异微小，但对见过的转录赋予更高概率。
